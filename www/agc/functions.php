...
function user_authorization($user, $pass, $user_option, $user_update, $bcrypt, $return_hash, $api_call, $source)
{
    ...
    if ($auth < 1) {
        ...
        if ($cl_user_ct > 0) {
            ...
            if ($failed_login_count < $LOCK_trigger_attempts) {
                ...
                if ($LOCK_over > $last_login_date) {
                    ...
                    $rslt=mysql_to_mysqli($stmt, $link);
                    if ($mel > 0) {
                        mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '05027', $user, $server_ip, $session_name, $one_mysql_log);
                    }
                }
            }
        }
        ...
        if ($login_problem < 1) {
            ...
            $rslt=mysql_to_mysqli($stmt, $link);
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '05028', $user, $server_ip, $session_name, $one_mysql_log);
            }
        }
    }
    return $auth_key;
...
function custom_list_fields_values($lead_id, $list_id, $uniqueid, $user, $DB, $call_id, $did_id, $did_extension, $did_pattern, $did_description, $dialed_number, $dialed_label, $only_field, $source_field, $source_field_value)
{
    ...
    if ($tablecount_to_print > 0) {
        ...
        if ($fieldscount_to_print > 0) {
            ...
            if ($list_lead_ct > 0) {
                ...
                if ($DB) {
                    $CFoutput .= _QXZ("ERROR: no custom data for this lead: ")."$lead_id\n";
                }
            }
                    ...
                    if ($A_field_type[$o]=='SOURCESELECT') {
                        ...
                        if (!preg_match("/^source=>/i", $field_options_array[0])) {
                            ...
                            if (preg_match("/\|$sourceselect_field\|/i", $vicidial_list_fields)) {
                                ...
                                if (preg_match("/^gender$/i", $sourceselect_field)) {
                                    $sourceselect_type='SELECT';
                                }
                                ...
                                $rslt=mysql_to_mysqli($stmt, $link);
                                if ($mel > 0) {
                                    mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '05XXX', $user, $server_ip, $session_name, $one_mysql_log);
                                }
                                if ($DB) {
                                    echo "$stmt\n";
                                }
                            ...
                            if ((strlen($only_field) > 0) and (strlen($source_field_value) > 0) and (preg_match("/^$sourceselect_field$/i", $source_field))) {
                                ...
                                if ($DB) {
                                    echo "Using form field value: |$only_field|$source_field_value|$source_field|\n";
                                }
                                if ($sourceselect_value == '--BLANK--') {
                                    $sourceselect_value='';
                                }
                            } else {
                                $rslt=mysql_to_mysqli($stmt, $link);
                            ...
                            if ($DB) {
                                echo "Starting SOURCESELECT: |$sourceselect_field|$sourceselect_value|$temp_sourcematch|   |$only_field|$source_field_value|$source_field|\n";
                            }
                            $temp_matchfound=0;
                            $NFA=0;
                            while (($te < $field_options_count) and ($temp_matchfound < 2)) {
                                ...
                                if ($DB) {
                                    echo "SOURCESELECT 2: $te|$field_options_array[$te]($temp_sourcematch)|$temp_matchfound|\n";
                                }
                                $te++;
                            }
                            if ($NFA < 1) {
                            ...
                            if ($DB) {
                                echo "SOURCESELECT 3: $te|$field_options_count|$field_options_array[0]|\n";
                            }
                        }
                    }
                    $te=0;
    ...
    if (preg_match("/--A--|--U--|--M--/", $CFoutput)) {
        ...
        if ($gender == 'U') {
            $CFoutput = preg_replace('/--A--genderUSELECT--B--/i', 'SELECTED', $CFoutput);
        }
        if ($gender == 'M') {
            $CFoutput = preg_replace('/--A--genderMSELECT--B--/i', 'SELECTED', $CFoutput);
        }
        if ($gender == 'F') {
            $CFoutput = preg_replace('/--A--genderFSELECT--B--/i', 'SELECTED', $CFoutput);
        }
        ...
        if ((preg_match("/--M--/", $CFoutput)) or (preg_match("/--N--/", $CFoutput))) {
            if (!preg_match("/^4|^5|^6/", $php_version)) {
                ...
                while ($MathMatch_count > $Mct) {
                    ...
                    if ($DBM > 0) {
                        $CFoutput .= "MATH DEBUG: $Mct|$temp_MathEq|$temp_MathResult|\n";
                    }
