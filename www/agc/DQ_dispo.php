...
if ($DB>0) {
    echo "$lead_id|$dispo|$campaign_id|$user|$pass|$DB|$log_to_file|\n";
}
if (strlen($dispo) < 1) {
...
if (strlen($lead_id) > 0) {
    ...
    while ($DQ_settings_ct >= $sea) {
        if (preg_match("/^finished_statuses=>|^finished_statuses => /", $DQ_settings[$sea])) {
            ...
            if ($DB) {
                print "DEBUG finished_statuses defined: |$finished_statuses|$finished_statusesSQL| \n";
            }
        }
        $sea++;
    }
    ...
    if (preg_match("/'$dispo'/", $finished_statusesSQL)) {
        if ($DB) {
            print "DEBUG finished_statuses match with dispo: |$dispo|$finished_statusesSQL| \n";
        }
        ...
        if ($VLrows > 0) {
            ...
            while ($DQc <= 20) {
                ...
                if ($VDQGrows > 0) {
                    ...
                    if ($DB) {
                        print "DEBUG DQ field value match: |$DQfields[$DQc]|$DQfield_values[$DQc]|   |$quota_goal|$quota_count|$quota_status| \n";
                    }
                    if ($new_quota_count == $quota_goal) {
                        $DQgoals_reached++;
                        if ($DB) {
                            print "DEBUG DQ goal reached: |$new_quota_count == $quota_goal| \n";
                        }
                    }
                    ...
                    if ($DB) {
                        print "DEBUG DQ quota goals updated: |$affected_rows|$last_list_id| \n";
                    }
                }
                $DQc++;
            }
