...
if (isset($_GET["calls_in_queue_option"])) {
    $calls_in_queue_option=$_GET["calls_in_queue_option"];
} elseif (isset($_POST["calls_in_queue_option"])) {
    $calls_in_queue_option=$_POST["calls_in_queue_option"];
}
if (isset($_GET["calls_in_queue_display"])) {
    $calls_in_queue_display=$_GET["calls_in_queue_display"];
} elseif (isset($_POST["calls_in_queue_display"])) {
    $calls_in_queue_display=$_POST["calls_in_queue_display"];
}
if (isset($_GET["DB"])) {
    $DB=$_GET["DB"];
...
if ($mel > 0) {
    mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09001', $user, $server_ip, $session_name, $one_mysql_log);
}
...
$rslt=mysql_to_mysqli($stmt, $link);
if ($mel > 0) {
    mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09002', $user, $server_ip, $session_name, $one_mysql_log);
}
...
$rslt=mysql_to_mysqli($stmt, $link);
if ($mel > 0) {
    mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09003', $user, $server_ip, $session_name, $one_mysql_log);
}
...
if ($ACTION == 'top_panel') {
    ...
    $rslt=mysql_to_mysqli($stmt, $link);
    if ($mel > 0) {
        mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09004', $user, $server_ip, $session_name, $one_mysql_log);
    }
    ...
    if ($cl_user_ct > 0) {
        ...
        if (($VLAstatus == 'PAUSED') and ($VLApreview_lead_id > 0)) {
            $live_call = 'PREVIEW';
        }
    } else {
    ...
    if (strlen($VLAcallerid) > 5) {
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($mel > 0) {
            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09005', $user, $server_ip, $session_name, $one_mysql_log);
        }
        ...
        if ((($calls_inqueue_count_one != '') and ($calls_inqueue_count_one != 'DISABLED')) or (($calls_inqueue_count_two != '') and ($calls_inqueue_count_two != 'DISABLED'))) {
            ...
            if ($SCinfo_ct > 0) {
                ...
                if (strlen($cic_one_stmt) > 10) {
                    ...
                    if ($DB) {
                        echo "CIC ONE|$TEMPcic_one_stmt|\n";
                    }
                    $rslt=mysql_to_mysqli($TEMPcic_one_stmt, $link);
                    if ($mel > 0) {
                        mysql_error_logging($NOW_TIME, $link, $mel, $TEMPcic_one_stmt, '03XXX', $user, $server_ip, $session_name, $one_mysql_log);
                    }
                    $row=mysqli_fetch_row($rslt);
                    $RingCallsOne=$row[0];
                    if ($RingCallsOne > 0) {
                        ...
                        if ($DB) {
                            echo "CIC ONETIME|$stmt|\n";
                        }
                        $rslt=mysql_to_mysqli($stmt, $link);
                        if ($mel > 0) {
                            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09XXX', $user, $server_ip, $session_name, $one_mysql_log);
                        }
                        $cl_calls_ct = mysqli_num_rows($rslt);
                        if ($cl_calls_ct > 0) {
                            $row=mysqli_fetch_row($rslt);
            ...
            if ($SCinfo_ct > 0) {
                ...
                if (strlen($cic_two_stmt) > 10) {
                    ...
                    if ($DB) {
                        echo "CIC TWO|$TEMPcic_two_stmt|\n";
                    }
                    $rslt=mysql_to_mysqli($TEMPcic_two_stmt, $link);
                    if ($mel > 0) {
                        mysql_error_logging($NOW_TIME, $link, $mel, $TEMPcic_two_stmt, '03XXX', $user, $server_ip, $session_name, $two_mysql_log);
                    }
                    $row=mysqli_fetch_row($rslt);
                    $RingCallsTwo=$row[0];
                    if ($RingCallsTwo > 0) {
    ...
    if ((strlen($RingCallsOne) < 10) and (strlen($RingCallsTwo) < 10)) {
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($mel > 0) {
            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09006', $user, $server_ip, $session_name, $one_mysql_log);
        }
        $cl_calls_ct = mysqli_num_rows($rslt);
        if ($cl_calls_ct > 0) {
            $row=mysqli_fetch_row($rslt);
        ...
        if ($CIQcount > 0) {
            ...
            $rslt=mysql_to_mysqli($stmt, $link);
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09007', $user, $server_ip, $session_name, $one_mysql_log);
            }
            $cl_calls_ct = mysqli_num_rows($rslt);
            if ($cl_calls_ct > 0) {
                $row=mysqli_fetch_row($rslt);
...
if ($ACTION == 'agent_status') {
    ...
    $rslt=mysql_to_mysqli($stmt, $link);
    if ($mel > 0) {
        mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09008', $user, $server_ip, $session_name, $one_mysql_log);
    }
    $cl_user_ct = mysqli_num_rows($rslt);
    if ($cl_user_ct > 0) {
        $row=mysqli_fetch_row($rslt);
        ...
        if (($VLAstatus == 'INCALL') or ($VLAstatus == 'QUEUE') or ($VLAstatus == 'MQUEUE')) {
            $agent_status = 'DEAD';
        }
        if (($VLAstatus == 'PAUSED') and ($VLAlead_id > 0)) {
            $agent_status = 'DISPO';
        }
        if (($VLAstatus == 'PAUSED') and ($VLApreview_lead_id > 0)) {
            $agent_status = 'PREVIEW';
        }
        if (($VLAstatus == 'PAUSED') and ($VLApause_code == 'LAGGED')) {
            $agent_status = 'LAGGED';
        }
        if (($VLAstatus == 'PAUSED') and ($agent_status == 'NONE')) {
            $agent_status = 'PAUSED';
        }
        if ($VLAstatus == 'READY') {
            $agent_status = 'READY';
    ...
    if (strlen($VLAcallerid) > 15) {
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($mel > 0) {
            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09009', $user, $server_ip, $session_name, $one_mysql_log);
        }
        $cl_calls_ct = mysqli_num_rows($rslt);
        if ($cl_calls_ct > 0) {
            $row=mysqli_fetch_row($rslt);
    ...
    $rslt=mysql_to_mysqli($stmt, $link);
    if ($mel > 0) {
        mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09010', $user, $server_ip, $session_name, $one_mysql_log);
    }
    $cl_calls_ct = mysqli_num_rows($rslt);
    if ($cl_calls_ct > 0) {
        $row=mysqli_fetch_row($rslt);
    ...
    if ($CIQcount > 0) {
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($mel > 0) {
            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '09011', $user, $server_ip, $session_name, $one_mysql_log);
        }
        $cl_calls_ct = mysqli_num_rows($rslt);
        if ($cl_calls_ct > 0) {
            $row=mysqli_fetch_row($rslt);
