...
class Evaluator
{
    ...
    private function getVariable(string $name)
    {
        ...
        if (!isset($value)) {
            return "Math Error: Unknown variable";
        }
    ...
    {
        if ($argument == '') {
            return "Math Error: Empty argument";
        }
    ...
    private function subTerm()
    {
        ...
        while (in_array($char = $this->text[$this->pos] ?? false, ['*', '/', '^', '%'])) {
            ...
            switch ($char) {
                case '*':
                    if ((!is_numeric($value)) or (strlen($value) < 1) or (!is_numeric($term)) or (strlen($term) < 1)) {
                        return "Math Error: Empty variable";
                    }
    ...
    private function calculate()
    {
        ...
        while (in_array($char = $this->text[$this->pos] ?? false, ['+', '-'])) {
            ...
            if ((strlen($subTerm) < 1) or (!is_numeric($subTerm))) {
                return "Math Error: Empty variable";
            }
            //    throw new Exception('Empty variable', 7);
            if ($char == '-') {
    ...
    public function execute(string $formula)
    {
        ...
        if ($b != 0) {
            return "Math Error: Unmatched brackets";
        }
