...
if (isset($_GET["live_queue"])) {
    $live_queue=$_GET["live_queue"];
} elseif (isset($_POST["live_queue"])) {
    $live_queue=$_POST["live_queue"];
}
if (isset($_GET["live_queue_name"])) {
    $live_queue_name=$_GET["live_queue_name"];
} elseif (isset($_POST["live_queue_name"])) {
    $live_queue_name=$_POST["live_queue_name"];
}
if ($non_latin < 1) {
...
if (isset($_GET["widget_title"])) {
    $widget_title=$_GET["widget_title"];
} elseif (isset($_POST["widget_title"])) {
    $widget_title=$_POST["widget_title"];
}
if (isset($_GET["widget_text"])) {
    $widget_text=$_GET["widget_text"];
} elseif (isset($_POST["widget_text"])) {
    $widget_text=$_POST["widget_text"];
}
if (isset($_GET["widget_queue"])) {
    $widget_queue=$_GET["widget_queue"];
} elseif (isset($_POST["widget_queue"])) {
    $widget_queue=$_POST["widget_queue"];
}
if (isset($_GET["widget_agent"])) {
    $widget_agent=$_GET["widget_agent"];
} elseif (isset($_POST["widget_agent"])) {
    $widget_agent=$_POST["widget_agent"];
}
if (isset($_GET["widget_sla_level"])) {
    $widget_sla_level=$_GET["widget_sla_level"];
} elseif (isset($_POST["widget_sla_level"])) {
    $widget_sla_level=$_POST["widget_sla_level"];
}
if (isset($_GET["widget_alarms"])) {
    $widget_alarms=$_GET["widget_alarms"];
} elseif (isset($_POST["widget_alarms"])) {
    $widget_alarms=$_POST["widget_alarms"];
}
...
if ($vicidial_queue_group) {
    $queue_group=$vicidial_queue_group;
}
$TODAY=date("Y-m-d");
...
if ($widget_type=="LOST_PCT") {
    ...
    if ($DB) {
        echo "$lost_calls_stmt<BR>($lost_calls, $total_calls)";
    }
...
if ($widget_type=="ANSWERED_PCT") {
    ...
    if ($DB) {
        echo "$answered_calls_stmt<BR>($answered_calls, $total_calls)";
    }
...
if ($widget_type=="SLA_LEVEL_PCT") {
    ...
    if ($DB) {
        echo $svc_lvl_stmt."<BR>$svc_lvl_row[0], $svc_lvl_row[1]<BR><BR>";
    }
    if ($svc_lvl_row[1]>0) {
...
if ($widget_type=="LIVE_QUEUES") {
    ...
    while($vla_row=mysqli_fetch_array($vla_rslt)) {
        ...
        } elseif ($vla_row["status"]=="PAUSED") {
            $live_queue_rslts["$campaign_id"]["paused"]++;
            $paused++;
        } elseif(preg_match('/READY|CLOSER/', $vla_row["status"])) {
...
function GetQueueIngroups($queue_id)
{
    ...
    if ($DB) {
        echo $included_inbound_groups."<BR>\n";
    }
