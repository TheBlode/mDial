...
} elseif (isset($_POST["status_name"])) {
...
if (isset($_GET["sort_answered_details"])) {
    $sort_answered_details=$_GET["sort_answered_details"];
} elseif (isset($_POST["sort_answered_details"])) {
    $sort_answered_details=$_POST["sort_answered_details"];
}
...
if (!$start_datetime || !$end_datetime) {
    echo "Missing information - needs start date/time, end date/time";
    $die=1;
}
...
while ($row=mysqli_fetch_array($rslt)) {
    ...
    if ($status_names["$row[status]"]) {
        $row["status"]=$status_names["$row[status]"];
    }
