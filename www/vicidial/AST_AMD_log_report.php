...
if (isset($_GET["AMDSTATUS"])) {
    $AMDSTATUS=$_GET["AMDSTATUS"];
} elseif (isset($_POST["AMDSTATUS"])) {
    $AMDSTATUS=$_POST["AMDSTATUS"];
}
if (isset($_GET["AMDRESPONSE"])) {
    $AMDRESPONSE=$_GET["AMDRESPONSE"];
} elseif (isset($_POST["AMDRESPONSE"])) {
    $AMDRESPONSE=$_POST["AMDRESPONSE"];
}
...
while($i < $AMDSTATUS_ct) {
    ...
    while ($j < $AMDRESPONSE_ct) {
        if ((strlen($AMDSTATUS[$i]) > 0) and (preg_match("/\|$AMDSTATUS[$i]\|/", $AMDSTATUS_string)) and (strlen($AMDRESPONSE[$j]) > 0) and (preg_match("/\|$AMDRESPONSE[$j]\|/", $AMDRESPONSE_string))) {
            if (preg_match('/\-\-ALL\-\-/', $AMDSTATUS_string)) {
                $HC_subclause="";
            } else {
                $HC_subclause="AMDSTATUS='$AMDSTATUS[$i]'";
            }
            if (preg_match('/\-\-ALL\-\-/', $AMDRESPONSE_string)) {
                $DS_subclause="";
            } else {
                $DS_subclause="AMDRESPONSE='$AMDRESPONSE[$j]'";
            }
            if ($HC_subclause=="" || $DS_subclause=="") {
                $conjunction="";
            } else {
                $conjunction=" and ";
            }
