...
if (isset($_GET["web_ip"])) {
    $web_ip=$_GET["web_ip"];
} elseif (isset($_POST["web_ip"])) {
    $web_ip=$_POST["web_ip"];
}
...
if ($ACTION=="latency_by_user" && $user && $log_date && $web_ip) {
    if ($log_date!=date("Y-m-d")) {
        $latency_tbl="vicidial_agent_latency_log_archive";
    } else {
        $latency_tbl="vicidial_agent_latency_log";
    }
...
if ($ACTION=="latency_gaps" && $log_date) {
    if ($archive_flag) {
        $latency_tbl="vicidial_latency_gaps_archive";
    } else {
        $latency_tbl="vicidial_latency_gaps";
    }
    $group_array=array();
    $user_array=array();
    $full_name_array=array();
