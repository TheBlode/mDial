...
if (isset($_GET["exclude_rollover"])) {
    $exclude_rollover=$_GET["exclude_rollover"];
} elseif (isset($_POST["exclude_rollover"])) {
    $exclude_rollover=$_POST["exclude_rollover"];
}
if (isset($_GET["inbound_rate"])) {
    $inbound_rate=$_GET["inbound_rate"];
} elseif (isset($_POST["inbound_rate"])) {
...
if ($groups_to_print < 1) {
    ...
    if ($group_ct > 0) {
        ...
        while($i < $group_ct) {
            ...
            if ($answer_count[$i]>$max_answer) {
                $max_answer=$answer_count[$i];
            }
            if ($talk_sec[$i]>$max_talk) {
                $max_talk=$talk_sec[$i];
            }
            if ($talk_avg[$i]>$max_avgtalk) {
                $max_avgtalk=$talk_avg[$i];
            }
            if ($queue_seconds[$i]>$max_queue) {
                $max_queue=$queue_seconds[$i];
            }
            if ($queue_avg[$i]>$max_avgqueue) {
                $max_avgqueue=$queue_avg[$i];
            }
            if ($max_queue_seconds[$i]>$max_maxqueue) {
                $max_maxqueue=$max_queue_seconds[$i];
            }
            if ($drop_count[$i]>$max_totalabandons) {
                $max_totalabandons=$drop_count[$i];
            }
            ...
            while ($h < 24) {
                if ($Hcalltime[$h] > 0) {
                    ...
                    if ($Hcalls_count[$h]>$sub_max_calls) {
                        $sub_max_calls=$Hcalls_count[$h];
                    }
                    if ($Hanswer_count[$h]>$sub_max_answer) {
                        $sub_max_answer=$Hanswer_count[$h];
                    }
                    if ($Htalk_sec[$h]>$sub_max_talk) {
                        $sub_max_talk=$Htalk_sec[$h];
                    }
                    if ($Htalk_avg[$h]>$sub_max_avgtalk) {
                        $sub_max_avgtalk=$Htalk_avg[$h];
                    }
                    if ($Hqueue_seconds[$h]>$sub_max_queue) {
                        $sub_max_queue=$Hqueue_seconds[$h];
                    }
                    if ($Hqueue_avg[$h]>$sub_max_avgqueue) {
                        $sub_max_avgqueue=$Hqueue_avg[$h];
                    }
                    if ($Hmax_queue_seconds[$h]>$sub_max_maxqueue) {
                        $sub_max_maxqueue=$Hmax_queue_seconds[$h];
                    }
                    if ($Hdrop_count[$h]>$sub_max_totalabandons) {
                        $sub_max_totalabandons=$Hdrop_count[$h];
                    }
                    $q++;
