...
if (($stage == 'empty') or (strlen($stage) < 1)) {
    ...
    if ($LOGmodify_campaigns > 0) {
        echo "<option SELECTED value='CAMPAIGNS'>"._QXZ("CAMPAIGNS")."</option>";
    }
    if ($LOGmodify_lists > 0) {
        echo "<option value='LISTS'>"._QXZ("LISTS")."</option>";
    }
    if ($LOGmodify_ingroups > 0) {
        echo "<option value='IN-GROUPS'>"._QXZ("IN-GROUPS")."</option>";
    }
    if ($LOGmodify_inbound_dids > 0) {
        echo "<option value='DIDS'>"._QXZ("DIDS")."</option>";
    }
    if ($LOGmodify_inbound_dids > 0) {
        echo "<option value='CALLMENUS'>"._QXZ("CALLMENUS")."</option>";
    }
    if ($LOGmodify_users > 0) {
        echo "<option value='USERS'>"._QXZ("USERS")."</option>";
    }
    if ($LOGmodify_usergroups > 0) {
        echo "<option value='USER-GROUPS'>"._QXZ("USER-GROUPS")."</option>";
    }
    if ($LOGmodify_phones > 0) {
        echo "<option value='PHONES'>"._QXZ("PHONES")."</option>";
    }
    if ($LOGmodify_servers > 0) {
        echo "<option value='SERVERS'>"._QXZ("SERVERS")."</option>";
    }
    if ($LOGmodify_shifts > 0) {
        echo "<option value='SHIFTS'>"._QXZ("SHIFTS")."</option>";
    }
    echo "</select>";
    ...
    if (strlen($stmt) > 10) {
        ...
        while ($ids_to_print > $i) {
            ...
            if ($stage == 'PHONES') {
                if ($first_id == "$row[0]-----$row[1]") {
                    $first_id_menu .= " SELECTED";
                    $first_found++;
                }
            ...
            if ($stage == 'PHONES') {
                if ($second_id == "$row[0]-----$row[1]") {
                    $second_id_menu .= " SELECTED";
                    $second_found++;
                }
        ...
        if ($first_found < 1) {
            $first_id_menu .= "<option SELECTED value=''>"._QXZ("Select first")." "._QXZ("$stage")." "._QXZ("entry")."</option>";
        }
        if ($second_found < 1) {
            $second_id_menu .= "<option SELECTED value=''>"._QXZ("Select second")." "._QXZ("$stage")." "._QXZ("entry")."</option>";
        }
        echo "<TABLE CELLPADDING=4 CELLSPACING=4 WIDTH=100%><TR><TD COLSPAN=2 border=0>";
        echo "<FORM ACTION=\"$PHP_SELF\" METHOD=GET ID='vicidial_report' NAME='vicidial_report'>\n";
        echo "<input type=hidden name=DB value=\"$DB\">\n";
        ...
        if ((strlen($first_id) > 0) and (strlen($second_id) > 0)) {
            ...
            if ($stage == 'IN-GROUPS') {
                $stmtA="SELECT * from vicidial_inbound_groups where group_id='$first_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'DIDS') {
                $stmtA="SELECT * from vicidial_inbound_dids where did_pattern='$first_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'CALLMENUS') {
            ...
            if ($stage == 'USER-GROUPS') {
                $stmtA="SELECT * from vicidial_user_groups where user_group='$first_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'PHONES') {
            ...
            if ($stage == 'SERVERS') {
                $stmtA="SELECT * from servers where server_ip='$first_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'SHIFTS') {
                $stmtA="SELECT * from vicidial_shifts where shift_id='$first_id' $LOGadmin_viewable_groupsSQL;";
            }
            $stmtB='';
            $stmtBB='';
            $countBB='';
            ...
            if ($stage == 'IN-GROUPS') {
                $stmtB="SELECT * from vicidial_inbound_groups where group_id='$second_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'DIDS') {
                $stmtB="SELECT * from vicidial_inbound_dids where did_pattern='$second_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'CALLMENUS') {
            ...
            if ($stage == 'USER-GROUPS') {
                $stmtB="SELECT * from vicidial_user_groups where user_group='$second_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'PHONES') {
            ...
            if ($stage == 'SERVERS') {
                $stmtB="SELECT * from servers where server_ip='$second_id' $LOGadmin_viewable_groupsSQL;";
            }
            if ($stage == 'SHIFTS') {
                $stmtB="SELECT * from vicidial_shifts where shift_id='$second_id' $LOGadmin_viewable_groupsSQL;";
            }
                ...
                if (strlen($stmtAA) > 10) {
                    ...
                    while (($ids_to_print > $i) and ($i < 1000)) {
                        ...
                        while($fields > $j) {
                            ...
                            if ($DB) {
                                echo "$i   Fields: $fields|J: $j|col: $col|value: $value\n";
                            }
                            $j++;
                        }
                        $i++;
