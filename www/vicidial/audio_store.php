...
if (isset($_GET["audio_server_ip"])) {
    $audio_server_ip=$_GET["audio_server_ip"];
} elseif (isset($_POST["audio_server_ip"])) {
    $audio_server_ip=$_POST["audio_server_ip"];
}
...
if (isset($_GET["audiofile_name"])) {
    $audiofile_name=$_GET["audiofile_name"];
} elseif (isset($_POST["audiofile_name"])) {
    $audiofile_name=$_POST["audiofile_name"];
}
if (isset($_GET["master_audiofile"])) {
    $master_audiofile=$_GET["master_audiofile"];
} elseif (isset($_POST["master_audiofile"])) {
    $master_audiofile=$_POST["master_audiofile"];
}
if (isset($_GET["new_audiofile"])) {
    $new_audiofile=$_GET["new_audiofile"];
} elseif (isset($_POST["new_audiofile"])) {
    $new_audiofile=$_POST["new_audiofile"];
}
...
if (isset($_GET["force_allow"])) {
    $force_allow=$_GET["force_allow"];
} elseif (isset($_POST["force_allow"])) {
    $force_allow=$_POST["force_allow"];
}
...
if (strlen($sounds_web_directory) < 30) {
    ...
    if ($DB > 0) {
        echo "$WeBServeRRooT/$sounds_web_directory\n";
    }
...
if (($wuv_ct > 0) or ($action == "ALL_WAV_VALIDATION")) {
    if ($DB) {
        echo "Starting WAV file validation process...\n";
    }
    $i=0;
    $filename_sort=$MT;
    $dirpath = "$WeBServeRRooT/$sounds_web_directory";
    ...
    while($k < $i) {
        ...
        if ($size == $audio_filesize) {
            ...
            if (preg_match("/\.wav$/", $audio_filename)) {
                ...
                $invalid_wav=0;
                if (!preg_match('/^wav/i', $wav_type)) {
                    $invalid_wav++;
                }
                if (!preg_match('/^pcm/i', $wav_compression)) {
                    $invalid_wav++;
                }
                if ($wav_channels > 1) {
                    $invalid_wav++;
                }
                if (($wav_framerate > 8000) or ($wav_framerate < 8000)) {
                    $invalid_wav++;
                }
                if (($wav_bits > 16) or ($wav_bits < 16)) {
                    $invalid_wav++;
                }
            ...
            if ($DB) {
                echo "$k\t$file_names[$m]\t$wav_asterisk_valid\n";
            }
        }
        $k++;
    }
    if ($DB) {
        echo "SUMMARY: WAV VALID: $wav_valid   WAV INVALID: $wav_invalid   NOT WAV: $not_wav\n";
    }
}
...
if ($action == "COPYFILE") {
    if ($DB) {
        echo "COPYFILE: |$new_audiofile|$master_audiofile|\n";
    }
