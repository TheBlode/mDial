...
if (!$group) {
    ...
    while ($i < $users_to_print) {
        ...
        if ($LOGadmin_hide_phone_data != '0') {
            if ($DB > 0) {
                $DBdata .= "HIDEPHONEDATA|$row[1]|$LOGadmin_hide_phone_data|   ";
            }
            $phone_temp = $row[1];
        ...
        if ($LOGadmin_hide_lead_data != '0') {
            if ($DB > 0) {
                $DBdata .= "HIDELEADDATA|$row[2]|$LOGadmin_hide_lead_data|   ";
            }
            if (strlen($row[2]) > 0) {
                $state_temp = $row[2];
                $row[2] = preg_replace("/./", 'X', $state_temp);
            }
            if (strlen($row[11]) > 0) {
                $vlc_temp = $row[11];
                $row[11] = preg_replace("/./", 'X', $vlc_temp);
            }
        }
        $FMT_i =        sprintf("%-4s", $i);
        ...
        if ($DB > 0) {
            $DBdata .= "GMT: $lead_offset($gmt|$SERVER_GMT)|LC: $last_call_epoch($row[14])|$last_call_age|   ";
        }
        if ($last_call_age < 24) {
        ...
        if ($DB) {
            echo "| $FMT_i | $priority | <a href='./admin_modify_lead.php?lead_id=$lead_id_ns&archive_search=No&archive_log=0'>$lead_id</a> | <a href='./admin.php?ADD=311&list_id=$list_id_ns'>$list_id</a> | $phone_number  $phone_code || $state | $status | $count | $gmt | $rank | $alt_dial | $source | $vendor_lead_code | $lead_age | $last_call_age_TEXT | $hopper_id | $owner | $DBdata\n";
        } else {
            echo "| $FMT_i | $priority | <a href='./admin_modify_lead.php?lead_id=$lead_id_ns&archive_search=No&archive_log=0'>$lead_id</a> | <a href='./admin.php?ADD=311&list_id=$list_id_ns'>$list_id</a> | $phone_number | $phone_code | $state | $status | $count | $gmt | $rank | $alt_dial | $source | $vendor_lead_code $source_id_TEXT| $lead_age | $last_call_age_TEXT |\n";
        }
        $i++;
    }
    echo "+------+--------+-----------+------------+-------------+---------+-------+--------+-------+--------+--------+-------+-------+----------------------$SIDhead+----------+-----------+\n";
