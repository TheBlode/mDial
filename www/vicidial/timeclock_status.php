...
while ($users_to_print > $o) {
    ...
    if ($DB>0) {
        $MAIN.="|$Tlogin_sec[$o]|$Tlogin_time[$o]|";
    }
...
while ($users_to_print > $o) {
    if (($Tlogin_sec[$o] > 0) or (strlen($Vevent_time[$o]) > 0)) {
        if (($Tstatus[$o]=='START') or ($Tstatus[$o]=='LOGIN')) {
            if ($Tlogin_sec[$o] > 0) {
                $bgcolor[$o]='bgcolor="#FFFF33"';
                $CSV_status=_QXZ("TC Logged in only");
            } # yellow
            if (($Tlogin_sec[$o] > 0) and (strlen($Vevent_time[$o]) > 0)) {
                $bgcolor[$o]='bgcolor="#99FF33"';
                $CSV_status=_QXZ("TC Logged in and VICI active");
            } # green
        } else {
            if ($Tlogin_sec[$o] > 0) {
                $bgcolor[$o]='bgcolor="#CCCC00"';
                $CSV_status=_QXZ("TC Logged out only");
            } # yellow
            if (strlen($Vevent_time[$o]) > 0) {
                $bgcolor[$o]='bgcolor="#FF6666"';
                $CSV_status=_QXZ("VICI active only");
            } # red
            if (($Tlogin_sec[$o] > 0) and (strlen($Vevent_time[$o]) > 0)) {
                $bgcolor[$o]='bgcolor="#66CC66"';
                $CSV_status=_QXZ("TC Logged out and VICI active");
            } # green
        }
        $s++;
        $MAIN.="<tr $bgcolor[$o]>";
