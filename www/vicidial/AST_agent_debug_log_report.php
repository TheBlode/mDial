    ...
    if (mysqli_num_rows($rpt_rslt)>0) {
        ...
        for ($i=1; $i<=mysqli_num_rows($rpt_rslt); $i++) {
            ...
            if ($i>=$lower_limit && $i<=$upper_limit) {
                ...
                if (strlen($row["php_script"])>20) {
                    $row["php_script"]=substr($row["php_script"], -20)."";
                }
                $agntdb_rpt.=" | ".sprintf("%-20s", $row["php_script"]);
                if (strlen($row["action"])>32) {
                    $row["action"]=substr($row["action"], 0, 32)."...";
                }
                $agntdb_rpt.=" | ".sprintf("%-35s", $row["action"]);
                $agntdb_rpt.=" | ".sprintf("%-10s", $row["lead_id"]);
                if (strlen($row["stage"])>67) {
                    $row["stage"]=substr($row["stage"], 0, 67)."...";
                }
