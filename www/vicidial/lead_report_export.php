...
if (isset($_GET["did_filter"])) {
    $did_filter=$_GET["did_filter"];
} elseif (isset($_POST["did_filter"])) {
    $did_filter=$_POST["did_filter"];
}
if (isset($_GET["vlc_filter"])) {
    $vlc_filter=$_GET["vlc_filter"];
} elseif (isset($_POST["vlc_filter"])) {
    $vlc_filter=$_POST["vlc_filter"];
}
if (isset($_GET["dids"])) {
    $dids=$_GET["dids"];
...
if (isset($_GET["run_export"])) {
    $run_export=$_GET["run_export"];
} elseif (isset($_POST["run_export"])) {
    $run_export=$_POST["run_export"];
}
if (isset($_GET["header_row"])) {
    $header_row=$_GET["header_row"];
} elseif (isset($_POST["header_row"])) {
    $header_row=$_POST["header_row"];
}
if (isset($_GET["rec_fields"])) {
    $rec_fields=$_GET["rec_fields"];
} elseif (isset($_POST["rec_fields"])) {
    $rec_fields=$_POST["rec_fields"];
}
...
} elseif (isset($_POST["export_fields"])) {
...
if (isset($_GET["vicidial_list_archive_only"])) {
    $vicidial_list_archive_only=$_GET["vicidial_list_archive_only"];
} elseif (isset($_POST["vicidial_list_archive_only"])) {
    $vicidial_list_archive_only=$_POST["vicidial_list_archive_only"];
}
if (isset($_GET["VLC_enabled"])) {
    $VLC_enabled=$_GET["VLC_enabled"];
} elseif (isset($_POST["VLC_enabled"])) {
    $VLC_enabled=$_POST["VLC_enabled"];
}
if (isset($_GET["exclude_call_log_data"])) {
    $exclude_call_log_data=$_GET["exclude_call_log_data"];
} elseif (isset($_POST["exclude_call_log_data"])) {
    $exclude_call_log_data=$_POST["exclude_call_log_data"];
}
...
if ($run_export > 0) {
    ...
    } # If DID searching is utilized, automatically switch to EXTENDED view
    ...
    if (($vlc_filter=="NO") or ($VLC_enabled != 'Y')) {
        $vlc_SQL="";
        $RUNvlc=0;
    }
    ...
    if ($RUNgroup > 0 || $RUNdid > 0) {
        if ($did_filter=="YES") {
            $export_fields_SQL .= ",vdl.extension";
        }
                ...
                if ($export_fields == 'EXTENDED') {
                    ...
                    if ($did_filter=="YES") {
                        if ($RUNdid>0) {
                            ...
                            if ($DB>0) {
                                echo "A) $did_stmt - ".mysqli_num_rows($did_rslt)." - $start_epoch<BR>\n";
                            }
                            ...
                            if ($DB>0) {
                                echo "B) $log_stmt - ".mysqli_num_rows($log_rslt)." - $end_epoch<BR>\n";
                            }
                        }
                        ...
                        if ($customfield_ct > 0) {
                            ...
                            while ($columns_ct >= $t) {
                                if ($enc_fields > 0) {
                                    ...
                                    $field_enc_all='';
                                    if ($DB) {
                                        echo "|$column_list|$encrypt_list|$hide_list|\n";
                                    }
    ...
    if ($did_filter) {
        echo "<option value='$did_filter' selected>$did_filter</option>";
    }
    ...
    if ($VLC_enabled == 'Y') {
        ...
        if ($vlc_filter) {
            echo "<option value='$vlc_filter' selected>$vlc_filter</option>";
        }
        echo "<option value='NO'>"._QXZ("NO")."</option><option value='YES'>"._QXZ("YES")."</option></select>\n";
        echo "<BR><BR>\n";
    }
