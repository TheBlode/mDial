...
if (!isset($order_by)) {
    $order_by="monitor_start_time-asc";
}
if (strlen($shift)<2) {
    $shift='ALL';
}
...
while ($managers_to_print > $o) {
    if  (preg_match("/$manager_list[$o]\|/i", $managers_string)) {
        $MAIN.="<option selected value=\"$manager_list[$o]\">$manager_list[$o] - $manager_names[$o]</option>\n";
    } else {
        $MAIN.="<option value=\"$manager_list[$o]\">$manager_list[$o] - $manager_names[$o]</option>\n";
    }
    $o++;
}
$MAIN.="</SELECT>\n";
...
if ($agents_selected==0 && $campaigns_selected==0) {
    ...
    if (mysqli_num_rows($log_rslt)>0) {
        ...
        while ($log_row=mysqli_fetch_array($log_rslt)) {
            if ($q%2==0) {
                $tdclass="$SSstd_row1_background";
            } else {
                $tdclass="$SSstd_row2_background";
            }
