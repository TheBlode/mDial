...
if (isset($_GET["begin_query_time"])) {
    $begin_query_time=$_GET["begin_query_time"];
} elseif (isset($_POST["begin_query_time"])) {
    $begin_query_time=$_POST["begin_query_time"];
}
if (isset($_GET["end_query_time"])) {
    $end_query_time=$_GET["end_query_time"];
} elseif (isset($_POST["end_query_time"])) {
    $end_query_time=$_POST["end_query_time"];
}
if (isset($_GET["group"])) {
    $group=$_GET["group"];
...
$STARTtime = date("U");
if (strlen($begin_query_time) < 10) {
    $begin_query_time = "$NOW_DATE 09:00:00";
}
if (strlen($end_query_time) < 10) {
    $end_query_time = "$NOW_DATE 15:30:00";
}
if (!isset($group)) {
    $group = '';
}
...
if (!$group) {
    ...
    if ($i > 1000) {
        $time_scale_abb = '10 '._QXZ("minutes");
        $time_scale_tick = '2 '._QXZ("minutes");
    }
    if ($i > 1500) {
        $time_scale_abb = '15 '._QXZ("minutes");
        $time_scale_tick = '3 '._QXZ("minutes");
    }
    if ($i > 2000) {
        $time_scale_abb = '20 '._QXZ("minutes");
        $time_scale_tick = '4 '._QXZ("minutes");
    }
    if ($i > 3000) {
        $time_scale_abb = '30 '._QXZ("minutes");
        $time_scale_tick = '5 '._QXZ("minutes");
    }
    if ($i > 4000) {
