...
} elseif (isset($_POST["email_type"])) {
    $email_type=$_POST["email_type"];
}
if (isset($_GET["date_type"])) {
    $date_type=$_GET["date_type"];
} elseif (isset($_POST["date_type"])) {
...
if (!$date_type || $date_type=="email_date") {
    $emailed="checked";
    $date_title="received";
}
if ($date_type=="call_date") {
    $called="checked";
    $date_title="viewed";
}
if ($date_type=="date_answered") {
    $answrd="checked";
    $date_title="answered";
}
...
if (!$email_type || $email_type=="received") {
    $rcvd="checked";
    $date_type="email_date";
    $email_title="received";
}
if ($email_type=="viewed") {
    $view="checked";
    $email_title="viewed";
}
if ($email_type=="answered") {
    $answ="checked";
    $email_title="answered";
}
...
if ($groups_to_print < 1) {
    ...
    if ($email_type=="received") {
        ...
        } elseif ($date_type=="date_answered") {
    ...
    if (mysqli_num_rows($rslt)>0) {
        ...
        while($row=mysqli_fetch_array($rslt)) {
            ...
            if (mb_strlen($email_from, 'UTF-8')>27) {
                $email_from=mb_substr($email_from, 0, 27, 'UTF-8')."...";
            }
            $sender_name=$row["email_from_name"];
            if (mb_strlen($sender_name, 'UTF-8')>17) {
                $sender_name=mb_substr($sender_name, 0, 17, 'UTF-8')."...";
            }
            $message=preg_replace('/\r|\n/', '', strip_tags($row["message"]));
            if (mb_strlen($message, 'UTF-8')>47) {
                $message=mb_substr($message, 0, 47, 'UTF-8')."...";
            }
            $call_date=$row["call_date"];
            $date_answered=$row["date_response_sent"];
            $sent_message=preg_replace('/\r|\n/', ' ', strip_tags($row["sent_message"]));
            if (mb_strlen($sent_message, 'UTF-8')>47) {
                $sent_message=mb_substr($sent_message, 0, 47, 'UTF-8')."...";
            }
