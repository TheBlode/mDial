...
if (isset($_GET["hourly_breakdown"])) {
    $hourly_breakdown=$_GET["hourly_breakdown"];
} elseif (isset($_POST["hourly_breakdown"])) {
    $hourly_breakdown=$_POST["hourly_breakdown"];
}
if (isset($_GET["show_disposition_statuses"])) {
    $show_disposition_statuses=$_GET["show_disposition_statuses"];
} elseif (isset($_POST["show_disposition_statuses"])) {
    $show_disposition_statuses=$_POST["show_disposition_statuses"];
}
if (isset($_GET["ignore_afterhours"])) {
    $ignore_afterhours=$_GET["ignore_afterhours"];
} elseif (isset($_POST["ignore_afterhours"])) {
    $ignore_afterhours=$_POST["ignore_afterhours"];
}
if (isset($_GET["exclude_afterhours"])) {
    $exclude_afterhours=$_GET["exclude_afterhours"];
} elseif (isset($_POST["exclude_afterhours"])) {
    $exclude_afterhours=$_POST["exclude_afterhours"];
}
...
while ($i < $lists_to_print) {
    ...
    for ($j=0; $j<$lists_selected; $j++) {
        if ($list_ids[$j] && $lists[$i]==$list_ids[$j]) {
            $list_name_str.="$lists[$i] - $list_names[$i], ";
        }
        if ($list_ids[$j]=="--ALL--") {
            $lists_selected_str.="'$lists[$i]', ";
        }
    }
    $i++;
}
...
while ($lists_to_print > $o) {
    $selected="";
    for ($i=0; $i<$lists_selected; $i++) {
        if (($file_download < 1) and ($DB)) {
            echo "<!-- $lists[$o] == $list_ids[$i] //-->\n";
        }
        if ($lists[$o] == $list_ids[$i]) {
            $selected="selected";
        }
    }
...
if ($groups_selected==0) {
    ...
    if ($shift && $shift!="ALL") {
        ...
        } elseif ($shift_row["ct_sunday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_monday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_tuesday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_wednesday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_thursday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_friday_start"]!="2400") {
        ...
        } elseif ($shift_row["ct_saturday_start"]!="2400") {
    ...
    while ($d < $TOTintervals) {
        ...
        if (round(MathZDC($totABANDONSsecdate[$d], $totABANDONSdate[$d]))>$max_avgabandontime) {
            $max_avgabandontime=round(MathZDC($totABANDONSsecdate[$d], $totABANDONSdate[$d]));
        }
        ...
        if (round(MathZDC($totANSWERSspeeddate[$d], $totANSWERSdate[$d]))>$max_avganswerspeed) {
            $max_avganswerspeed=round(MathZDC($totANSWERSspeeddate[$d], $totANSWERSdate[$d]));
        }
        ...
        if (($totANSWERSdate[$d]*15)>$max_totalwraptime) {
            $max_totalwraptime=($totANSWERSdate[$d]*15);
        }
        ...
        if (date("w", strtotime($daySTART[$d]))==0 && date("w", strtotime($daySTART[$d-1]))!=0 && $d>0) {  # 2nd date/"w" check is for DST
            ...
            if (round(MathZDC($totABANDONSsecwtd, $totABANDONSwtd))>$max_wtd_avgabandontime) {
                $max_wtd_avgabandontime=round(MathZDC($totABANDONSsecwtd, $totABANDONSwtd));
            }
            ...
            if (round(MathZDC($totANSWERSspeedwtd, $totANSWERSwtd))>$max_wtd_avganswerspeed) {
                $max_wtd_avganswerspeed=round(MathZDC($totANSWERSspeedwtd, $totANSWERSwtd));
            }
            ...
            if (($totANSWERSwtd*15)>$max_wtd_totalwraptime) {
                $max_wtd_totalwraptime=($totANSWERSwtd*15);
            }
            ...
            if (trim($totCALLSwtd)>$max_wtd_offered) {
                $max_wtd_offered=trim($totCALLSwtd);
            }
            if (trim($totANSWERSwtd)>$max_wtd_answered) {
                $max_wtd_answered=trim($totANSWERSwtd);
            }
            if (trim($totAGENTSwtd)>$max_wtd_agents) {
                $max_wtd_agents=trim($totAGENTSwtd);
            }
            if (trim($totABANDONSwtd)>$max_wtd_abandoned) {
                $max_wtd_abandoned=trim($totABANDONSwtd);
            }
            if (trim($totABANDONSpctwtd)>$max_wtd_abandonpct) {
                $max_wtd_abandonpct=trim($totABANDONSpctwtd);
            }
            if (round(MathZDC($totANSWERSsecwtd, $totANSWERSwtd))>$max_wtd_avgtalktime) {
                $max_wtd_avgtalktime=round(MathZDC($totANSWERSsecwtd, $totANSWERSwtd));
            }
            if (trim($totANSWERSsecwtd)>$max_wtd_totaltalktime) {
                $max_wtd_totaltalktime=trim($totANSWERSsecwtd);
            }
            if (trim($totANSWERSsecwtd+($totANSWERSwtd*15))>$max_wtd_totalcalltime) {
                $max_wtd_totalcalltime=trim($totANSWERSsecwtd+($totANSWERSwtd*15));
            }
        ...
        if (date("d", strtotime($daySTART[$d]))==1 && $d>0 && date("d", strtotime($daySTART[$d-1]))!=1) {
            ...
            if (round(MathZDC($totABANDONSsecmtd, $totABANDONSmtd))>$max_mtd_avgabandontime) {
                $max_mtd_avgabandontime=round(MathZDC($totABANDONSsecmtd, $totABANDONSmtd));
            }
            ...
            if (round(MathZDC($totANSWERSspeedmtd, $totANSWERSmtd))>$max_mtd_avganswerspeed) {
                $max_mtd_avganswerspeed=round(MathZDC($totANSWERSspeedmtd, $totANSWERSmtd));
            }
            ...
            if (($totANSWERSmtd*15)>$max_mtd_totalwraptime) {
                $max_mtd_totalwraptime=($totANSWERSmtd*15);
            }
            ...
            if (trim($totCALLSmtd)>$max_mtd_offered) {
                $max_mtd_offered=trim($totCALLSmtd);
            }
            if (trim($totANSWERSmtd)>$max_mtd_answered) {
                $max_mtd_answered=trim($totANSWERSmtd);
            }
            if (trim($totABANDONSmtd)>$max_mtd_abandoned) {
                $max_mtd_abandoned=trim($totABANDONSmtd);
            }
            if (trim($totAGENTSmtd)>$max_mtd_agents) {
                $max_mtd_agents=trim($totAGENTSmtd);
            }
            if (trim($totABANDONSpctmtd)>$max_mtd_abandonpct) {
                $max_mtd_abandonpct=trim($totABANDONSpctmtd);
            }
            ...
            if (trim($totANSWERSsecmtd)>$max_mtd_totaltalktime) {
                $max_mtd_totaltalktime=trim($totANSWERSsecmtd);
            }
            if (trim($totANSWERSsecmtd+($totANSWERSmtd*15))>$max_mtd_totalcalltime) {
                $max_mtd_totalcalltime=trim($totANSWERSsecmtd+($totANSWERSmtd*15));
            }
            ...
            if (date("m", strtotime($daySTART[$d]))==1 || date("m", strtotime($daySTART[$d]))==4 || date("m", strtotime($daySTART[$d]))==7 || date("m", strtotime($daySTART[$d]))==10) { # Quarterly line
                ...
                if (round(MathZDC($totABANDONSsecqtd, $totABANDONSqtd))>$max_qtd_avgabandontime) {
                    $max_qtd_avgabandontime=round(MathZDC($totABANDONSsecqtd, $totABANDONSqtd));
                }
                ...
                if (round(MathZDC($totANSWERSspeedqtd, $totANSWERSqtd))>$max_qtd_avganswerspeed) {
                    $max_qtd_avganswerspeed=round(MathZDC($totANSWERSspeedqtd, $totANSWERSqtd));
                }
                ...
                if (($totANSWERSqtd*15)>$max_qtd_totalwraptime) {
                    $max_qtd_totalwraptime=($totANSWERSqtd*15);
                }
                ...
                if (trim($totCALLSqtd)>$max_qtd_offered) {
                    $max_qtd_offered=trim($totCALLSqtd);
                }
                if (trim($totANSWERSqtd)>$max_qtd_answered) {
                    $max_qtd_answered=trim($totANSWERSqtd);
                }
                if (trim($totABANDONSqtd)>$max_qtd_abandoned) {
                    $max_qtd_abandoned=trim($totABANDONSqtd);
                }
                if (trim($totAGENTSqtd)>$max_qtd_answers) {
                    $max_qtd_answered=trim($totAGENTSqtd);
                }
                if (trim($totABANDONSpctqtd)>$max_qtd_abandonpct) {
                    $max_qtd_abandonpct=trim($totABANDONSpctqtd);
                }
                ...
                if (trim($totANSWERSsecqtd)>$max_qtd_totaltalktime) {
                    $max_qtd_totaltalktime=trim($totANSWERSsecqtd);
                }
                if (trim($totANSWERSsecqtd+($totANSWERSqtd*15))>$max_qtd_totalcalltime) {
                    $max_qtd_totalcalltime=trim($totANSWERSsecqtd+($totANSWERSqtd*15));
                }
                ...
                if(in_array($month, $qtr1)) {
                    $qtr="1st";
                } elseif(in_array($month, $qtr2)) {
                    $qtr="2nd";
                } elseif(in_array($month, $qtr3)) {
                    $qtr="3rd";
                } elseif(in_array($month, $qtr4)) {
                    $qtr="4th";
                }
        ...
        if (!isset($totAGENTSdate[$d])) {
            $totAGENTSdate[$d]=array();
        }
        ...
        if (trim($totCALLSdate[$d])>$max_offered) {
            $max_offered=trim($totCALLSdate[$d]);
        }
        if (trim($totANSWERSdate[$d])>$max_answered) {
            $max_answered=trim($totANSWERSdate[$d]);
        }
        if (trim($totAGENTSday)>$max_agents) {
            $max_agents=trim($totAGENTSday);
        }
        if (trim($totABANDONSdate[$d])>$max_abandoned) {
            $max_abandoned=trim($totABANDONSdate[$d]);
        }
        if (trim($totABANDONSpctDATE[$d])>$max_abandonpct) {
            $max_abandonpct=trim($totABANDONSpctDATE[$d]);
        }
        ...
        if (trim($totANSWERSsecdate[$d])>$max_totaltalktime) {
            $max_totaltalktime=trim($totANSWERSsecdate[$d]);
        }
        if (trim($totANSWERSsecdate[$d]+($totANSWERSdate[$d]*15))>$max_totalcalltime) {
            $max_totalcalltime=trim($totANSWERSsecdate[$d]+($totANSWERSdate[$d]*15));
        }
    ...
    if (date("w", strtotime($daySTART[$d]))>0) {
        ...
        if (trim($totANSWERSavgTIMEwtd)>$max_wtd_avgtalktime) {
            $max_wtd_avgtalktime=trim($totANSWERSavgTIMEwtd);
        }
        if (trim($totANSWERSsecwtd)>$max_wtd_totaltalktime) {
            $max_wtd_totaltalktime=trim($totANSWERSsecwtd);
        }
    ...
    if (date("d", strtotime($daySTART[$d]))!=1) {
        ...
        if (trim($totAGENTSqtd)>$max_qtd_agents) {
            $max_qtd_agents=trim($totAGENTSqtd);
        }
        if (trim($totABANDONSqtd)>$max_qtd_abandoned) {
            $max_qtd_abandoned=trim($totABANDONSqtd);
        }
    ...
    while ($i < $TOTintervals) {
        ...
        if ($qrtQUEUEavg[$i] > $hi_hold_count) {
            $hi_hold_count = $qrtQUEUEavg[$i];
        }
        $qrtQUEUEavg[$i] = round($qrtQUEUEavg[$i], 0);
        if (strlen($qrtQUEUEavg[$i])<1) {
            $qrtQUEUEavg[$i]=0;
        }
        $qrtQUEUEmax[$i] = round($qrtQUEUEmax[$i], 0);
        if (strlen($qrtQUEUEmax[$i])<1) {
            $qrtQUEUEmax[$i]=0;
        }
        $i++;
    }
