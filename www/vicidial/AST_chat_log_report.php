...
if (isset($_GET["inbound_group"])) {
    $inbound_group=$_GET["inbound_group"];
} elseif (isset($_POST["inbound_group"])) {
...
if (isset($_GET["chat_log_type"])) {
    $chat_log_type=$_GET["chat_log_type"];
} elseif (isset($_POST["chat_log_type"])) {
    $chat_log_type=$_POST["chat_log_type"];
}
if (isset($_GET["download_chat_id"])) {
    $download_chat_id=$_GET["download_chat_id"];
} elseif (isset($_POST["download_chat_id"])) {
    $download_chat_id=$_POST["download_chat_id"];
}
if (isset($_GET["download_chat_subid"])) {
    $download_chat_subid=$_GET["download_chat_subid"];
} elseif (isset($_POST["download_chat_subid"])) {
    $download_chat_subid=$_POST["download_chat_subid"];
}
if (isset($_GET["download_user"])) {
    $download_user=$_GET["download_user"];
} elseif (isset($_POST["download_user"])) {
    $download_user=$_POST["download_user"];
}
if (isset($_GET["download_manager"])) {
    $download_manager=$_GET["download_manager"];
} elseif (isset($_POST["download_manager"])) {
    $download_manager=$_POST["download_manager"];
}
...
while ($inbound_groups_to_print > $o) {
    if  (preg_match("/$inbound_groups[$o]\|/i", $inbound_group_string)) {
        $HTML_text.="<option selected value=\"$inbound_groups[$o]\">$inbound_groups[$o]</option>\n";
    } else {
        $HTML_text.="<option value=\"$inbound_groups[$o]\">$inbound_groups[$o]</option>\n";
    }
    $o++;
}
$HTML_text.="</SELECT>\n";
...
if (!$group) {
        ...
        if (mysqli_num_rows($rslt)>0) {
            ...
            while ($row=mysqli_fetch_array($rslt)) {
                ...
                while ($sub_row=mysqli_fetch_array($sub_rslt)) {
                    ...
                    if (strlen($sub_row["message"])>50) {
                        $message=substr($sub_row["message"], 0, 50)."...";
                    } else {
                        $message=$sub_row["message"];
                    }
    ...
    if ($file_download == 1) {
        ...
        if ($chat_log_type=="INTERNAL") {
            ...
            if ($download_chat_subid) {
                $user_stmt.=" and manager_chat_subid='$download_chat_subid' ";
            }
            ...
            if ($download_chat_subid) {
                $csv_stmt.=" and manager_chat_subid='$download_chat_subid' ";
            }
            ...
            while($csv_row=mysqli_fetch_array($csv_rslt)) {
                if ($csv_row["message_posted_by"]==$download_manager) {
                    $poster="$mgr_name";
                } else {
                    $poster=$user_name["$csv_row[message_posted_by]"];
                }
