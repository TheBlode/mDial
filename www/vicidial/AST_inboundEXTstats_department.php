...
if (isset($_GET["end_query_date"])) {
    $end_query_date=$_GET["end_query_date"];
} elseif (isset($_POST["end_query_date"])) {
    $end_query_date=$_POST["end_query_date"];
}
...
if (!isset($end_query_date)) {
    $end_query_date = $NOW_DATE;
}
if (!isset($server_ip)) {
    $server_ip = '10.10.11.20';
}
...
while ($dept_to_print > $o) {
    if ($dept[$o] == $group) {
        echo "<option selected value=\"$dept[$o]\">$dept[$o]</option>\n";
    } else {
        echo "<option value=\"$dept[$o]\">$dept[$o]</option>\n";
    }
    $o++;
}
echo "</SELECT>\n";
