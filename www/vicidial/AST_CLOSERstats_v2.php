...
if (isset($_GET["camp_select"])) {
    $camp_select=$_GET["camp_select"];
} elseif (isset($_POST["camp_select"])) {
    $camp_select=$_POST["camp_select"];
}
...
if ($DB) {
    echo "campaigns_string: $campaigns_ct|$campaigns_string|\n";
}
...
if ($camp_select > 0) {
    ...
    while ($campaigns_to_print > $o) {
        if (preg_match("/$campaigns_allowed[$o]\|/i", $campaigns_string)) {
            $MAIN.="<option selected value=\"$campaigns_allowed[$o]\">$campaigns_allowed[$o] - $campaigns_names[$o]</option>\n";
        } else {
            $MAIN.="<option value=\"$campaigns_allowed[$o]\">$campaigns_allowed[$o] - $campaigns_names[$o]</option>\n";
        }
        $o++;
    }
    $MAIN.="</SELECT>\n";
...
if ($groups_to_print < 1) {
    ...
    if (mysqli_num_rows($ct_rslt)>0) {
        ...
        while (strlen($default_time_BEGIN)<4) {
            $default_time_BEGIN="0".$default_time_BEGIN;
        }
        while (strlen($default_time_END)<4) {
            $default_time_END="0".$default_time_END;
        }
        ...
        if ($ct_row["ct_sunday_start"]==0 && $ct_row["ct_sunday_stop"]==0) {
            ...
            while (strlen($sunday_BEGIN)<4) {
                $sunday_BEGIN="0".$sunday_BEGIN;
            }
            while (strlen($sunday_END)<4) {
        ...
        if ($ct_row["ct_monday_start"]==0 && $ct_row["ct_monday_stop"]==0) {
            ...
            while (strlen($monday_BEGIN)<4) {
                $monday_BEGIN="0".$monday_BEGIN;
            }
            while (strlen($monday_END)<4) {
                $monday_END="0".$monday_END;
            }
        ...
        if ($ct_row["ct_tuesday_start"]==0 && $ct_row["ct_tuesday_stop"]==0) {
            ...
            while (strlen($tuesday_BEGIN)<4) {
                $tuesday_BEGIN="0".$tuesday_BEGIN;
            }
            while (strlen($tuesday_END)<4) {
                $tuesday_END="0".$tuesday_END;
            }
        ...
        if ($ct_row["ct_wednesday_start"]==0 && $ct_row["ct_wednesday_stop"]==0) {
            ...
            while (strlen($wednesday_BEGIN)<4) {
                $wednesday_BEGIN="0".$wednesday_BEGIN;
            }
            while (strlen($wednesday_END)<4) {
                $wednesday_END="0".$wednesday_END;
            }
        ...
        if ($ct_row["ct_thursday_start"]==0 && $ct_row["ct_thursday_stop"]==0) {
            ...
            while (strlen($thursday_BEGIN)<4) {
                $thursday_BEGIN="0".$thursday_BEGIN;
            }
            while (strlen($thursday_END)<4) {
                $thursday_END="0".$thursday_END;
            }
        ...
        if ($ct_row["ct_friday_start"]==0 && $ct_row["ct_friday_stop"]==0) {
            ...
            while (strlen($friday_BEGIN)<4) {
                $friday_BEGIN="0".$friday_BEGIN;
            }
            while (strlen($friday_END)<4) {
                $friday_END="0".$friday_END;
            }
        ...
        if ($ct_row["ct_saturday_start"]==0 && $ct_row["ct_saturday_stop"]==0) {
            ...
            while (strlen($saturday_BEGIN)<4) {
                $saturday_BEGIN="0".$saturday_BEGIN;
            }
            while (strlen($saturday_END)<4) {
    ...
    if ($days_with_custom_times==7) {
        ...
        $time_END="23:59:59";
        if ($DB) {
            $MAIN.="** Default call time disregarded, all days have their own call times **<BR>\n";
        }
    }
    ...
    for ($x=0; $x<count($call_time_ranges); $x++) {
        ...
        if (!$min_graph_seconds || $graph_seconds<$min_graph_seconds) {
            $min_graph_seconds=$graph_seconds;
            $min_graph_start_time=$graph_start_time;
        }
        ...
        $graph_seconds = $hours * 3600 + $minutes * 60 + $seconds;
        if (!$max_graph_seconds || $graph_seconds>$max_graph_seconds) {
            $max_graph_seconds=$graph_seconds;
            $max_graph_end_time=$graph_end_time;
        }
    }
    ...
    if (!$min_graph_start_time) {
        $min_graph_start_time="00:00:00";
    }
    if (!$max_graph_start_time) {
        $max_graph_start_time="23:59:59";
    }
    ...
    if ($report_display_type=="HTML") {
        ...
        for ($i=0; $i<count($graph_data_ary); $i++) {
            if ($graph_data_ary[$i][1]>$max_value) {
                $max_value=$graph_data_ary[$i][1];
            }
            if ($graph_data_ary[$i][1]>0) {
                $w++;
            }
        }
    ...
    while ($i < $reasons_to_print) {
        ...
        if (($row[1] > 5) and ($row[1] <= 10)) {
            $hd10 = ($hd10 + $row[0]);
        }
        if (($row[1] > 10) and ($row[1] <= 15)) {
            $hd15 = ($hd15 + $row[0]);
        }
        if (($row[1] > 15) and ($row[1] <= 20)) {
            $hd20 = ($hd20 + $row[0]);
        }
        if (($row[1] > 20) and ($row[1] <= 25)) {
            $hd25 = ($hd25 + $row[0]);
        }
        if (($row[1] > 25) and ($row[1] <= 30)) {
            $hd30 = ($hd30 + $row[0]);
        }
        if (($row[1] > 30) and ($row[1] <= 35)) {
            $hd35 = ($hd35 + $row[0]);
        }
        if (($row[1] > 35) and ($row[1] <= 40)) {
            $hd40 = ($hd40 + $row[0]);
        }
        if (($row[1] > 40) and ($row[1] <= 45)) {
            $hd45 = ($hd45 + $row[0]);
        }
        if (($row[1] > 45) and ($row[1] <= 50)) {
            $hd50 = ($hd50 + $row[0]);
        }
        if (($row[1] > 50) and ($row[1] <= 55)) {
            $hd55 = ($hd55 + $row[0]);
        }
        if (($row[1] > 55) and ($row[1] <= 60)) {
            $hd60 = ($hd60 + $row[0]);
        }
        if (($row[1] > 60) and ($row[1] <= 90)) {
            $hd90 = ($hd90 + $row[0]);
        }
        if ($row[1] > 90) {
            $hd99 = ($hd99 + $row[0]);
    ...
    if ($report_display_type=="HTML") {
        ...
        } elseif ($max_calls<=100) {
            while ($maxcalls%10!=0) {
                $maxcalls++;
            }
        } elseif ($max_calls<=1000) {
            while ($maxcalls%50!=0) {
                $maxcalls++;
            }
        } else {
    ...
    if (($BDansweredCALLS > 0) and ($TOTALcalls > 0)) {
        $pad_0 = (MathZDC($ad_0, $TOTALcalls) * 100);
        $pad_0 = round($pad_0, 0);
        $pad_5 = (MathZDC($ad_5, $TOTALcalls) * 100);
        $pad_5 = round($pad_5, 0);
        $pad10 = (MathZDC($ad10, $TOTALcalls) * 100);
        $pad10 = round($pad10, 0);
        $pad15 = (MathZDC($ad15, $TOTALcalls) * 100);
        $pad15 = round($pad15, 0);
        $pad20 = (MathZDC($ad20, $TOTALcalls) * 100);
        $pad20 = round($pad20, 0);
        $pad25 = (MathZDC($ad25, $TOTALcalls) * 100);
        $pad25 = round($pad25, 0);
        $pad30 = (MathZDC($ad30, $TOTALcalls) * 100);
        $pad30 = round($pad30, 0);
        $pad35 = (MathZDC($ad35, $TOTALcalls) * 100);
        $pad35 = round($pad35, 0);
        $pad40 = (MathZDC($ad40, $TOTALcalls) * 100);
        $pad40 = round($pad40, 0);
        $pad45 = (MathZDC($ad45, $TOTALcalls) * 100);
        $pad45 = round($pad45, 0);
        $pad50 = (MathZDC($ad50, $TOTALcalls) * 100);
        $pad50 = round($pad50, 0);
        $pad55 = (MathZDC($ad55, $TOTALcalls) * 100);
        $pad55 = round($pad55, 0);
        $pad60 = (MathZDC($ad60, $TOTALcalls) * 100);
        $pad60 = round($pad60, 0);
        $pad90 = (MathZDC($ad90, $TOTALcalls) * 100);
        $pad90 = round($pad90, 0);
        $pad99 = (MathZDC($ad99, $TOTALcalls) * 100);
        $pad99 = round($pad99, 0);
        $pCad_0 = (MathZDC($Cad_0, $TOTALcalls) * 100);
        $pCad_0 = round($pCad_0, 0);
        $pCad_5 = (MathZDC($Cad_5, $TOTALcalls) * 100);
        $pCad_5 = round($pCad_5, 0);
        $pCad10 = (MathZDC($Cad10, $TOTALcalls) * 100);
        $pCad10 = round($pCad10, 0);
        $pCad15 = (MathZDC($Cad15, $TOTALcalls) * 100);
        $pCad15 = round($pCad15, 0);
        $pCad20 = (MathZDC($Cad20, $TOTALcalls) * 100);
        $pCad20 = round($pCad20, 0);
        $pCad25 = (MathZDC($Cad25, $TOTALcalls) * 100);
        $pCad25 = round($pCad25, 0);
        $pCad30 = (MathZDC($Cad30, $TOTALcalls) * 100);
        $pCad30 = round($pCad30, 0);
        $pCad35 = (MathZDC($Cad35, $TOTALcalls) * 100);
        $pCad35 = round($pCad35, 0);
        $pCad40 = (MathZDC($Cad40, $TOTALcalls) * 100);
        $pCad40 = round($pCad40, 0);
        $pCad45 = (MathZDC($Cad45, $TOTALcalls) * 100);
        $pCad45 = round($pCad45, 0);
        $pCad50 = (MathZDC($Cad50, $TOTALcalls) * 100);
        $pCad50 = round($pCad50, 0);
        $pCad55 = (MathZDC($Cad55, $TOTALcalls) * 100);
        $pCad55 = round($pCad55, 0);
        $pCad60 = (MathZDC($Cad60, $TOTALcalls) * 100);
        $pCad60 = round($pCad60, 0);
        $pCad90 = (MathZDC($Cad90, $TOTALcalls) * 100);
        $pCad90 = round($pCad90, 0);
        $pCad99 = (MathZDC($Cad99, $TOTALcalls) * 100);
        $pCad99 = round($pCad99, 0);
        $ApCad_0 = (MathZDC($Cad_0, $BDansweredCALLS) * 100);
        $ApCad_0 = round($ApCad_0, 0);
        $ApCad_5 = (MathZDC($Cad_5, $BDansweredCALLS) * 100);
        $ApCad_5 = round($ApCad_5, 0);
        $ApCad10 = (MathZDC($Cad10, $BDansweredCALLS) * 100);
        $ApCad10 = round($ApCad10, 0);
        $ApCad15 = (MathZDC($Cad15, $BDansweredCALLS) * 100);
        $ApCad15 = round($ApCad15, 0);
        $ApCad20 = (MathZDC($Cad20, $BDansweredCALLS) * 100);
        $ApCad20 = round($ApCad20, 0);
        $ApCad25 = (MathZDC($Cad25, $BDansweredCALLS) * 100);
        $ApCad25 = round($ApCad25, 0);
        $ApCad30 = (MathZDC($Cad30, $BDansweredCALLS) * 100);
        $ApCad30 = round($ApCad30, 0);
        $ApCad35 = (MathZDC($Cad35, $BDansweredCALLS) * 100);
        $ApCad35 = round($ApCad35, 0);
        $ApCad40 = (MathZDC($Cad40, $BDansweredCALLS) * 100);
        $ApCad40 = round($ApCad40, 0);
        $ApCad45 = (MathZDC($Cad45, $BDansweredCALLS) * 100);
        $ApCad45 = round($ApCad45, 0);
        $ApCad50 = (MathZDC($Cad50, $BDansweredCALLS) * 100);
        $ApCad50 = round($ApCad50, 0);
        $ApCad55 = (MathZDC($Cad55, $BDansweredCALLS) * 100);
        $ApCad55 = round($ApCad55, 0);
        $ApCad60 = (MathZDC($Cad60, $BDansweredCALLS) * 100);
        $ApCad60 = round($ApCad60, 0);
        $ApCad90 = (MathZDC($Cad90, $BDansweredCALLS) * 100);
        $ApCad90 = round($ApCad90, 0);
        $ApCad99 = (MathZDC($Cad99, $BDansweredCALLS) * 100);
        $ApCad99 = round($ApCad99, 0);
    }
    ...
    while ($i < $statuses_to_print) {
        ...
        if ($row[2]>$max_total_time) {
            $max_total_time=$row[2];
        }
        ...
        $STATUSrate =    sprintf("%9s", $STATUSrate);
        while(strlen($STATUSrate)>9) {
    ...
    while ($i < $positions_to_print) {
        ...
        if (($row[1] > 4) and ($row[1] <= 5)) {
            $qp_5 = ($qp_5 + $row[0]);
        }
        if (($row[1] > 5) and ($row[1] <= 6)) {
            $qp_6 = ($qp_6 + $row[0]);
        }
        if (($row[1] > 6) and ($row[1] <= 7)) {
            $qp_7 = ($qp_7 + $row[0]);
        }
        if (($row[1] > 7) and ($row[1] <= 8)) {
            $qp_8 = ($qp_8 + $row[0]);
        }
        if (($row[1] > 8) and ($row[1] <= 9)) {
            $qp_9 = ($qp_9 + $row[0]);
        }
        if (($row[1] > 9) and ($row[1] <= 10)) {
            $qp10 = ($qp10 + $row[0]);
        }
        if (($row[1] > 10) and ($row[1] <= 15)) {
            $qp15 = ($qp15 + $row[0]);
    ...
    while ($i <= 96) {
        ...
        if ($h == 1) {
            $time = "   15 ";
            $SQLtime = "$hour:15:00";
            $SQLtimeEND = "$hour:30:00";
        }
        if ($h == 2) {
            $time = "   30 ";
            $SQLtime = "$hour:30:00";
            $SQLtimeEND = "$hour:45:00";
        }
        if ($h == 3) {
            $time = "   45 ";
            $SQLtime = "$hour:45:00";
