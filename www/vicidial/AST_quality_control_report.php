...
if (isset($_GET["qc_finish_start_date"])) {
    $qc_finish_start_date=$_GET["qc_finish_start_date"];
} elseif (isset($_POST["qc_finish_start_date"])) {
    $qc_finish_start_date=$_POST["qc_finish_start_date"];
}
if (isset($_GET["qc_finish_end_date"])) {
    $qc_finish_end_date=$_GET["qc_finish_end_date"];
} elseif (isset($_POST["qc_finish_end_date"])) {
    $qc_finish_end_date=$_POST["qc_finish_end_date"];
}
if (isset($_GET["qc_finish_start_time"])) {
    $qc_finish_start_time=$_GET["qc_finish_start_time"];
} elseif (isset($_POST["qc_finish_start_time"])) {
    $qc_finish_start_time=$_POST["qc_finish_start_time"];
}
if (isset($_GET["qc_finish_end_time"])) {
    $qc_finish_end_time=$_GET["qc_finish_end_time"];
} elseif (isset($_POST["qc_finish_end_time"])) {
    $qc_finish_end_time=$_POST["qc_finish_end_time"];
}
if (isset($_GET["group"])) {
    $group=$_GET["group"];
...
if (isset($_GET["QCusers"])) {
    $QCusers=$_GET["QCusers"];
} elseif (isset($_POST["QCusers"])) {
    $QCusers=$_POST["QCusers"];
}
if (isset($_GET["QCuser_groups"])) {
    $QCuser_groups=$_GET["QCuser_groups"];
} elseif (isset($_POST["QCuser_groups"])) {
    $QCuser_groups=$_POST["QCuser_groups"];
}
if (isset($_GET["QCstatuses"])) {
    $QCstatuses=$_GET["QCstatuses"];
} elseif (isset($_POST["QCstatuses"])) {
    $QCstatuses=$_POST["QCstatuses"];
}
if (isset($_GET["DB"])) {
    $DB=$_GET["DB"];
...
if (!isset($QCusers) || !is_array($QCusers)) {
    $QCusers = array();
}
if (!isset($QCstatuses) || !is_array($QCstatuses)) {
    $QCstatuses = array();
}
if (!isset($QCuser_groups) || !is_array($QCuser_groups)) {
    $QCuser_groups = array();
}
if (!isset($query_date)) {
    $query_date = $NOW_DATE;
...
while ($campaigns_to_print > $o) {
    if (preg_match("/$groups[$o]\|/i", $group_string) && !preg_match('/\-\-ALL\-\-/', $group_string)) {
...
while ($users_to_print > $o) {
    if  (preg_match("/$user_list[$o]\|/i", $users_string) && !preg_match('/\-\-ALL\-\-/', $users_string)) {
...
while ($QC_statuses_to_print > $o) {
    if  (preg_match("/$all_QC_statuses[$o]\|/i", $QCstatus_string) && !preg_match('/\-\-ALL\-\-/', $QCstatus_string)) {
        $HTML_text.="<option selected value=\"$all_QC_statuses[$o]\">$all_QC_statuses[$o]</option>\n";
    } else {
        $HTML_text.="<option value=\"$all_QC_statuses[$o]\">$all_QC_statuses[$o]</option>\n";
    }
    $o++;
}
$HTML_text.="</SELECT>\n";
...
while ($QCusers_to_print > $o) {
    if  (preg_match("/$QCuser_list[$o]\|/i", $QCusers_string) && !preg_match('/\-\-ALL\-\-/', $QCusers_string)) {
        $HTML_text.="<option selected value=\"$QCuser_list[$o]\">$QCuser_list[$o] - $QCuser_names[$o]</option>\n";
    } else {
        $HTML_text.="<option value=\"$QCuser_list[$o]\">$QCuser_list[$o] - $QCuser_names[$o]</option>\n";
    }
    $o++;
}
$HTML_text.="</SELECT>\n";
...
if ($SUBMIT && ($QCuser_ct>0 || $user_ct>0 || $group_ct>0 || $QCstatus_ct>0 || ($qc_finish_start_date && $qc_finish_end_date))) {
    if ($qc_finish_start_date && $qc_finish_end_date) {
        if (!$qc_finish_start_time) {
            $qc_finish_start_time = "00:00:00";
        }
        if (!$qc_finish_end_time) {
            $qc_finish_end_time = "23:59:59";
        }
...
function GetScore($QCID, $checkpointID)
{
    ...
    $checkpoint_clause="";
    if ($checkpointID) {
        $checkpoint_clause=" and qc_checkpoint_log_id='$checkpointID'";
    }
