...
if (isset($_GET["show_summary"])) {
    $show_summary=$_GET["show_summary"];
} elseif (isset($_POST["show_summary"])) {
    $show_summary=$_POST["show_summary"];
}
if (isset($_GET["show_unanswered_xfers"])) {
    $show_unanswered_xfers=$_GET["show_unanswered_xfers"];
} elseif (isset($_POST["show_unanswered_xfers"])) {
    $show_unanswered_xfers=$_POST["show_unanswered_xfers"];
}
...
while ($campaigns_to_print > $o) {
    if (preg_match("/\|$campaigns[$o]\|/i", $campaign_string) && !preg_match('/\-\-ALL\-\-/', $campaign_string)) {
        $HTML_text.="<option selected value=\"$campaigns[$o]\">$campaigns[$o]</option>\n";
    } else {
        $HTML_text.="<option value=\"$campaigns[$o]\">$campaigns[$o]</option>\n";
    }
    $o++;
}
$HTML_text.="</SELECT>\n";
...
while ($user_groups_to_print > $o) {
    if  (preg_match("/\|$user_groups[$o]\|/i", $user_group_string) && !preg_match('/\-\-ALL\-\-/', $user_group_string)) {
...
if (!$group) {
    ...
    if ($campaign_ct>0 || $user_group_ct>0 || $user_ct>0) {
        ...
        */
        ...
        while ($xfer_row=mysqli_fetch_array($xfer_log_rslt)) {
            ...
            if ($closer=="VDXL" && $show_unanswered_xfers) {
                $closer="VDCL";
            }
            $campaign_id="";
            ...
            if ($campaign_id) {
                ...
                if ($DB) {
                    echo "<B>$closer_xfer_log_stmt</B>|".mysqli_num_rows($closer_xfer_log_rslt)."\n";
                }
                if (mysqli_num_rows($closer_xfer_log_rslt)>0) {
                    ...
                    if ($DB) {
                        echo "<B>$closer_agent_log_stmt</B>|".mysqli_num_rows($closer_agent_log_rslt)."\n";
                    }
                    ...
                    } elseif ($closer=="VDCL" && $show_unanswered_xfers) {
    ...
    if ($show_summary) {
        ...
        $sale_dispo_str.="|";
        if ($DB) {
            $HTML_text.=_QXZ("Sale dispo string").": $sale_dispo_str\n";
        }
        ...
        */
        ...
        foreach ($complete_fronter_stats as $fronter => $dispos) {
            ...
            foreach ($dispos as $dispo => $count) {
                $recL=0;
                if ((preg_match("/\|$dispo\|/", $sale_dispo_str)) and ($recL < 1)) {
                    $A1=$count;
                    $recL++;
                    $sales=($sales + $count);
                }
                if (($dispo=='DROP' || $dispo=='TIMEOT') and ($recL < 1)) {
                    $DROP=($DROP+$count);
                    $recL++;
                }
                if ($recL < 1) {
        ...
        while ($i < $users_to_print) {
            ...
            if ($names_to_print > 0) {
                $row=mysqli_fetch_row($rslt);
                if ($non_latin < 1) {
                    $full_name[$i] =    sprintf("%-15s", $row[0]);
                    while(strlen($full_name[$i])>15) {
                        $full_name[$i] = substr("$full_name[$i]", 0, -1);
                    }
                } else {
                    $full_name[$i] =    sprintf("%-45s", $row[0]);
                    while(mb_strlen($full_name[$i], 'utf-8')>15) {
                        $full_name[$i] = mb_substr("$full_name[$i]", 0, -1, 'utf-8');
                    }
                }
            } else {
                $full_name[$i] = '               ';
            ...
            $ppc =    sprintf("%4s", $ppc);
            if ($USERcalls[$i]>$max_calls) {
                $max_calls=$USERcalls[$i];
            }
            if ($A1>$max_sales) {
                $max_sales=$A1;
            }
