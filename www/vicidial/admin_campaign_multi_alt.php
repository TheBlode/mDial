...
if (isset($_GET["lead_order_randomize"])) {
    $lead_order_randomize=$_GET["lead_order_randomize"];
} elseif (isset($_POST["lead_order_randomize"])) {
    $lead_order_randomize=$_POST["lead_order_randomize"];
}
if (isset($_GET["lead_order_secondary"])) {
    $lead_order_secondary=$_GET["lead_order_secondary"];
} elseif (isset($_POST["lead_order_secondary"])) {
...
if ($action == "ALT_MULTI_SUBMIT") {
    ...
    if ((strlen($lead_order_randomize) > 0) and (strlen($lead_order_secondary) > 0)) {
        ...
        while ($do_values_ct > $o) {
            ...
            $owner_raw =        $owners[$o];
            $owner = preg_replace('/ |\n|\r|\t/', '', $owner_raw);
            if (isset($_GET["$campaign_id$US$owner"])) {
                $owner_check=$_GET["$campaign_id$US$owner"];
            } elseif (isset($_POST["$campaign_id$US$owner"])) {
                $owner_check=$_POST["$campaign_id$US$owner"];
            }
            if (isset($_GET["rank_$campaign_id$US$owner"])) {
                $owner_rank=$_GET["rank_$campaign_id$US$owner"];
            } elseif (isset($_POST["rank_$campaign_id$US$owner"])) {
                $owner_rank=$_POST["rank_$campaign_id$US$owner"];
            }
            ...
            if ($DB > 0) {
                echo "OWNER: $o|$campaign_id|$owner|$owner_check|$owner_rank|$affected_rows\n<BR>";
            }
        ...
        $affected_rows = mysqli_affected_rows($link);
        if ($DB > 0) {
            echo "$affected_rows|$filter_stmt\n<BR>";
        }
        ...
        $affected_rows = mysqli_affected_rows($link);
        if ($DB > 0) {
            echo "$campaign_id|$lead_order_randomize|$lead_order_secondary|ML$campaign_id|DOWN RANK|$affected_rows|$stmt\n<BR>";
        }
        if ($affected_rows > 0) {
...
if ($action == "BLANK") {
    ...
    if ($vlf_values_ct > 0) {
        ...
        if ($DB) {
            echo "$lead_filter_sql|$lead_filter_comments";
        }
    }
    echo "<TABLE><TR><TD>\n";
