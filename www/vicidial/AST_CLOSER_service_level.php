...
if (!$group) {
    ...
    while ($d < $DURATIONday) {
        ...
        if (trim($totDROPSdate[$d])>$max_drops) {
            $max_drops=trim($totDROPSdate[$d]);
        }
        if (trim($totDROPSpctDATE[$d])>$max_droppct) {
            $max_droppct=trim($totDROPSpctDATE[$d]);
        }
        if (trim($totDROPSavgDATE[$d])>$max_avgdrops) {
            $max_avgdrops=trim($totDROPSavgDATE[$d]);
        }
        if (trim($totQUEUEdate[$d])>$max_hold) {
            $max_hold=trim($totQUEUEdate[$d]);
        }
        if (trim($totQUEUEpctDATE[$d])>$max_holdpct) {
            $max_holdpct=trim($totQUEUEpctDATE[$d]);
        }
        if (trim($totQUEUEavgDATE[$d])>$max_avgholds) {
            $max_avgholds=trim($totQUEUEavgDATE[$d]);
        }
        if (trim($totQUEUEtotDATE[$d])>$max_avgholdstotal) {
            $max_avgholdstotal=trim($totQUEUEtotDATE[$d]);
        }
        if (trim($totCALLSdate[$d])>$max_calls) {
            $max_calls=trim($totCALLSdate[$d]);
        }
        if (trim($totCALLSsecDATE[$d])>$max_totalcalltime) {
            $max_totalcalltime=trim($totCALLSsecDATE[$d]);
        }
        if (trim($totCALLSavgDATE[$d])>$max_avgcalltime) {
            $max_avgcalltime=trim($totCALLSavgDATE[$d]);
        }
    ...
    while ($i < $TOTintervals) {
        ...
        if ($Gavg_hold < 1) {
            if ($i < 0) {
                ...
                if (trim($qrtQUEUEavg[$i])>$max_avg_hold_time) {
                    $max_avg_hold_time=trim($qrtQUEUEavg[$i]);
                }
            }
        } else {
            $TOT_lines++;
        ...
        if (trim($qrtCALLS[$i])>$max_calls) {
            $max_calls=trim($qrtCALLS[$i]);
        }
        $i++;
    }
    $totQUEUEavgRAW = MathZDC($totCALLS, $totQUEUEsec);
    $totQUEUEavg =    sprintf("%5s", $totQUEUEavg);
    while (strlen($totQUEUEavg)>5) {
    ...
    while ($h < $TOTintervals) {
        ...
        while (strlen($qrtQUEUEavg[$h])>4) {
            $qrtQUEUEavg[$h] = preg_replace('/.$/', '', $qrtQUEUEavg[$h]);
        }
        ...
        if (($Aavg_hold[$h] < 1) or ($Aavg_hold[$h] > 119)) {
            ...
            if ($Aavg_hold[$h] > 119) {
                $qrtQUEUEavg_scale[$h] = '<SPAN class="orange">xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</SPAN>';
            }
        } else {
            ...
            while ($k < 32) {
                if ($k <= $qrtQUEUEavg_val[$h]) {
                    if ($k < 1) {
                        $qrtQUEUEavg_scale[$h] .= '<SPAN class="orange">';
                    }
                    $qrtQUEUEavg_scale[$h] .= 'x';
                } else {
                    if (($k > 0) and ($blank < 1)) {
                        $qrtQUEUEavg_scale[$h] .= '</SPAN>';
                    }
                    $qrtQUEUEavg_scale[$h] .= ' ';
                    $blank++;
                }
