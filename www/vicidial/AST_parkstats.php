...
if (isset($_GET["sort_by_details"])) {
    $sort_by_details=$_GET["sort_by_details"];
} elseif (isset($_POST["sort_by_details"])) {
    $sort_by_details=$_POST["sort_by_details"];
}
...
while ($groups_to_print > $o) {
    if ($groups[$o] == $group) {
        $MAIN.="<option selected value=\"$groups[$o]\">$groups[$o]</option>\n";
    } else {
        $MAIN.="<option value=\"$groups[$o]\">$groups[$o]</option>\n";
    }
    $o++;
}
...
if (!$group) {
    ...
    while ($i < $users_to_print) {
        $row=mysqli_fetch_row($rslt);
        $user[$i] =            $row[0];
        $full_name[$i] =    sprintf("%-43s", "$row[0] - $row[1]");
        while(strlen($full_name[$i])>43) {
