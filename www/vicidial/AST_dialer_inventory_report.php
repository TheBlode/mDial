...
} elseif (isset($_POST["selected_list"])) {
...
if (isset($_GET["time_setting"])) {
    $time_setting=$_GET["time_setting"];
} elseif (isset($_POST["time_setting"])) {
...
if (isset($_GET["report_source"])) {
    $report_source=$_GET["report_source"];
} elseif (isset($_POST["report_source"])) {
...
if (isset($_GET["DBX"])) {
    $DBX=$_GET["DBX"];
} elseif (isset($_POST["DBX"])) {
    $DBX=$_POST["DBX"];
}
if (isset($_GET["snapshot_time"])) {
    $snapshot_time=$_GET["snapshot_time"];
} elseif (isset($_POST["snapshot_time"])) {
    $snapshot_time=$_POST["snapshot_time"];
}
if (isset($_GET["override_24hours"])) {
    $override_24hours=$_GET["override_24hours"];
} elseif (isset($_POST["override_24hours"])) {
...
while ($campaigns_to_print > $o) {
    if (preg_match("/$groups[$o]\|/i", $group_string)) {
        $campaign_span_txt.="<option selected value=\"$groups[$o]\">$groups[$o]</option>";
    } else {
        $campaign_span_txt.="<option value=\"$groups[$o]\">$groups[$o]</option>";
    }
    $o++;
}
...
if ($DBX > 0) {
    $HTML_header.= "|$snapshot_stmt|\n";
}
$snapshot_rslt=mysql_to_mysqli($snapshot_stmt, $link);
$snapshot_span_txt.="<SELECT NAME='snapshot_time'>\n";
if ($snapshot_time) {
    $snapshot_span_txt.="\t<option value=\"$snapshot_time\" selected>$snapshot_time</option>\n";
}
...
$HTML_header.=" </STYLE>\n";
if ($report_type=='LIST') {
    $onload="onload=\"ToggleSpan('list_span', 'campaign_span')\"";
}
...
if ($SUBMIT) {
    ...
    if ($snapshot_time && $report_source=="SNAPSHOT") {
        ...
        while($shift_row=mysqli_fetch_array($shift_rslt)) {
            ...
            $$total_varname=0;
            if ($DB > 0) {
                echo "<BR>-$c-$shift_ary[$c]-$shift_row[shift_id]-\n";
            }
            $c++;
        }
        if ($report_type=="CAMPAIGNS") {
            $time_clause=$group_SQL;
        }
        if ($report_type=="LIST") {
            $time_clause="and list_id='$selected_list'";
        }
        ...
        while ($row=mysqli_fetch_array($rslt)) {
            ...
            if (strlen($row["list_description"])>0) {
                $list_info=$row["list_description"];
            } else {
                $list_info=$row["list_name"];
            }
            ...
            while ($b < count($shift_ary)) {
                ...
                while ($a < count($shift_data_a)) {
                    ...
                    if ($DB > 0) {
                        echo "<BR>-$a-$b-$shift_ary[$b]-$shift_data_b[0]($shift_data_b[1])-$line_match-\n";
                    }
                    $a++;
                }
                if ($no_match==0) {
        ...
        if ($time_setting=="LOCAL") {
            ...
            if ($report_type=="CAMPAIGNS") {
                $time_clause="where list_id in (SELECT list_id from vicidial_lists where inventory_report='Y' ".substr($group_SQL, 4).")";
            }
            if ($report_type=="LIST") {
                $time_clause="where list_id='$selected_list'";
            }
            $gmt_stmt="SELECT distinct gmt_offset_now, gmt_offset_now-($local_offset) from vicidial_list $time_clause ;";
            if ($DB) {
                $HTML_header.=$gmt_stmt."<BR>\n";
            }
            $gmt_rslt=mysql_to_mysqli($gmt_stmt, $link);
        ...
        if ($group_ct>0 && $report_type=="CAMPAIGNS") {
            for ($i=0; $i<$group_ct; $i++) {
                ...
                while ($row=mysqli_fetch_row($rslt)) {
                    ...
                    if (!preg_match("/ $row[0] /", "$active_dial_statuses")) {
                        $inactive_dial_statuses.="$row[0] ";
                    }
                }
                $inventory_statuses=substr($inventory_statuses, 0, -1);
                if ($DB) {
                    $HTML_header.= " "._QXZ("CAMPAIGN DIAL STATUSES", 30, "r").": |$active_dial_statuses|\n";
                }
                if ($DB) {
                    $HTML_header.= " "._QXZ("INVENTORY STATUSES RESULTS", 30, "r").": |$inventory_statuses|\n";
                }
                if ($DB) {
                    $HTML_header.= " "._QXZ("INACTIVE STATUSES RESULTS", 30, "r").": |$inactive_dial_statuses|\n";
                }
                ...
                while ($lists_row=mysqli_fetch_array($lists_rslt)) {
                    ...
                    foreach ($shift_ary2 as $key => $val) {
                        ...
                        while ($gmt_row=mysqli_fetch_row($gmt_rslt)) {
                            ...
                            if ($DB) {
                                $HTML_header.="<B>$shift_stmt<BR>$shift_row[0]</B>\n";
                            }
        ...
        } elseif ($selected_list && $report_type=="LIST") {
            ...
            if ($DB) {
                $HTML_header.= " "._QXZ("DIAL STATUSES", 30, "r").": |$dial_statuses|\n";
            }
            if ($DB) {
                $HTML_header.= " "._QXZ("INVENTORY STATUSES RESULTS", 30, "r").": |$inventory_statuses|\n";
            }
            ...
            if ($DB) {
                $HTML_header.="<B>$campaign_stmt; - $dial_statuses</B>\n";
            }
