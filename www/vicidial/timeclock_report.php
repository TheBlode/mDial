...
if ((!isset($query_date)) or (strlen($query_date) < 10)) {
    $query_date = $TODAY;
}
...
if ($order == 'group_up') {
    $order_SQL = "order by vicidial_timeclock_log.user_group";
}
if ($order == 'group_down') {
    $order_SQL = "order by vicidial_timeclock_log.user_group desc";
}
if (strlen($user) > 0) {
    $user_SQL = "and vicidial_timeclock_log.user='$user'";
} else {
    $user_SQL='';
}
...
if (!$report_display_type || $report_display_type=="TEXT") {
    ...
    while ($i < $rows_to_print) {
        ...
        $full_name[$i] =    $row[1];
        $login_sec[$i] =    $row[2];
        $TOTlogin_sec = ($TOTlogin_sec + $row[2]);
        $u_group[$i] =        $row[3];
        if ($login_sec[$i] > 0) {
