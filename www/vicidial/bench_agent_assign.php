...
if (isset($_GET["absent_agent"])) {
    $absent_agent=$_GET["absent_agent"];
} elseif (isset($_POST["absent_agent"])) {
    $absent_agent=$_POST["absent_agent"];
}
if (isset($_GET["bench_agent"])) {
    $bench_agent=$_GET["bench_agent"];
} elseif (isset($_POST["bench_agent"])) {
    $bench_agent=$_POST["bench_agent"];
}
...
if ((strlen($absent_agent) < 1) or (strlen($bench_agent) < 1)) {
    ...
    $owners_to_print=0;
    if (strlen($list_id) > 1) {
        $list_idSQL = "and list_id='$list_id'";
    }
    ...
    if ($rslt) {
        $benches_to_print = mysqli_num_rows($rslt);
    }
    $j=0;
    ...
    while ($j < $leads_to_print) {
        ...
        if ($DB) {
            echo "$tempUPDATEaffected_rows|$stmt|\n";
        }
