...
if ($group_ct < 1) {
    ...
    */
    ...
    $order_SQL='';
    if ($order == 'sph_up') {
        $order_SQL = "order by full_name,campaign_group_id,role";
    }
    if ($order == 'sph_down') {
        $order_SQL = "order by full_name,campaign_group_id,role";
    }
    if ($order == 'hours_up') {
        $order_SQL = "order by login";
    }
    if ($order == 'hours_down') {
        $order_SQL = "order by login desc";
    }
    if ($order == 'sales_up') {
        $order_SQL = "order by sales, sph desc";
    }
    if ($order == 'sales_down') {
        $order_SQL = "order by sales desc, sph";
    }
    if ($order == 'calls_up') {
        $order_SQL = "order by calls";
    }
    if ($order == 'calls_down') {
        $order_SQL = "order by calls desc";
    }
    if ($order == 'user_up') {
        $order_SQL = "order by vicidial_users.user";
    }
    if ($order == 'user_down') {
        $order_SQL = "order by vicidial_users.user desc";
    }
    if ($order == 'name_up') {
        $order_SQL = "order by full_name";
    }
    if ($order == 'name_down') {
        $order_SQL = "order by full_name desc";
    }
    if (strlen($user) > 0) {
        $user_SQL = "and vicidial_agent_sph.user='$user'";
    } else {
        $user_SQL='';
    }
    ...
    while ($i < $rows_to_print) {
        ...
        $group[$i] =        $row[3];
        $login_sec[$i] =    $row[4];
        $TOTlogin_sec = ($TOTlogin_sec + $row[4]);
        $calls[$i] =        $row[5];
        $TOTcalls = ($TOTcalls + $row[5]);
        $sales[$i] =        $row[6];
        $TOTsales = ($TOTsales + $row[6]);
        if ($login_sec[$i] > 0) {
