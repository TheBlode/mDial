...
if (isset($_GET["reset_lead_called_campaigns"])) {
    $reset_lead_called_campaigns=$_GET["reset_lead_called_campaigns"];
} elseif (isset($_POST["reset_lead_called_campaigns"])) {
    $reset_lead_called_campaigns=$_POST["reset_lead_called_campaigns"];
}
if (isset($_GET["all_or_active_only"])) {
    $all_or_active_only=$_GET["all_or_active_only"];
} elseif (isset($_POST["all_or_active_only"])) {
    $all_or_active_only=$_POST["all_or_active_only"];
}
if (isset($_GET["submit_campaign_reset"])) {
    $submit_campaign_reset=$_GET["submit_campaign_reset"];
} elseif (isset($_POST["submit_campaign_reset"])) {
    $submit_campaign_reset=$_POST["submit_campaign_reset"];
}
...
if (($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ((preg_match("/Administration Change Log/", $LOGallowed_reports)) or (preg_match("/ALL REPORTS/", $LOGallowed_reports)))) {
    ...
    if ($submit_campaign_reset && $reset_lead_called_campaigns) {
        if ($all_or_active_only=="Y") {
            $list_id_clause="and active='Y'";
            $verbiage="(active lists only)";
        }
        ...
        while ($list_id_row=mysqli_fetch_array($list_id_rslt)) {
            ...
            if (($daily_reset_limit > $resets_today) or ($daily_reset_limit < 0)) {
                ...
                if ($affected_rowsB > 0) {
                    ...
                    echo "<B>"._QXZ("NOT")."</B> "._QXZ("RESET")."<BR>";
                }
            } else {
        ...
        if (mysqli_num_rows($list_id_rslt)<7) {
            for ($j=mysqli_num_rows($list_id_rslt); $j<7; $j++) {
                echo "<BR>";
            }
        }
    } else {
