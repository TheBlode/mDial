...
if (isset($_GET["reset_agent"])) {
    $reset_agent=$_GET["reset_agent"];
} elseif (isset($_POST["reset_agent"])) {
    $reset_agent=$_POST["reset_agent"];
}
    ...
    if ($sc_ct > 0) {
        ...
        while ($p <= $container_ct) {
            ...
            if (preg_match("/^RESET_USER_GROUPS/", $line)) {
                $reset_user_groups = $line;
                $reset_user_groups = trim(preg_replace("/.*=>/", '', $reset_user_groups));
            }
            if (preg_match("/^RESET_PASSWORD/", $line)) {
                $reset_password = $line;
                $reset_password = trim(preg_replace("/.*=>/", '', $reset_password));
            }
            $p++;
        }
    } else {
...
if ($DB) {
    echo "DEBUG: |$reset_password|$reset_user_groups|\n";
}
