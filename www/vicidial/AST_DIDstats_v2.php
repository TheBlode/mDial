...
if (isset($_GET["time_BEGIN"])) {
    $time_BEGIN=$_GET["time_BEGIN"];
} elseif (isset($_POST["time_BEGIN"])) {
...
if (!$group) {
    ...
    if (strlen($extension[0]) > 0) {
        ...
        while ($d < $stats_array_ct) {
            ...
            while (strlen($stat_description) > 30) {
                $stat_description = preg_replace('/.$/i', '', $stat_description);
            }
            $carrier_desc =    sprintf("%-30s", $carrier_desc);
            while (strlen($carrier_desc) > 30) {
                $carrier_desc = preg_replace('/.$/i', '', $carrier_desc);
            }
            $custom1 =    sprintf("%-30s", $custom1);
            while (strlen($custom1) > 30) {
                $custom1 = preg_replace('/.$/i', '', $custom1);
            }
            $custom2 =    sprintf("%-30s", $custom2);
            while (strlen($custom2) > 30) {
                $custom2 = preg_replace('/.$/i', '', $custom2);
            }
            $custom3 =    sprintf("%-30s", $custom3);
            while (strlen($custom3) > 30) {
                $custom3 = preg_replace('/.$/i', '', $custom3);
            }
            $custom4 =    sprintf("%-30s", $custom4);
            while (strlen($custom4) > 30) {
                $custom4 = preg_replace('/.$/i', '', $custom4);
            }
            $custom5 =    sprintf("%-30s", $custom5);
            while (strlen($custom5) > 30) {
                $custom5 = preg_replace('/.$/i', '', $custom5);
            }
