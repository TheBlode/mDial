...
if (isset($_GET["logout_agent"])) {
    $logout_agent=$_GET["logout_agent"];
} elseif (isset($_POST["logout_agent"])) {
    $logout_agent=$_POST["logout_agent"];
}
    ...
    if ($sc_ct > 0) {
        ...
        while ($p <= $container_ct) {
            ...
            if (preg_match("/^LOGOUT_USER_GROUPS/", $line)) {
                $logout_user_groups = $line;
                $logout_user_groups = trim(preg_replace("/.*=>/", '', $logout_user_groups));
            }
            $p++;
        }
    } else {
