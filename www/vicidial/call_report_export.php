...
if (isset($_GET["query_hour"])) {
    $query_hour=$_GET["query_hour"];
} elseif (isset($_POST["query_hour"])) {
...
if (isset($_GET["ivr_export"])) {
    $ivr_export=$_GET["ivr_export"];
} elseif (isset($_POST["ivr_export"])) {
    $ivr_export=$_POST["ivr_export"];
}
...
if ($run_export > 0) {
    ...
    if ($date_field=="entry_date") {
        ...
        if (!preg_match("/EXTENDED/", $export_fields)) {
    ...
    if ($DB) {
        echo "** $regexLOGallowed_campaigns --- $LOGallowed_campaigns --- $campaign_string **\n";
    }
    ...
    if ($RUNcampaign > 0) {
        ...
        if (($outbound_to_print < 1) and ($RUNgroup < 1)) {
            ...
            while ($i < $outbound_to_print) {
                ...
                if ($LOGadmin_hide_phone_data != '0') {
                    if ($DB > 0) {
                        $DBout .= "HIDEPHONEDATA|$row[1]|$LOGadmin_hide_phone_data|\n";
                    }
                    $phone_temp = $row[1];
                ...
                if ($LOGadmin_hide_lead_data != '0') {
                    if ($DB > 0) {
                        $DBout .= "HIDELEADDATA|$row[6]|$row[7]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";
                    }
                    if (strlen($row[6]) > 0) {
                        $data_temp = $row[6];
        ...
        if ($DB) {
            $DBout .= "NO OUTBOUND CALLS: $RUNcampaign|$campaign_string|$campaign_ct\n";
        }
    }
    if ($RUNgroup > 0) {
        if (($export_fields == 'EXTENDED') or ($export_fields == 'EXTENDED_2') or ($export_fields == 'EXTENDED_3') or ($export_fields == 'EXTENDED_4')) {
    ...
    if (($outbound_to_print > 0) or ($inbound_to_print > 0)) {
        ...
        if ($header_row=='YES') {
            ...
            } elseif ($export_fields == 'ALTERNATE_2') {
        ...
        while ($k > $i) {
            ...
            if (($rec_fields=='ID') or ($rec_fields=='FILENAME') or ($rec_fields=='LOCATION') or ($rec_fields=='ALL')) {
                ...
                if ($recordings_ct < 1) {
                    ...
                    $rslt=mysql_to_mysqli($stmt, $link);
                    if ($DB) {
                        echo "CHECKING FOR RECORDINGS IN OTHER TABLE:$stmt\n";
                    }
                    $recordings_ct = mysqli_num_rows($rslt);
                    $u=0;
                    while ($recordings_ct > $u) {
