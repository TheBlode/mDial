...
if (isset($_GET["dial_status"])) {
    $dial_status=$_GET["dial_status"];
} elseif (isset($_POST["dial_status"])) {
...
if (!isset($hangup_cause)) {
    $hangup_cause = array();
}
if (!isset($dial_status)) {
    $dial_status = array();
}
...
while($i < $hangup_cause_ct) {
    ...
    while ($j < $dial_status_ct) {
        if ((strlen($hangup_cause[$i]) > 0) and (preg_match("/\|$hangup_cause[$i]\|/", $hangup_cause_string)) and (strlen($dial_status[$j]) > 0) and (preg_match("/\|$dial_status[$j]\|/", $dialstatus_string))) {
            if (preg_match('/\-\-ALL\-\-/', $hangup_cause_string)) {
                $HC_subclause="";
            } else {
                $HC_subclause="hangup_cause='$hangup_cause[$i]'";
            }
            if (preg_match('/\-\-ALL\-\-/', $dialstatus_string)) {
                $DS_subclause="";
            } else {
                $DS_subclause="dialstatus='$dial_status[$j]'";
            }
...
if ((preg_match('/\-\-ALL\-\-/', $hangup_cause_string)) or ($hangup_cause_ct < 1)) {
    ...
    if (preg_match('/\-\-ALL\-\-/', $hangup_cause_string)) {
        $hangup_causeQS="&hangup_cause[]=--ALL--";
    }
} else {
...
if ((preg_match('/\-\-ALL\-\-/', $dial_status_string)) or ($dial_status_ct < 1)) {
    ...
    if (preg_match('/\-\-ALL\-\-/', $dial_status_string)) {
        $dial_statusQS="&dial_status[]=--ALL--";
    }
} else {
