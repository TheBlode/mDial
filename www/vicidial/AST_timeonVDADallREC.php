...
if (isset($_GET["RECmonitorLINK"])) {
    $RECmonitorLINK=$_GET["RECmonitorLINK"];
} elseif (isset($_POST["RECmonitorLINK"])) {
    $RECmonitorLINK=$_POST["RECmonitorLINK"];
}
...
if ($non_latin > 0) {
    $rslta=mysql_to_mysqli("SET NAMES 'UTF8'", $link);
}  $rslt=mysql_to_mysqli($stmt, $link);
if (!isset($DB)) {
    $DB=0;
}
...
$Aecho .= "$Aline";
if ($orderby=='timeup') {
    $orderSQL='status,last_call_time';
}
if ($orderby=='timedown') {
    $orderSQL='status desc,last_call_time desc';
}
if ($orderby=='campaignup') {
    $orderSQL='campaign_id,status,last_call_time';
}
if ($orderby=='campaigndown') {
    $orderSQL='campaign_id desc,status desc,last_call_time desc';
}
if ($orderby=='groupup') {
    $orderSQL='user_group,status,last_call_time';
}
if ($orderby=='groupdown') {
    $orderSQL='user_group desc,status desc,last_call_time desc';
}
if ($UidORname > 0) {
    if ($orderby=='userup') {
...
if ($talking_to_print > 0) {
    ...
    while ($i < $talking_to_print) {
        ...
        if ($SIPmonitorLINK>0) {
            $L=" <a href=\"sip:6$Lsessionid@$server_ip\">LISTEN</a>";
            $R='';
        }
        if ($IAXmonitorLINK>0) {
            $L=" <a href=\"tel:$servip$EXTF$Lsessionid\">LISTEN</a>";
            $R='';
        }
        if ($SIPmonitorLINK>1) {
        ...
        if ($IAXmonitorLINK>1) {
            $R=" | <a href=\"tel:$servip$Lsessionid\">BARGE</a>";
        }
        if ($RECmonitorLINK>0) {
