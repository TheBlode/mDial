...
if ($report_display_type) {
    $HTML_text.="<option value='$report_display_type' selected>$report_display_type</option>";
}
...
if (!$group) {
    ...
    while ($m < $k) {
        ...
        if ($non_latin < 1) {
            ...
            while(mb_strlen($Slast_user_group, 'utf-8')>20) {
                $Slast_user_group = mb_substr("$Suser_group", 0, -1, 'utf-8');
            }
    ...
    while ($m < $k) {
        ...
        if ($non_latin < 1) {
            ...
            $Suser_group=    sprintf("%-45s", $Suser_group);
            while(mb_strlen($Suser_group, 'utf-8')>15) {
                $Suser_group = mb_substr("$Suser_group", 0, -1, 'utf-8');
            }
            $Suser =    sprintf("%-24s", $Suser);
            while(mb_strlen($Suser, 'utf-8')>8) {
