...
if (isset($_GET["RTdisplay_type"])) {
    $RTdisplay_type=$_GET["RTdisplay_type"];
} elseif (isset($_POST["RTdisplay_type"])) {
    $RTdisplay_type=$_POST["RTdisplay_type"];
}
...
if (($report_display_type=='HTML') or ($report_display_type=='WALL_2')) {
    ...
    $calls_waiting_td='bgcolor="#'.$SSmenu_background.'"';
    $calls_waiting_font="class=\"android_large bold\" color=\"white\"";
    if ($out_live > 0) {
        $calls_waiting_td='bgcolor="#FFCCCC"';
        $calls_waiting_font="class=\"android_large bold\"";
    }
    if ($out_live > 4) {
    ...
    if ($allow_chats) {
        $chats_waiting_td='bgcolor="#'.$SSmenu_background.'"';
        $chats_waiting_font="class=\"android_large bold\" color=\"white\"";
        if ($chats_waiting > 0) {
            $chats_waiting_td='bgcolor="#FFCCCC"';
            $chats_waiting_font="class=\"android_large bold\"";
        }
        if ($chats_waiting > 4) {
...
} elseif ((preg_match('/ALL\-ACTIVE/i', $group_string)) and (strlen($group_SQL) < 3)) {
    $UgroupSQL = '';
} else {
    $UgroupSQL = " and vicidial_live_agents.campaign_id IN($group_SQL)";
}
...
if ($talking_to_print > 0) {
    ...
    while ($j < $talking_to_print) {
        ...
        } elseif (preg_match("/3-WAY/i", $Astatus[$i])) {
        ...
        if ($report_display_type=='HTML') {
            ...
            if ($UGdisplay > 0) {
                $UGD = "<td NOWRAP align=right> $G$user_group$EG </td>";
            } else {
                $UGD = "";
            }
            if ($SERVdisplay > 0) {
...
if (($report_display_type=='HTML') or ($report_display_type=='WALL_2')) {
    ...
    $agents_waiting_td='bgcolor="#'.$SSmenu_background.'"';
    ...
    if ($agent_ready > 0) {
        ...
        $agents_waiting_font="class=\"android_large bold\"";
    }
    if ($agent_ready > 4) {
    ...
    if ($agent_ready > 14) {
        ...
        $agents_waiting_font="class=\"android_large bold\" color=\"white\"";
    }
