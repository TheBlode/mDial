    ...
    while ($m < $k) {
        ...
        if ($file_download<1) {
            if ($non_latin < 1) {
                ...
                while(mb_strlen($Sfull_name, 'utf-8')>25) {
...
while ($user_groups_to_print > $o) {
    if  (preg_match("/$allowed_user_groups[$o]\|/i", $user_group_string)) {
        echo "<option selected value=\"$allowed_user_groups[$o]\">$allowed_user_groups[$o]</option>\n";
    } else {
        echo "<option value=\"$allowed_user_groups[$o]\">$allowed_user_groups[$o]</option>\n";
    }
    $o++;
}
echo "</SELECT>\n";
