...
if (isset($_GET["show_parks"])) {
    $show_parks=$_GET["show_parks"];
} elseif (isset($_POST["show_parks"])) {
    $show_parks=$_POST["show_parks"];
}
if (isset($_GET["DB"])) {
    $DB=$_GET["DB"];
...
if ($DB) {
    echo "|$user_group_string|$user_group_ct|$user_groupQS|$i<BR>";
}
...
if ((strlen($group[0]) < 1) or (strlen($user_group[0]) < 1)) {
    ...
    while ($i < $rows_to_print) {
        ...
        if (($lead > 0) and ((!preg_match("/NULL/i", $status)) and (strlen($status) > 0))) {
            $TOTcalls++;
        }
        $user_found=0;
        if ($uc < 1) {
    ...
    if ($DB) {
        echo "Done gathering $i records, analyzing...<BR>\n";
    }
    if ($file_download < 1) {
    ...
    while (($m < $uc) and ($m < 50000)) {
        ...
        $RAWcalls = $Scalls[$m];
        $RAWtimeSEC = $Stime[$m];
        if (trim($Scalls[$m])>$max_calls) {
            $max_calls=trim($Scalls[$m]);
        }
        if (trim($Stime[$m])>$max_agenttime) {
            $max_agenttime=trim($Stime[$m]);
        }
        if (trim($Swait[$m])>$max_wait) {
            $max_wait=trim($Swait[$m]);
        }
        if (trim($Stalk[$m])>$max_talk) {
            $max_talk=trim($Stalk[$m]);
        }
        if (trim($Sdispo[$m])>$max_dispo) {
            $max_dispo=trim($Sdispo[$m]);
        }
        if (trim($Spause[$m])>$max_pause) {
            $max_pause=trim($Spause[$m]);
        }
        if (trim($Sdead[$m])>$max_dead) {
            $max_dead=trim($Sdead[$m]);
        }
        if (trim($Scustomer[$m])>$max_customer) {
            $max_customer=trim($Scustomer[$m]);
        }
        if (trim($Sconnected[$m])>$max_connected) {
            $max_connected=trim($Sconnected[$m]);
        }
        if (trim($Swaitpct[$m])>$max_waitpct) {
            $max_waitpct=trim($Swaitpct[$m]);
        }
        if (trim($Stalkpct[$m])>$max_talkpct) {
            $max_talkpct=trim($Stalkpct[$m]);
        }
        if (trim($Sdispopct[$m])>$max_dispopct) {
            $max_dispopct=trim($Sdispopct[$m]);
        }
        if (trim($Spausepct[$m])>$max_pausepct) {
            $max_pausepct=trim($Spausepct[$m]);
        }
        if (trim($Sdeadpct[$m])>$max_deadpct) {
            $max_deadpct=trim($Sdeadpct[$m]);
        }
        ...
        while ($n < $punches_to_print) {
            if ($Suser[$m] == "$TCuser[$n]") {
                ...
                if (trim($RAWtimeTCsec)>$max_timeclock) {
                    $max_timeclock=trim($RAWtimeTCsec);
                }
        ...
        if (trim($user_holds)>$max_user_holds) {
            $max_user_holds=trim($user_holds);
        }
        if (trim($park_array[$Suser[$m]][1]+0)>$max_total_hold_time) {
            $max_total_hold_time=trim($park_array[$Suser[$m]][1]+0);
        }
        if (trim(round(MathZDC(($park_array[$Suser[$m]][1]+0), $user_holds)))>$max_avg_hold_time) {
            $max_avg_hold_time=trim(round(MathZDC(($park_array[$Suser[$m]][1]+0), $user_holds)));
        }
        if (trim($user_hpc)>$max_user_hpc) {
            $max_user_hpc=trim($user_hpc);
        }
        ...
        if ($file_download<1) {
            if ($non_latin < 1) {
                ...
                while(mb_strlen($Sname[$m], 'utf-8')>25) {
        ...
        if ($file_download < 1) {
            ...
            if (strlen($RAWwaitpct)<0) {
                $RAWwaitpct='0.0%';
            }
            if (strlen($RAWtalk)<1) {
                $RAWtalk='0';
            }
            if (strlen($RAWtalkpct)<0) {
                $RAWtalkpct='0.0%';
            }
            if (strlen($RAWdispo)<1) {
                $RAWdispo='0';
            }
            if (strlen($RAWdispopct)<0) {
                $RAWdispopct='0.0%';
            }
            if (strlen($RAWpause)<1) {
                $RAWpause='0';
            }
            if (strlen($RAWpausepct)<0) {
                $RAWpausepct='0.0%';
            }
            if (strlen($RAWdead)<1) {
                $RAWdead='0';
            }
            if (strlen($RAWdeadpct)<0) {
                $RAWdeadpct='0.0%';
            }
            if (strlen($RAWcustomer)<1) {
                $RAWcustomer='0';
            }
            if (strlen($RAWconnected)<1) {
                $RAWconnected='0';
            }
    ...
    $k=$m;
    if ($DB) {
        echo "Done analyzing...   $TOTwait|$TOTtalk|$TOTdispo|$TOTpause|$TOTdead|$TOTcustomer|$TOTconnected|$TOTALtime|$TOTcalls|$uc|<BR>\n";
    }
