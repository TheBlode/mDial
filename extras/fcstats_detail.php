...
if ($query_hour) {
    $HTML_text.="<option value='$query_hour' selected>$query_hour</option>\n";
}
...
if ($end_hour) {
    $HTML_text.="<option value='$end_hour' selected>$end_hour</option>\n";
}
$HTML_text.="<option value=''>HR</option>\n";
for ($q=0; $q<24; $q++) {
    $q=substr("0$q", -2);
...
if (!$group) {
    ...
    $sale_dispo_str.="|";
    if ($DB) {
        $HTML_text.="Sale dispo string: $sale_dispo_str\n";
    }
    ...
    while ($i < $users_to_print) {
        ...
        if ($calls_to_print==0) {
            $hidden_span_txt.=": ".sprintf("%-59s", "*** NO TRANSFER INFORMATION TO REPORT ***")." :\n";
        }
        ...
        if ($USERcalls[$i]>$max_xfers) {
            $max_xfers=$USERcalls[$i];
        }
    ...
    while ($i < $users_to_print) {
        ...
        $totA1 = ($totA1 + $A1);
        $TOTsales = ($TOTsales + $A1);
        $totA2 = ($totA2 + $A2);
        $TOTsales = ($TOTsales + $A2);
        $totTOP = ($totTOP + $A2);
        $totA3 = ($totA3 + $A3);
        $TOTsales = ($TOTsales + $A3);
        $totBOT = ($totBOT + $A3);
        $totA4 = ($totA4 + $A4);
        $TOTsales = ($TOTsales + $A4);
        $totTOP = ($totTOP + $A4);
        $totBOT = ($totBOT + $A4);
        $totA5 = ($totA5 + $A5);
        $totA6 = ($totA6 + $A6);
