...
if (strlen($uniqueid) < 10) {
    print "ERROR: uniqueid is not valid";
    exit;
}
$HTTPheader = array();
if (strlen($headers) > 1) {
    ...
    while ($h < $headersARYct) {
        ...
        if ($DB) {
            echo "HTTP Header SET: |" . $headerXary[0] . ": " . $headerXary[1] ."|\n";
        }
        $h++;
    }
}
if (preg_match("/HTTPURLTOPOST=|HTTPSURLTOPOST=/", $query_string)) {
    ...
    if (($curl_ready > 0) and (strlen($post_page) > 8)) {
        ...
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($mel > 0) {
            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00422', $user, $server_ip, $session_name, $one_mysql_log);
        }
        $affected_rows = mysqli_affected_rows($link);
    } else {
        print "ERROR: post url is invalid";
        exit;
    }
} else {
    print "ERROR: post url is not populated - " . strlen($query_string);
    exit;
}
if (strlen($resp) > 0) {
    echo "$resp";
