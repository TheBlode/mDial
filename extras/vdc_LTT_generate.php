...
if (isset($_GET["talk_time_min"])) {
    $talk_time_min=$_GET["talk_time_min"];
} elseif (isset($_POST["talk_time_min"])) {
    $talk_time_min=$_POST["talk_time_min"];
}
...
if (isset($_GET["dispo_name"])) {
    $dispo_name=$_GET["dispo_name"];
} elseif (isset($_POST["dispo_name"])) {
    $dispo_name=$_POST["dispo_name"];
}
if (isset($_GET["event"])) {
    $event=$_GET["event"];
...
if (strlen($lead_id) < 1) {
    ...
    if ($address3 == 'ACME Shipping') {
        $EXship01a = 'EXshipACME.wav';
    }
    if ($address3 == 'International Parcel Shipping') {
        $EXship01a = 'EXshipIPS.wav';
    }
    if ($address3 == 'United States Postal Service') {
        $EXship01a = 'EXshipUSPS.wav';
    }
    ...
    if ($security_phraseARY[0] == '0') {
        $EXship05a = 'digits/0.wav|';
    } else {
    ...
    if ($security_phraseARY[1] == '0') {
        $EXship05b = 'digits/0.wav|';
    } else {
...
function convertWordToSpell($DB, $word)
{
    ...
    if ($DB > 0) {
        print "DEBUG: ($word $orderARY_ct)";
    }
    $i=0;
    $spell='';
    while($orderARY_ct > $i) {
        if (preg_match('/^[0-9]/', $orderARY[$i])) {
            $spell .= "digits/".$orderARY[$i].".wav|";
        }
        ...
        if ($orderARY[$i] == '#') {
            $spell .= "digits/pound.wav|";
        }
        if ($orderARY[$i] == '*') {
            $spell .= "digits/star.wav|";
        }
        if ($orderARY[$i] == '-') {
            $spell .= "letters/dash.wav|";
        }
        if ($orderARY[$i] == ' ') {
            $spell .= "letters/space.wav|";
        }
        if ($orderARY[$i] == '/') {
            $spell .= "letters/slash.wav|";
        }
        if ($orderARY[$i] == '+') {
            $spell .= "letters/plus.wav|";
        }
        if ($orderARY[$i] == '!') {
            $spell .= "letters/exclaimation-point.wav|";
        }
        if ($orderARY[$i] == '=') {
            $spell .= "letters/equals.wav|";
        }
        if ($orderARY[$i] == '@') {
            $spell .= "letters/at.wav|";
        }
        if ($orderARY[$i] == '.') {
            $spell .= "letters/dot.wav|";
        }
        $i++;
    }
    return $spell;
...
function convertNumberToOrder($DB, $num)
{
    ...
    $order='';
    if (($num >= 1) and ($num <= 20)) {
        $order .= "digits/h-".$num.".wav|";
    }
    if (($num >= 21) and ($num <= 99)) {
        if (substr($num, -1) == '0') {
            $order .= "digits/h-".$num.".wav|";
        } else {
            $order .= "digits/".substr($num, 0, 1)."0.wav|digits/h-".substr($num, -1).".wav|";
        }
    }
    return $order;
}
