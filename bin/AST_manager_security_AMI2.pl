...
$LItelnetlog =
  "$PATHlogs/security_event_telnet_log.txt";    # uncomment for telnet log
...
sub handle_event {
    ...
    switch ( $event_hash{'Event'} ) {
        ...
        case "InvalidAccountID" {
            return handle_invalid_account_id(%event_hash);
        }
        case "SessionLimit" { return handle_session_limit(%event_hash); }
        case "MemoryLimit"  { return handle_memory_limit(%event_hash); }
        case "LoadAverageLimit" {
            return handle_load_average_limit(%event_hash);
        }
        case "RequestNotAllowed" {
            return handle_request_not_allowed(%event_hash);
        }
        case "AuthMethodNotAllowed" {
            return handle_auth_method_not_allowed(%event_hash);
        }
        case "RequestBadFormat" {
            return handle_request_bad_format(%event_hash);
        }
        case "SuccessfulAuth" { return handle_successful_auth(%event_hash); }
        case "UnexpectedAddress" {
            return handle_unexpected_address(%event_hash);
        }
        case "ChallengeResponseFailed" {
            return handle_challenge_response_failed(%event_hash);
        }
        ...
        case "InvalidTransport" {
            return handle_invalid_transport(%event_hash);
        }
        else { return 2; }
    }
}
...
sub handle_unexpected_address {
        ...
        && ( exists $event_hash{'ExpectedAddress'} ) )
    {
        ...
        ( $event_hash{'RemoteIP'}, $event_hash{'RemotePort'} ) =
          split_address( $event_hash{'RemoteAddress'} );
        ( $event_hash{'ExpectedIP'}, $event_hash{'ExpectedPort'} ) =
          split_address( $event_hash{'ExpectedAddress'} );
...
sub handle_invalid_password {
        ...
        && ( exists $event_hash{'RemoteAddress'} ) )
    {
        ...
        my $optional_two =
          "ReceivedChallenge: $event_hash{'ReceivedChallenge'}";
...
sub handle_invalid_transport {
    ...
    {
        my $optional_one =
          "AttemptedTransport: $event_hash{'AttemptedTransport'}";
        ( $event_hash{'LocalIP'}, $event_hash{'LocalPort'} ) =
          split_address( $event_hash{'LocalAddress'} );
        ( $event_hash{'RemoteIP'}, $event_hash{'RemotePort'} ) =
          split_address( $event_hash{'RemoteAddress'} );
