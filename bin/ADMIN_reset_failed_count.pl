...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        print
          "Reset a given users failed count so they can attempt to log in.\n";
        print "\n";
...
$affected_rows = 0;
$stmtA =
  "UPDATE vicidial_users set failed_login_count=0 where user='$CLIuser';";
if ( $T < 1 ) {
...
if ( $DB > 0 ) {
    print
      "Failed Login Count for user $CLIuser has been reset: |$affected_rows|\n";
}
