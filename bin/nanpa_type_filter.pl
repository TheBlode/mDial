...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
          "  [--db-output-code=XXX] = code used to tag output to db table\n";
...
"  [--exclude-value=XXX] = OPTIONAL, value of the above field that you want to check in vicidial_list to exclude from processing\n";
        print
"  [--include-field=XXX] = OPTIONAL, field that you want to check in vicidial_list to include only for processing\n";
        print
"  [--include-value=XXX] = OPTIONAL, value of the above field that you want to check in vicidial_list to include only for processing\n";
        print
"                                    you can use EMPTY to stand for an empty string\n";
        print "\n";
        exit;
    }
    else {
...
if (   ( length($list_id) < 2 )
    || ( length($PREFIX_user) < 2 )
    || ( length($PREFIX_pass) < 2 )
    || ( length($PREFIX_url) < 2 ) )
{
    print
"user, pass, url and list-id options must be set for this script to run: |$PREFIX_user|XXXX|$list_id|\n";
    exit;
}
...
if ( length($db_output_code) < 2 ) {
    $db_output_code = $filedate . "_" . $VDrandom;
}
use DBI;
...
if ($output_to_db) {
    $stmtA =
"INSERT INTO vicidial_nanpa_filter_log SET output_code='$db_output_code',user='$TRIGGER_user',status='LAUNCHED',server_ip='$VARserver_ip',list_id='$list_id',start_time=NOW(),update_time=NOW(),status_line='Starting...',script_output=\"Starting...\nCELLPHONE LIST ID: $cellphone_list_id\nLANDLINE LIST ID: $landline_list_id\nINVALID LIST ID: $invalid_list_id\nVL FIELD UPDATE: $vl_field_update\nEXCLUDE FIELD: $exclude_field\nEXCLUDE VALUE: $exclude_value\nINCLUDE FIELD: $include_field\nINCLUDE VALUE: $include_value\n\" ON DUPLICATE KEY UPDATE status='LAUNCHED',server_ip='$VARserver_ip',list_id='$list_id',update_time=NOW(),status_line=\"Starting...\n\",script_output=CONCAT(script_output,\"Starting...\nCELLPHONE LIST ID: $cellphone_list_id\nLANDLINE LIST ID: $landline_list_id\nINVALID LIST ID: $invalid_list_id\nVL FIELD UPDATE: $vl_field_update\nEXCLUDE FIELD: $exclude_field\nEXCLUDE VALUE: $exclude_value\nINCLUDE FIELD: $include_field\nINCLUDE VALUE: $include_value\n\");";
...
$stmtA =
"SELECT lead_id,phone_number from vicidial_list $list_idSQL $excludeSQL $includeSQL;";
...
if ($output_to_db) {
    $stmtA =
"UPDATE vicidial_nanpa_filter_log SET status='LEADS_GATHERED',leads_count='$rec_count',update_time=NOW(),status_line='Leads Gathered: $rec_count',script_output=CONCAT(script_output,\"Leads Gathered: $rec_count\n\") where output_code='$db_output_code';";
    $affected_rows = $dbhA->do($stmtA);
    if ($DBX) { print STDERR "\n|$affected_rows|$stmtA|\n"; }
}
$rec_count    = 0;
...
while ( $sthArows > $rec_count ) {
    ...
    if ( ( $batch_count < 1000 ) && ( $rec_next < $sthArows ) ) {
        ...
        $secTEMP = time();
        $postdata =
          "user=$PREFIX_user&pass=$PREFIX_pass&phone_number=$batch_phones";
        $tempurl = $PREFIX_url;
        $tempfilename =
          "$lead_ids[$rec_count]$phones[$rec_count]$secTEMP" . '.txt';
        $temp_wget =
"$wgetbin -q --output-document=/tmp/$tempfilename $tempurl --post-data=\"$postdata\" ";
        if ( $DBX > 0 ) { print STDERR "$temp_wget\n"; }
        @wget_output = `$temp_wget `;
        open( tempweboutput, "/tmp/$tempfilename" )
          || die "can't open /tmp/$tempfilename: $!\n";
        ...
        foreach (@tempweboutput) {
            ...
            if (   ( $phones[$temp_rec] =~ /$result_split[1]/ )
                && ( length( $lead_ids[$temp_rec] ) > 0 ) )
            {
                $stmtA = '';
                ...
                if (   ( $result_split[0] =~ /I/ )
                    && ( length($invalid_list_id) > 1 ) )
                {
                    $stmtA =
"UPDATE vicidial_list SET list_id=$invalid_list_id where lead_id=$lead_ids[$temp_rec];";
                }
                if (   ( $result_split[0] =~ /C/ )
                    && ( length($cellphone_list_id) > 1 ) )
                {
                    $stmtA =
"UPDATE vicidial_list SET list_id=$cellphone_list_id where lead_id=$lead_ids[$temp_rec];";
                }
                if (   ( $result_split[0] =~ /S|V/ )
                    && ( length($landline_list_id) > 1 ) )
                {
                    $stmtA =
"UPDATE vicidial_list SET list_id=$landline_list_id where lead_id=$lead_ids[$temp_rec];";
                }
                if ( ( length($stmtA) > 10 ) && ( $TEST < 1 ) ) {
                    ...
                    $affected_rows = $dbhA->do($stmtA);
                    if ($DBX) {
                        print STDERR "|$affected_rows records changed|\n";
                    }
                }
                if ( ( length($vl_field_update) > 2 ) && ( $TEST < 1 ) ) {
                    $stmtA =
"UPDATE vicidial_list SET $vl_field_update='$result_split[0]' where lead_id=$lead_ids[$temp_rec];";
                    if ($DBX) {
                        print STDERR "\n|$stmtA|$phones[$temp_rec]|\n";
                    }
                    $affected_rows = $dbhA->do($stmtA);
        ...
        if ($output_to_db) {
            $stmtA =
"UPDATE vicidial_nanpa_filter_log SET status='NANPA_LOOKUPS',update_time=NOW(),filter_count='$rec_count',status_line='(INV: $invalid|CEL: $cellphone|LND: $landline)' where output_code='$db_output_code';";
            $affected_rows = $dbhA->do($stmtA);
            if ($DBX) { print STDERR "\n|$affected_rows|$stmtA|\n"; }
        }
    }
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /00$/i ) {
            print STDERR
" $dance $rec_count    (INV: $invalid|CEL: $cellphone|LND: $landline)\r";
        }
    }
}
$secY  = time();
...
if ($output_to_db) {
    $stmtA =
"UPDATE vicidial_nanpa_filter_log SET status='COMPLETED',filter_count='$rec_count',update_time=NOW(),status_line='Process Completed: $secZ seconds',script_output=CONCAT(script_output,\"Process Completed: $secZ seconds\n   INVALID:    $invalid\n   CELLPHONE:  $cellphone\n   LANDLINE:   $landline\n\n\") where output_code='$db_output_code';";
    $affected_rows = $dbhA->do($stmtA);
    if ($DBX) { print STDERR "\n|$affected_rows|$stmtA|\n"; }
}
exit;
