...
( $Ssec, $Smin, $Shour, $Smday, $Smon, $Syear, $Swday, $Syday, $Sisdst ) =
  localtime($STARTtarget);
$Syear = ( $Syear + 1900 );
$Smon++;
if ( $Smon < 10 )  { $Smon  = "0$Smon"; }
if ( $Smday < 10 ) { $Smday = "0$Smday"; }
$START_week       = ( ( $Syday / 7 ) + 1 );
$START_week       = sprintf( "%2d", $START_week );
...
( $Ssec, $Smin, $Shour, $Smday, $Smon, $Syear, $Swday, $Syday, $Sisdst ) =
  localtime($PAST_STARTtarget);
$Syear = ( $Syear + 1900 );
$Smon++;
if ( $Smon < 10 )  { $Smon  = "0$Smon"; }
if ( $Smday < 10 ) { $Smday = "0$Smday"; }
$PAST_START_week       = ( ( $Syday / 7 ) + 1 );
$PAST_START_week       = sprintf( "%2d", $PAST_START_week );
$PAST_STARTtarget_date = "$Syear-$Smon-$Smday";
$PAST_ENDtarget        = ( $ENDtarget - ( 86400 * 7 ) );
( $Ssec, $Smin, $Shour, $Smday, $Smon, $Syear, $Swday, $Syday, $Sisdst ) =
  localtime($PAST_ENDtarget);
$Syear = ( $Syear + 1900 );
$Smon++;
if ( $Smon < 10 )  { $Smon  = "0$Smon"; }
if ( $Smday < 10 ) { $Smday = "0$Smday"; }
...
( $Ssec, $Smin, $Shour, $Smday, $Smon, $Syear, $Swday, $Syday, $Sisdst ) =
  localtime($PAST_Wednesdaytarget);
$Syear = ( $Syear + 1900 );
$Smon++;
if ( $Smon < 10 )  { $Smon  = "0$Smon"; }
if ( $Smday < 10 ) { $Smday = "0$Smday"; }
...
$TLoutfile .=
  "THIS WEEK:  week $START_week    $STARTtarget_date - $ENDtarget_date\n";
$TLoutfile .=
"LAST WEEK:  week $PAST_START_week    $PAST_STARTtarget_date - $PAST_ENDtarget_date\n\n";
...
"AGENT                USER     CALLS  TALK     PAUSE    WAIT     DISPO    ACTIVE   LOGTIME  FIRST                LAST                    \n";
    $stmtA =
"select count(*) as calls, full_name,vicidial_users.user,sum(talk_sec),sum(pause_sec),sum(wait_sec),sum(dispo_sec) from vicidial_users,$vicidial_agent_log where event_time <= '$PAST_date[$h] 23:59:59' and event_time >= '$PAST_date[$h] 00:00:00' and vicidial_users.user=$vicidial_agent_log.user and pause_sec<48800 and wait_sec<48800 and talk_sec<48800 and dispo_sec<48800 group by vicidial_users.user order by full_name limit 10000;";
    ...
    while ( $sthArows > $rec_count ) {
        ...
        $stmtB =
"select event_time,UNIX_TIMESTAMP(event_time) from $vicidial_agent_log where event_time <= '$PAST_date[$h] 23:59:59' and event_time >= '$PAST_date[$h] 00:00:00' and user='$aryA[2]' order by event_time limit 1;";
        ...
        $first_log  = $aryB[1];
        $stmtB =
"select event_time,UNIX_TIMESTAMP(event_time) from $vicidial_agent_log where event_time <= '$PAST_date[$h] 23:59:59' and event_time >= '$PAST_date[$h] 00:00:00' and user='$aryA[2]' order by event_time desc limit 1;";
        $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
        $sthB->execute or die "executing: $stmtA ",          $dbhB->errstr;
        @aryB      = $sthB->fetchrow_array;
