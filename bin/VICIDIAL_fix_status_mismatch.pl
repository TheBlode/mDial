...
print
"This program is designed to scan all leads marked A4 and set them to their proper status according to the logs. \n\n";
$stmtA =
"select distinct lead_id,status from vicidial_closer_log where status IN('A1','A2','A3','A4') and call_date > '2008-03-01 01:00:00';";
if ($DBX) { print STDERR "\n|$stmtA|\n"; }
...
"select status,start_epoch from vicidial_closer_log where lead_id='$lead_id[$b]' order by closecallid desc LIMIT 1;";
    ...
    if ( $Cstatus !~ /$status[$b]/ ) {
        $stmtA =
"UPDATE vicidial_list set status='$Cstatus' where lead_id='$lead_id[$b]';";
        if ($DB) {
            print STDERR
              "|$b|$lead_id[$b]|$Nstatus|$Cstatus|$status[$b]||$stmtA|\n";
        }
        $h++;
    }
    $b++;
    ...
    if ( $b =~ /000$/i ) {
        print "|$b|$h|$lead_id[$b]|$Nstatus|$Cstatus|$status[$b]|\n";
    }
}
chop($DUP_updates);
$dbhA->disconnect();
