...
( $Ysec, $Ymin, $Yhour, $Ymday, $Ymon, $Yyear, $Ywday, $Yyday, $Yisdst ) =
  localtime($YDtarget);
$Yyear = ( $Yyear + 1900 );
$Ymon++;
...
if ( $purge > 0 ) {
    ...
    $stmtA =
      "DELETE FROM twoday_vicidial_closer_log where call_date < '$YDSQLdate';";
    if ( !$T ) {
        $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print "\n|$affected_rows records changed|$stmtA|\n"; }
    }
    $stmtA =
      "DELETE FROM twoday_recording_log where start_time < '$YDSQLdate';";
    if ( !$T ) {
        $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print "\n|$affected_rows records changed|$stmtA|\n"; }
    }
    $stmtA =
      "DELETE FROM twoday_vicidial_xfer_log where call_date < '$YDSQLdate';";
    if ( !$T ) {
        $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print "\n|$affected_rows records changed|$stmtA|\n"; }
    }
    $stmtA =
      "DELETE FROM twoday_vicidial_agent_log where event_time < '$YDSQLdate';";
    if ( !$T ) {
        $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print "\n|$affected_rows records changed|$stmtA|\n"; }
    }
