...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--seconds=X] = maximum number of wait seconds for agent. Default is 60\n";
        print
"  [--campaign=XXX] = REQUIRED! Campaign that sales will be pulled from. For more than 1, separate with dash\n";
        print
...
"  [--email-sender=vicidial@localhost] = sender for the email results\n";
        print
"  [--container=XXX] = the ID of a settings container to run from the system, overwrites other options\n";
        print "  [--quiet] = quiet\n";
...
"SELECT container_entry FROM vicidial_settings_containers where container_id = '$container';";
    ...
    if ( length($container_entry) < 3 ) {
        if ( $DB > 0 ) {
            print "NOTICE: Invalid Container: |$container|$container_entry|\n";
        }
    }
    else {
        $container_entry =~ s/\n|\r|\t/ /gi;
...
if ($DBX) {
    print
"Start time: $timestamp($web_u_time)   AGENT WAIT SECONDS: $seconds($max_wait_epoch)\n";
}
if ( length($campaign) < 2 ) {
    if ( !$Q ) { print "ERROR: no campaign defined: |$campaign|$campaignSQL|"; }
}
$stmtA =
"SELECT user,status,campaign_id,server_ip,UNIX_TIMESTAMP(last_state_change) FROM vicidial_live_agents where status IN('CLOSER','READY') and UNIX_TIMESTAMP(last_state_change) < '$max_wait_epoch' $campaignSQL order by last_state_change;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
...
while ( $sthArowsSERVERS > $i ) {
    ...
    if ($DBX) {
        print
"Agent $i: - $Auser ($Astatus - $Acampaign - $Aserver)   Waiting: $Awaitsec sec\n";
    }
    $Ealert .=
"Agent $i: - $Auser ($Astatus - $Acampaign - $Aserver)   Waiting: $Awaitsec sec\n";
}
