...
sub handle_event {
    ...
    switch ( $event_hash{'Event'} ) {
        ...
        case "SIPCriticalTimeout" {
            return handle_sip_crit_timeout_event(%event_hash);
        }
        case "PeerRegistered" {
            return handle_peer_registered_event(%event_hash);
        }
        case "SIPRTPDisconnect" {
            return handle_sip_rtp_disconnect_event(%event_hash);
        }
...
sub handle_sip_crit_timeout_event {
        ...
        && ( exists $event_hash{'Timeout'} ) )
    {
        ...
        $stmtA =
"INSERT INTO vicidial_peer_event_log SET event_date=NOW(), event_type='CRITICALTIMEOUT', server_ip='$server_ip', host_ip='$ip', port='$port', channel_type='SIP', data='|Type=$event_hash{'Type'}|CallID=$event_hash{'CallID'}|SeqNo=$event_hash{'SeqNo'}|Timeout=$event_hash{'Timeout'}|'";
        ...
        print STDERR
"SIPCriticalTimeout event does not have a Type, CallID, SeqNo, Host, or Timeout ?!!!\n";
        return 3;
    }
}
...
sub handle_peer_registered_event {
    my %event_hash = @_;
    if (   ( exists $event_hash{'ChannelType'} )
        ...
        && ( exists $event_hash{'Host'} ) )
    {
        ...
        $stmtA =
"INSERT INTO vicidial_peer_event_log SET event_date=NOW(6), event_type='REGISTERED', server_ip='$server_ip', host_ip='$ip', port='$port', channel_type='$event_hash{'ChannelType'}', peer='$event_hash{'Peer'}'";
        if ($DB) { print STDERR "|$stmtA|"; }
        my $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print STDERR "$affected_rows|\n"; }
        $stmtA =
"UPDATE phones set phone_ip='$ip' where server_ip='$server_ip' and extension='$phone_extension' and protocol='$event_hash{'ChannelType'}';";
        if ($DB) { print STDERR "|$stmtA|\n"; }
        my $affected_rows = $dbhA->do($stmtA);
        ...
        print STDERR
"PeerRegistered event does not have a ChannelType, Peer, or Host ?!!!\n";
        return 3;
    }
}
...
sub handle_sip_rtp_disconnect_event {
        ...
        && ( exists $event_hash{'RTPLastRX'} ) )
    {
        ...
        $stmtA =
"INSERT INTO vicidial_peer_event_log SET event_date=NOW(6), event_type='RTPDISCONNECT', server_ip='$server_ip', channel_type='$type', channel='$event_hash{'Channel'}', data='|RTPLastRX=$event_hash{'RTPLastRX'}|'";
        if ($DB) { print STDERR "|$stmtA|"; }
        my $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print STDERR "$affected_rows|\n"; }
        return 1;
        ...
        print STDERR
          "SIPRTPDisconnect event does not have a Channel or RTPLastRX ?!!!\n";
        return 3;
    }
}
...
sub handle_peer_status_event {
        ...
        && ( exists $event_hash{'Peer'} ) )
    {
        ...
        if ( exists( $event_hash{'PingTime'} ) ) {
            $data .= "PingTime=$event_hash{'PingTime'}|";
        }
        if ( exists( $event_hash{'Time'} ) ) {
            $data .= "Time=$event_hash{'Time'}|";
        }
        if ( exists( $event_hash{'MaxPing'} ) ) {
            $data .= "MaxPing=$event_hash{'MaxPing'}|";
        }
        $stmtA =
"INSERT INTO vicidial_peer_event_log SET event_date=NOW(6), event_type='$event_hash{'PeerStatus'}', server_ip='$server_ip', channel_type='$event_hash{'ChannelType'}', peer='$event_hash{'Peer'}', data='$data'";
        if ($DB) { print STDERR "|$stmtA|"; }
        my $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print STDERR "$affected_rows|\n"; }
        if ( exists( $event_hash{'PingTime'} ) ) {
...
"UPDATE phones set peer_status = '$event_hash{'PeerStatus'}', ping_time = '$event_hash{'PingTime'}' where server_ip='$server_ip' and extension='$phone_extension' and protocol='$event_hash{'ChannelType'}';";
            ...
            $stmtA =
"UPDATE phones set peer_status = '$event_hash{'PeerStatus'}' where server_ip='$server_ip' and extension='$phone_extension' and protocol='$event_hash{'ChannelType'}';";
        }
        if ($DB) { print STDERR "|$stmtA|\n"; }
        my $affected_rows = $dbhA->do($stmtA);
        if ($DB) { print "|$affected_rows phones updated|\n"; }
        ...
        print STDERR
"PeerStatus event does not have a ChannelType, PeerStatus, Peer, or Time ?!!!\n";
        return 3;
    }
}
...
"UPDATE vicidial_manager set status='DEAD', channel='$event_hash{'Channel'}' where server_ip = '$server_ip' and uniqueid = '$event_hash{'Uniqueid'}' and cmd_line_d!='Exten: 8309' and cmd_line_d!='Exten: 8310';";
            ...
            if ($DB) {
                print STDERR
"Ignoring CXFER Local Hangup: |$event_hash{'Channel'}|$server_ip|$event_hash{'Uniqueid'}|$callid|\n";
            }
            return 1;
        }
    }
        ...
        print STDERR
"Hangup event does not have a Channel, Uniqueid, or CallerIDName ?!!!\n";
        return 3;
    }
}
...
"UPDATE vicidial_manager set status='UPDATED', channel='$event_hash{'Channel'}', uniqueid = '$event_hash{'Uniqueid'}' where server_ip = '$event_hash{'ServerIP'}' and callerid = '$call_id' and status != 'NEW'";
            ...
            if ( $event_hash{'Channel'} !~ /local/i ) {
                if ($DB) {
                    print STDERR
"|$event_hash{'Channel'}|NON LOCAL CHANNEL >>>> EXECUTING ABOVE STATMENT|\n";
                }
                my $affected_rows = $dbhA->do($stmtA);
                ...
                if ($DB) {
                    print STDERR
"|$event_hash{'Channel'}|LOCAL CHANNEL >>>> ABOVE STATMENT IGNORED|\n";
                }
            }
        }
        else {
            if ($DBX) {
                print
"Channel State $event_hash{'ChannelStateDesc'} not Dialing, Ringing, or Up. Ignoring.";
            }
        }
        return 1;
    }
        ...
        print STDERR
"Newstate event does not have a Channel, Uniqueid, ChannelStateDesc, ConnectedLineName, or CallerIDName ?!!!\n";
        return 3;
    }
}
...
"UPDATE vicidial_manager set status='UPDATED', channel='$event_hash{'Channel'}', uniqueid = '$event_hash{'Uniqueid'}' where server_ip = '$event_hash{'ServerIP'}' and callerid = '$call_id'";
        ...
        print STDERR
"NewCallerid event does not have a Channel, Uniqueid, ConnectedLineName, or CallerIDName ?!!!\n";
        return 3;
    }
}
...
"INSERT INTO vicidial_dtmf_log SET dtmf_time=NOW(),channel='$event_hash{'Channel'}',server_ip='$event_hash{'ServerIP'}',uniqueid='$event_hash{'Uniqueid'}',digit='$event_hash{'Digit'}',direction='$event_hash{'Direction'}',state='Begin'";
        ...
        my $affected_rows = $dbhA->do($stmtA);
        ( $s_hires, $usec ) =
          gettimeofday();    # get seconds and microseconds since the epoch
...
"|$affected_rows vicidial_dtmf inserted|$HRnow_date|$s_hires|$usec|\n";
        ...
        $dtmf_string =
"$HRnow_date|$s_hires|$usec|$event_hash{'Channel'}|$event_hash{'Uniqueid'}|$event_hash{'Digit'}|$event_hash{'Direction'}|Begin|$event_hash{'CallerIDName'}";
        &dtmf_logger;
        return 1;
    }
    else {
        print STDERR
"DTMFBegin event does not have a Channel, Uniqueid, Direction, Digit, ConnectedLineName, or CallerIDName ?!!!\n";
        return 3;
    }
}
...
"|$affected_rows vicidial_dtmf inserted|$HRnow_date|$s_hires|$usec|\n";
        ...
        $dtmf_string =
"$HRnow_date|$s_hires|$usec|$event_hash{'Channel'}|$event_hash{'Uniqueid'}|$event_hash{'Digit'}|$event_hash{'Direction'}|End|$event_hash{'CallerIDName'}";
        &dtmf_logger;
        return 1;
    }
    else {
        print STDERR
"DTMFEnd event does not have a Channel, Uniqueid, Direction, Digit, ConnectedLineName, or CallerIDName ?!!!\n";
        return 3;
    }
}
