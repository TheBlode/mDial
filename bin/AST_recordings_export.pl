...
if ( $ftp_transfer_only > 0 ) {
    ...
    foreach (@FILES) {
        if ( ( length( $FILES[$i] ) > 4 ) && ( !-d "$tempdir/$FILES[$i]" ) ) {
            ...
            if ( length($VARREPORT_dir) > 0 ) { $ftp->cwd("$VARREPORT_dir"); }
            if ( $NODATEDIR < 1 ) {
                if ( $YEARDIR > 0 ) {
                    $ftp->mkdir("$year");
                    $ftp->cwd("$year");
...
if ( $sale_statuses =~ /---ALL---/ ) {
    ...
    $archive_sale_statusesSQL =
      " and vicidial_log_archive.status IN($archive_sale_statusesSQL)";
    $archive_close_statusesSQL =
      " and vicidial_closer_log_archive.status IN($archive_close_statusesSQL)";
}
if ( length($with_inbound) < 2 ) {
    if ( length($NOTwith_inbound) < 2 ) { $with_inboundSQL = ''; }
        ...
        $archive_with_inboundSQL =
"vicidial_closer_log_archive.campaign_id NOT IN('$archive_with_inboundSQL')";
    }
}
else {
    ...
    if ( $did_only > 0 ) {
        ...
        $archive_with_inboundSQL =
          "recording_log_archive.user IN('$archive_with_inboundSQL')";
    }
    else {
        $with_inboundSQL =
          "vicidial_closer_log.campaign_id IN('$with_inboundSQL')";
        $archive_with_inboundSQL =
"vicidial_closer_log_archive.campaign_id IN('$archive_with_inboundSQL')";
    }
}
if ( length($campaignSQL) < 2 ) {
    if ( length($NOTcampaignSQL) < 2 ) {
        ...
        $archive_campaignSQL =
          "vicidial_log_archive.campaign_id NOT IN($NOTcampaignSQL)";
    }
}
else {
    ...
    $archive_campaignSQL =
      "vicidial_log_archive.campaign_id IN($archive_campaignSQL)";
}
if ( !$Q ) {
    print "\n\n\n\n\n\n\n\n\n\n\n\n-- AST_recordings_export.pl --\n\n";
    print
"This program is designed to gather recordings from campaigns and in-groups and post them to an FTP site. \n";
    print "\n";
...
if ( $did_only > 0 ) {
    $stmtA =
"SELECT lead_id,user,vicidial_id,vicidial_id,length_in_sec,UNIX_TIMESTAMP(start_time) from recording_log where $with_inboundSQL and start_time >= '$shipdate 00:00:00' and start_time <= '$shipdate_end 23:59:59' order by start_time;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
    ...
    if ( $archive_logs > 0 ) {
        $stmtA =
"SELECT lead_id,user,vicidial_id,vicidial_id,length_in_sec,UNIX_TIMESTAMP(start_time) from recording_log_archive where $archive_with_inboundSQL and start_time >= '$shipdate 00:00:00' and start_time <= '$shipdate_end 23:59:59' order by start_time;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        if ($DB) { print "$sthArows|$stmtA|\n"; }
    ...
    $stmtA =
"SELECT vicidial_list.lead_id,uniqueid,length_in_sec,UNIX_TIMESTAMP(vicidial_log.call_date) from vicidial_list,vicidial_log where $campaignSQL $sale_statusesSQL and call_date >= '$shipdate 00:00:00' and call_date <= '$shipdate_end 23:59:59' and vicidial_log.lead_id=vicidial_list.lead_id order by call_date;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows  = $sthA->rows;
    $rec_count = 0;
    ...
    && ( $email_post_audio > 0 ) )
{
    ...
    $mail{'content-type'} = "multipart/mixed; boundary=\"$boundary\"";
    $message              = encode_qp(
"Recording Export POST AUDIO TRANSFER:\n\n Total Records: $TOTAL_SALES\n Total Recordings: $TOTAL_RECORDINGS\n"
    );
    $boundary = '--' . $boundary;
    $mail{body} .= "$boundary\n";
    $mail{body} .= "Content-Type: text/plain; charset=\"iso-8859-1\"\n";
...
"Call is outside of defined call time: $CThourminute|$Cwday|   |$CTstart|$CTstop| \n";
        ...
        || ( ( $outbound_calltime_ignore > 0 ) && ( $outbound =~ /Y/ ) ) )
    {
        $str = '';
        $call_data =
          "|$outbound|$lead_id|$vicidial_id|$uniqueid|$length_in_sec|$epoch|";
        $ivr_id          = '0';
        $ivr_filename    = '';
        $recording_found = 0;
        $stmtB =
"SELECT recording_id,filename,location from recording_log where lead_id='$lead_id' and vicidial_id='$vicidial_id' and start_time >= '$shipdate 00:00:00' and start_time <= '$shipdate_end 23:59:59' order by start_time desc limit 100;";
...
`$wgetbin -q --output-document=$tempdir/$path_file $ivr_location `;
        ...
        if ( ( $archive_logs > 0 ) && ( $recording_found < 1 ) ) {
            $stmtB =
"SELECT recording_id,filename,location from recording_log_archive where lead_id='$lead_id' and vicidial_id='$vicidial_id' and start_time >= '$shipdate 00:00:00' and start_time <= '$shipdate_end 23:59:59' order by start_time desc limit 100;";
            $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
            $sthB->execute or die "executing: $stmtB ",          $dbhB->errstr;
            $sthBrows = $sthB->rows;
            if ($DBX) { print "$sthBrows|$stmtB|\n"; }
...
`$wgetbin -q --output-document=$tempdir/$path_file $ivr_location `;
        ...
        $Ealert .=
          "$TOTAL_SALES   $TOTAL_RECORDINGS   $rec_countB   $call_data\n";
        if ($DBX) {
            print
              "$TOTAL_SALES   $TOTAL_RECORDINGS   $rec_countB   $call_data\n";
        }
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /00$/i ) {
            print
"        |$rec_count|$TOTAL_SALES|$CALLTIME_KICK|$TOTAL_RECORDINGS|         |$lead_id|\n";
        }
    }
    $rec_count++;
}
