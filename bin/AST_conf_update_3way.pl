...
$stmtA =
"SELECT telnet_host,telnet_port,ASTmgrUSERNAME,ASTmgrSECRET,ASTmgrUSERNAMEupdate,ASTmgrUSERNAMElisten,ASTmgrUSERNAMEsend,max_vicidial_trunks,answer_transfer_agent,local_gmt,ext_context,vicidial_recording_limit,asterisk_version,vd_server_logs FROM servers where server_ip = '$server_ip';";
if ($DB) { print "|$stmtA|\n"; }
...
$event_string =
  "AST_conf_update_3way.pl script starting: $year-$mon-$mday $hour:$min:$sec";
&event_logger;
$loop_counter = 0;
...
"SELECT extension,conf_exten from vicidial_conferences where server_ip='$server_ip' and leave_3way='1';";
    ...
    while ( $sthArows > $rec_count ) {
        ...
        $event_string =
          "$PT_conf_extens[$rec_count]|$PTextensions[$rec_count]|";
        &event_logger;
        $rec_count++;
    }
    $sthA->finish();
...
"UPDATE vicidial_conferences set extension='$NEWexten[$i]',leave_3way='$leave_3waySQL' where server_ip='$server_ip' and conf_exten='$PT_conf_extens[$i]';";
        ...
        if ($DBX) {
            ...
            print <FILE>;
            close(FILE);
            unlink($telnet_log_file)
              or die "Can't delete $telnet_log_file: $!\n";
        }
    }
    sleep($CLIdelay);
    $loop_counter++;
