...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--end-date=YYYY-MM-DD] = For more than one day, use start-date as the start\n";
        print
"  [--ingroups=XXX-YYY] = In-groups that sales will be pulled from. for all use ---ALL---. REQUIRED\n";
        print
"  [--campaigns=XXX-YYY] = campaigns that sales will be pulled from. for all use ---ALL---. REQUIRED\n";
        print
"  [--statusess=XXX-YYY] = statuses that will be pulled. for all use ---ALL---. REQUIRED\n";
        print
...
if ( !$Q ) {
    ...
    print
"This script is designed to gather selected status lead data from call logs and  dump into a tab-delimited text file. \n";
    print "\n";
...
$SKIP_LEAD   = 0;
$header_data =
"LOG_lead_id\tLOG_call_date\tLOG_length_in_sec\tLOG_phone_number\tLOG_status\tLOG_user\tLOG_campaign_id\tLOG_list_id\tentry_date\tuser\tvendor_lead_code\tsource_id\tlist_id\tgmt_offset_now\ttitle\tfirst_name\tmiddle_initial\tlast_name\taddress1\taddress2\taddress3\tcity\tstate\tprovince\tpostal_code\tcountry_code\tgender\tdate_of_birth\talt_phone\temail\tsecurity_phrase\tcomments\tcalled_count\tlast_local_call_time\trank\towner\tentry_list_id\tcustom_fields_data\t";
...
$stmtA =
"SELECT lead_id,call_date,length_in_sec,phone_number,status,user,campaign_id,list_id from vicidial_closer_log where call_date >= \"$shipdate 00:00:00\" and call_date <= \"$shipdate_end 23:59:59\" $ingroupsSQL $statusesSQL;";
...
if ( $use_archives > 0 ) {
    ...
    $stmtA =
"SELECT lead_id,call_date,length_in_sec,phone_number,status,user,campaign_id,list_id from vicidial_closer_log_archive where call_date >= \"$shipdate 00:00:00\" and call_date <= \"$shipdate_end 23:59:59\" $ingroupsSQL $statusesSQL;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows  = $sthA->rows;
    $rec_count = 0;
...
$stmtA =
"SELECT lead_id,call_date,length_in_sec,phone_number,status,user,campaign_id,list_id from vicidial_log where call_date >= \"$shipdate 00:00:00\" and call_date <= \"$shipdate_end 23:59:59\" $campaignsSQL $statusesSQL;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArows  = $sthA->rows;
$rec_count = 0;
...
if ( $use_archives > 0 ) {
    ...
    $stmtA =
"SELECT lead_id,call_date,length_in_sec,phone_number,status,user,campaign_id,list_id from vicidial_log_archive where call_date >= \"$shipdate 00:00:00\" and call_date <= \"$shipdate_end 23:59:59\" $campaignsSQL $statusesSQL;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows  = $sthA->rows;
    $rec_count = 0;
...
if ( !$Q ) {
    print
"SALES EXPORT FOR $ingroups   ($PATHhome/LEAD_SELECT_EXPORT_$file_date$US$ingroups.txt)\n";
}
...
sub select_format_loop {
    ...
    $entry_list_id        = '';
    $stmtB =
"SELECT entry_date,user,vendor_lead_code,source_id,list_id,gmt_offset_now,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,entry_list_id from vicidial_list where lead_id=$L_lead_id;";
    ...
    $sthB->finish();
    $call_data =
"$L_lead_id\t$L_call_date\t$L_length_in_sec\t$L_phone_number\t$L_status\t$L_user\t$L_campaign_id\t$L_list_id\t$entry_date\t$user\t$vendor_lead_code\t$source_id\t$list_id\t$gmt_offset_now\t$title\t$first_name\t$middle_initial\t$last_name\t$address1\t$address2\t$address3\t$city\t$state\t$province\t$postal_code\t$country_code\t$gender\t$date_of_birth\t$alt_phone\t$email\t$security_phrase\t$comments\t$called_count\t$last_local_call_time\t$rank\t$owner\t$entry_list_id\t";
    $custom_data    = '';
    $custom_columns = '';
    if ( ( length($entry_list_id) > 1 ) && ( $entry_list_id >= 99 ) ) {
        ...
        if ( $sthBcustrows > 0 ) {
            ...
            while ( $sthBcolrows > $col_ct ) {
                ...
                if ( $col_ct > 0 ) { $custom_columns .= "$column_label,"; }
                $col_ct++;
            }
            $sthB->finish();
            $custom_columns =~ s/,$//gi;
            if ( $col_ct > 0 ) {
                $stmtB =
"SELECT $custom_columns from custom_$entry_list_id where lead_id=$L_lead_id;";
                ...
                while ( $sthBrows > $rec_countB ) {
                    ...
                    while ( $col_ct > $field_ct ) {
                        $custom_data .=
                          "\t$columns[$field_ct]: $aryB[$field_ct]";
                        $field_ct++;
                    }
                    $rec_countB++;
                    if ( length($custom_data) > 0 ) {
                        if ($DBX) {
                            print
"Custom data for $L_lead_id found in $entry_list_id: |$custom_data|\n";
                        }
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print "        |$rec_count|$TOTAL_LEADS|         |$L_lead_id|\n";
        }
    }
}
exit;
