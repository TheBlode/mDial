...
"  [--begin-date=YYYY-MM-DD] = begin date override, can also use '6days', '7days', '8days' and '30days', if not filled in will only use the --date\n";
        ...
        print
          "  [--ftp-port=XXX] = SFTP port number, optional, default is 22\n";
        print "  [--ftp-port=XXXXXXXX] = SFTP port\n";
        print
"  [--ftp-dir=XXXXXXXX] = remote SFTP server directory to post files to\n";
        print "\n";
        exit;
    }
    else {
...
if ( !$Q ) {
    ...
    print
      "This program is designed to wget a web report and email or SFTP it. \n";
    print "\n";
}
if ( length($report_id) > 1 ) {
    $stmtA =
...
if ( length($VARREPORT_host) > 7 ) {
    ...
    $sftp = Net::SFTP::Foreign->new(
        $VARREPORT_user . '@' . $VARREPORT_host,
        password => "$VARREPORT_pass",
        port     => "$VARREPORT_port",
        more     => "$sftp_more"
    );
    if ( $DBX > 0 ) {
        print "Sending /tmp/$HTMLfile to remote $VARREPORT_dir/$HTMLfile \n";
        $sftp->put( "/tmp/$HTMLfile", "$VARREPORT_dir/$HTMLfile",
...
sub writecallback {
    ...
    if ( $DB > 0 ) { print STDERR "Wrote $offset / $size bytes\r"; }
}

sub readcallback {
    my ( $sftp, $data, $offset, $size ) = @_;
    if ( $DB > 0 ) { print STDERR "Read $offset / $size bytes\r"; }
}
