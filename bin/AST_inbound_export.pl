...
"  [--end-date=YYYY-MM-DD] = For more than one day, use start-date as the start\n";
        print
"  [--ingroups=XXX-YYY] = In-groups that sales will be pulled from. REQUIRED\n";
        print
...
if ( !$Q ) {
    ...
    print
"This program is designed to gather ingroup log records including vicidial_list fields and export them to a data file. \n";
    print "\n";
...
$SKIP_LEAD   = 0;
$stmtA =
"SELECT lead_id,call_date,length_in_sec,phone_number,status,user,campaign_id from $vicidial_closer_log where call_date >= \"$shipdate 00:00:00\" and call_date <= \"$shipdate_end 23:59:59\" $ingroupsSQL;";
...
if ( !$Q ) {
    print
      "SALES EXPORT FOR $ingroups   ($PATHhome/$file_date$US$ingroups.txt)\n";
}
...
sub select_format_loop {
    ...
    $sthB->finish();
    $stmtB =
"SELECT vendor_lead_code,list_id,first_name,last_name,address1,address2,address3,city,state,country_code,security_phrase from vicidial_list where lead_id=$lead_id;";
    $sthB = $dbhB->prepare($stmtB) or die "preparing: ",        $dbhB->errstr;
    $sthB->execute                 or die "executing: $stmtB ", $dbhB->errstr;
    $sthBrows = $sthB->rows;
    if ($DBX) { print "$sthBrows|$stmtB|\n"; }
    ...
    $sthB->finish();
    $call_data =
"$call_date\t$phone_number\t$status\t$user\t$full_name\t$campaign_id\t$vendor_lead_code\t$list_id\t$first_name\t$last_name\t$address1\t$address2\t$address3\t$city\t$state\t$country_code\t$security_phrase\t$length_in_sec";
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print "        |$rec_count|$TOTAL_LEADS|         |$lead_id|\n";
        }
    }
}
exit;
