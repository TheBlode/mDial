...
while ( $sthArowsT > $k ) {
    ...
    if ( $aryA[0] =~ /archive/i ) { $ARCHIVEcount++; }
    else {
...
while ( $TABLEcount > $k ) {
    ...
    if ($DB) {
        print "($check_time seconds) |", $aryA[0], "|", $aryA[1], "|",
          $aryA[2], "|", $aryA[3], "|", "\n";
    }
    if ( $aryA[3] =~ /corrupt/ ) {
    ...
    $check_output .= "$check_table[$k]: ($check_time seconds) |", $aryA[0],
      "|", $aryA[1], "|", $aryA[2], "|", $aryA[3], "|", "<br>\n";
    $k++;
}
