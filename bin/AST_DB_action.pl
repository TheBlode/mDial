...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--list-id=XXX] = list that you can use in SQL, double-dash-delimited if more than one: 111--222\n";
        print "\n";
        exit;
    }
    else {
...
$affected_rows = 0;
$stmtA =
"UPDATE vicidial_list set called_since_last_reset='N',rank=90 where list_id IN('$list_idIN') and called_count < 1;";
if ($DB) { print STDERR "\n|$stmtA|\n"; }
...
else { $affected_rows = 'TEST'; }
$end_time = time();
$SQLlog   = $stmtA;
$SQLlog =~ s/\'|\"|\\|\///gi;
$stmtA =
"INSERT INTO vicidial_admin_log set event_date=NOW(), user='VDAD', ip_address='1.1.1.1', event_section='SERVERS', event_type='OTHER', record_id='$server_ip', event_code='DB_ACTION', event_sql='$SQLlog', event_notes='Affected rows: $affected_rows   TOTAL Elapsed time: "
  . ( $end_time - $start_time )
  . " sec  |$args|';";
