...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--list=XXX] = Campaign that sales will be pulled from. REQUIRED\n";
        print
...
if ( length($listSQL) > 1 ) { $listSQL = "list_id IN($listSQL)"; }
else {
    print "no list defined, exiting...";
    exit;
}
if ( !$Q ) {
    print "\n\n\n\n\n\n\n\n\n\n\n\n-- AST_list_export.pl --\n\n";
    print
"This program is designed to gather list records including custom fields and export them to a data file. \n";
    print "\n";
...
$secZm = ( $secZ / 60 );
if ( !$Q ) {
    print "SALES EXPORT FOR $list   ($PATHhome/$file_date$US$list.txt)\n";
}
...
"$lead_id\t$entry_date\t$modify_date\t$status\t$user\t$vendor_lead_code\t$source_id\t$list_id\t$gmt_offset_now\t$called_since_last_reset\t$phone_code\t$phone_number\t$title\t$first_name\t$middle_initial\t$last_name\t$address1\t$address2\t$address3\t$city\t$state\t$province\t$postal_code\t$country_code\t$gender\t$date_of_birth\t$alt_phone\t$email\t$security_phrase\t$comments\t$called_count\t$last_local_call_time\t$rank\t$owner\t$entry_list_id";
    ...
    if ( ( length($entry_list_id) > 1 ) && ( $entry_list_id >= 99 ) ) {
        ...
        if ( $sthBcustrows > 0 ) {
            ...
            if ( $col_ct > 0 ) {
                $stmtB =
"SELECT $custom_columns from custom_$entry_list_id where lead_id=$lead_id;";
                ...
                while ( $sthBrows > $rec_countB ) {
                    ...
                    if ( length($custom_data) > 0 ) {
                        if ($DBX) {
                            print
"Custom data for $lead_id found in $entry_list_id: |$custom_data|\n";
                        }
    ...
    if ($DBX) {
        print "$TOTAL_LEADS   $TOTAL_CUSTOM   $rec_countB   $call_data\n";
    }
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print
"        |$rec_count|$TOTAL_LEADS|$TOTAL_CUSTOM|         |$lead_id|\n";
        }
    }
    $rec_count++;
}
