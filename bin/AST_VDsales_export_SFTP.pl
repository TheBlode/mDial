...
"select vicidial_closer_log.user,8,8,8,8,8,8,8,vicidial_closer_log.phone_number,8,8,8,CONVERT_TZ(call_date,$convert_tz),vicidial_closer_log.lead_id,vicidial_users.full_name,vicidial_closer_log.status,8,8,vicidial_closer_log.list_id,campaign_id,8,8,CONVERT_TZ(call_date,$convert_tz),xfercallid,closecallid,uniqueid,length_in_sec,queue_seconds,vicidial_closer_log.list_id,vicidial_closer_log.list_id,UNIX_TIMESTAMP(vicidial_closer_log.call_date),agent_alert_delay from vicidial_closer_log,vicidial_users,vicidial_inbound_groups where $with_inboundSQL $close_statusesSQL and call_date > '$shipdate 00:00:01' and call_date < '$shipdate 23:59:59' and vicidial_users.user=vicidial_closer_log.user and vicidial_inbound_groups.group_id=vicidial_closer_log.campaign_id order by call_date;";
    ...
    while ( $sthArows > $rec_count ) {
        ...
        $agent_name           = '';
        $stmtB =
"select vicidial_xfer_log.user,full_name from vicidial_xfer_log,vicidial_users where lead_id='$lead_id' and closer='$closer' and xfercallid='$xfercallid' and call_date > '$shipdate 00:00:01' and call_date < '$shipdate 23:59:59' and vicidial_users.user=vicidial_xfer_log.user order by call_date desc limit 1;";
...
if ( $ftp_transfer > 0 ) {
    if ( $sftp_mode > 0 ) {
        ...
        my $sftp = Net::SFTP->new(
            ...
            ssh_args => [ port => "$VARREPORT_port", debug => 1 ]
        );
        if ( $DBX > 0 ) {
            print
"put $PATHweb/vicidial/server_reports/$outfile $VARREPORT_dir/$outfile\n";
        }
        $sftp_status = $sftp->put( "$PATHweb/vicidial/server_reports/$outfile",
            "$VARREPORT_dir/$outfile", \&writecallback );
        if ( $DB > 0 ) { print "\n$sftp_status\n"; }
    }
    else {
        use Net::FTP;
        ...
        if ( ( length( $FILES[$i] ) > 4 ) && ( !-d "$tempdir/$FILES[$i]" ) ) {
            if ( $sftp_mode > 0 ) {
                ...
                if ( $NODATEDIR < 1 ) {
                    if ( $YEARDIR > 0 ) {
                        if ( $DB > 0 ) {
                            print "making directory: $VARREPORT_dir/$year\n";
                        }
                    ...
                    if ( $DB > 0 ) {
                        print "making directory: $newVARREPORT_dir\n";
                    }
                    $sftp->do_mkdir( "$newVARREPORT_dir", $attrs );
                }
                if ( $DBX > 0 ) {
                    print
                      "put $tempdir/$FILES[$i] $newVARREPORT_dir/$FILES[$i]\n";
                }
                $sftp_status =
                  $sftp->put( "$tempdir/$FILES[$i]",
                    "$newVARREPORT_dir/$FILES[$i]",
                    \&writecallback );
                if ( $DB > 0 ) { print "\n$sftp_status\n"; }
                if ( $sftp_status > 0 ) {
                    if ( $DB > 0 ) {
                        print "Removing temp file: $tempdir/$FILES[$i]\n";
                    }
...
"Call is outside of defined call time: $CThourminute|$Cwday|   |$CTstart|$CTstop| \n";
        ...
        || ( ( $outbound_calltime_ignore > 0 ) && ( $outbound =~ /Y/ ) ) )
    {
        ...
        if ( $with_transfer_audio < 1 ) {
            ...
            $ivr_filename = '';
            $stmtB =
"select recording_id,filename,location from recording_log where lead_id='$lead_id' and vicidial_id='$vicidial_id' and start_time > '$shipdate 00:00:01' and start_time < '$shipdate 23:59:59' order by start_time desc limit 1;";
            $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
            $sthB->execute or die "executing: $stmtB ",          $dbhB->errstr;
            $sthBrows   = $sthB->rows;
            $rec_countB = 0;
            ...
            if ( $sthBrows < 1 ) {
                $stmtB =
"select recording_id,filename,location from recording_log where lead_id='$lead_id' and start_time > '$shipdate 00:00:01' and start_time < '$shipdate 23:59:59' order by length_in_sec desc limit 1;";
...
`$wgetbin -q --output-document=$tempdir/$path_file $ivr_location `;
            ...
            $ivr_location = '';
            $stmtB =
"select recording_id,filename,location from recording_log where vicidial_id='$vicidial_id' and start_time > '$shipdate 00:00:01' and start_time < '$shipdate 23:59:59' order by start_time limit 10;";
            $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
            $sthB->execute or die "executing: $stmtB ",          $dbhB->errstr;
            $sthBrows = $sthB->rows;
            if ($DBX) { print "   $sthBrows|$stmtB|\n"; }
...
"select closecallid,length_in_sec,queue_seconds,agent_alert_delay from vicidial_closer_log,vicidial_inbound_groups where lead_id='$lead_id' and call_date > '$call_date' and call_date < '$shipdate 23:59:59' and campaign_id=group_id order by call_date limit 10;";
            ...
            while ( $sthBrows > $u ) {
                ...
                $stmtB =
"select recording_id,filename,location from recording_log where vicidial_id='$closecallid' and start_time > '$shipdate 00:00:01' and start_time < '$shipdate 23:59:59' order by start_time limit 10;";
                $sthB = $dbhB->prepare($stmtB)
                  or die "preparing: ", $dbhB->errstr;
                $sthB->execute or die "executing: $stmtB ", $dbhB->errstr;
                $sthBrowsR = $sthB->rows;
...
`$wgetbin -q --output-document=$tempdir/$path_file $aryB[2] `;
        ...
        if ( ( $with_did_lookup > 0 ) && ( $outbound =~ /N/ ) ) {
            ...
            $did_date    = '';
            $stmtB =
"select did_pattern,did_description,CONVERT_TZ(call_date,$convert_tz) from vicidial_inbound_dids vid,vicidial_did_log vdl where uniqueid='$uniqueid' and call_date > '$shipdate 00:00:01' and call_date <= '$call_date' and vid.did_id=vdl.did_id order by call_date desc limit 1;";
            ...
            if ( $sthBrows > 0 ) {
                ...
                $stmtB =
"select vc.campaign_cid,vc.campaign_name,CONVERT_TZ(call_date,$convert_tz) from vicidial_campaigns vc,vicidial_log vl where lead_id='$lead_id' and call_date > '$shipdate 00:00:01' and call_date <= '$call_date' and vc.campaign_id=vl.campaign_id order by call_date desc limit 1;";
                if ( $DBX > 0 ) { print "$stmtB\n"; }
                $sthB = $dbhB->prepare($stmtB)
                  or die "preparing: ", $dbhB->errstr;
                $sthB->execute or die "executing: $stmtB ", $dbhB->errstr;
...
"103$uniqueid\t$call_date_array[0]\t$call_date_array[1]\t$phone_areacode\t$phone_number\t$did_pattern\t$closer\t$queue_seconds\t$talk_seconds\t$dispo_time\t$application\t-\t$ivr_filename\t$outbound\t$domestic\t\n";
        ...
        if ( $output_format =~ /^tab-SCcustomUSA$/ ) {
            ...
            $call_notes            = '';
            $stmtB =
"select CONVERT_TZ(call_date,$convert_tz),order_id,appointment_date,appointment_time,call_notes from vicidial_call_notes where lead_id='$lead_id' and vicidial_id='$uniqueid' and call_date > '$shipdate 00:00:01' and call_date < '$shipdate 23:59:59' order by call_date desc limit 1;";
            $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
            $sthB->execute or die "executing: $stmtB ",          $dbhB->errstr;
            $sthBrows = $sthB->rows;
...
"$appointment_call_date_format[1]-$appointment_call_date_format[2]-$appointment_call_date_format[0]";
            ...
            $dispo_time = 0;
            $stmtB =
"select dispo_sec from vicidial_agent_log where lead_id='$lead_id' and user='$closer' and event_time > '$shipdate 00:00:01' and event_time < '$shipdate 23:59:59' order by event_time desc limit 1;";
            $sthB = $dbhB->prepare($stmtB) or die "preparing: ", $dbhB->errstr;
            $sthB->execute or die "executing: $stmtB ",          $dbhB->errstr;
            $sthBrows = $sthB->rows;
            if ($DB) { print "$sthBrows|$stmtB|\n"; }
...
"103$uniqueid\t$call_date_array[0]\t$call_date_array[1]\t$phone_areacode\t$phone_number\t$did_pattern\t$closer\t$queue_seconds\t$talk_seconds\t$dispo_time\t$application\t$status\t$order_id\t$ivr_filename\t$in_out\t$appointment_date $appointment_time\t$appointment_call_date\t$call_notes\t\n";
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /00$/i ) {
            print
"$rec_count|$TOTAL_SALES|$CALLTIME_KICK|         |$phone_number|\n";
        }
    }
    $rec_count++;
}
