...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--container=XXX] = required, the ID of the settings container to run from the system\n";
        print "\n";
        exit;
    }
    else {
    ...
    foreach (@container_lines) {
        ...
        if ( length( $container_lines[$i] ) > 5 ) {
            if ( $container_lines[$i] =~ /^INSERT|^UPDATE|^DELETE|^SELECT/i ) {
                ...
                if ( !$Q ) {
                    print
                      " - line $i executed, affected: $affected_rows rows\n";
                }
                $temp_lines = $container_lines[$i];
                $temp_lines =~ s/"/'/gi;
                $stmtB =
"INSERT INTO vicidial_admin_log set event_date=NOW(), user='VDAD', ip_address='1.1.1.1', event_section='CONTAINERS', event_type='OTHER', record_id='$container', event_code='CONTAINER SQL RUN', event_sql=\"$temp_lines\", event_notes='line $i executed, $affected_rows rows affected';";
                if ( !$T ) { $Iaffected_rows = $dbhA->do($stmtB); }
                if ($DBX) {
                    print
                      " - admin log insert debug: |$Iaffected_rows|$stmtB|\n";
                }
            }
            else {
                if ( $DBX > 0 ) {
                    print "     not allowed SQL: $i|$container_lines[$i]|\n";
                }
            }
        }
        else {
