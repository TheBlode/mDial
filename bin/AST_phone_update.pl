...
$stmtA =
"SELECT telnet_host,telnet_port,ASTmgrUSERNAME,ASTmgrSECRET,ASTmgrUSERNAMEupdate,ASTmgrUSERNAMElisten,ASTmgrUSERNAMEsend,max_vicidial_trunks,answer_transfer_agent,local_gmt,ext_context,asterisk_version FROM servers where server_ip = '$server_ip';";
...
if ( $agent_lookup > 0 ) {
    $stmtA =
"SELECT distinct server_phone from vicidial_user_log where server_ip='$server_ip' and phone_ip='LOOKUP' and event_date > \"$BDtsSQLdate\" and extension LIKE \"SIP%\";";
    if ($DB) { print "|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
...
if ( ( $sip_count > 0 ) || ( $agent_lookup < 1 ) ) {
    ...
    @PTlogins     = @MT;
    $stmtA =
"SELECT extension,phone_ip,login from phones where server_ip='$server_ip' and protocol='SIP' $phone_listSQL;";
    if ($DB) { print "|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows  = $sthA->rows;
...
"Action: Command\nCommand: sip show peer $PTextensions[$i]\n\nAction: Ping$command_end",
        ...
        $j = 0;
        foreach (@list_channels) {    #  Addr->IP     : 10.10.14.25 Port 5060
        ...
        if ($DB) {
            print
"Phone IP- $PTextensions[$i]    NEW:|$NEW_IPaddr[$i]|  OLD:|$PTphone_ips[$i]|    ";
        }
        if (   ( $NEW_IPaddr[$i] =~ /$PTphone_ips[$i]/ )
            && ( length( $NEW_IPaddr[$i] ) eq length( $PTphone_ips[$i] ) ) )
        {
            if ($DB) {
                print "PHONE IP UNCHANGED, DOING NOTHING FOR THIS EXTENSION\n";
            }
        }
        else {
            if ( $NEW_IPaddr[$i] =~ /Unspecified/ ) {
                if ($DB) {
                    print
"PHONE IP UNSPECIFIED, DOING NOTHING FOR THIS EXTENSION\n";
                }
            }
            else {
                $stmtA =
"UPDATE phones set phone_ip='$NEW_IPaddr[$i]' where server_ip='$server_ip' and extension='$PTextensions[$i]';";
        ...
        if ( $agent_lookup > 0 ) {
            $stmtA =
"UPDATE vicidial_user_log set phone_ip='$NEW_IPaddr[$i]' where server_phone='$PTlogins[$i]' and server_ip='$server_ip' and phone_ip='LOOKUP' and event_date > \"$BDtsSQLdate\" and extension LIKE \"SIP%\";";
            $affected_rows =
...
if ( $agent_lookup > 0 ) {
    $stmtA =
"SELECT distinct server_phone from vicidial_user_log where server_ip='$server_ip' and phone_ip='LOOKUP' and event_date > \"$BDtsSQLdate\" and extension LIKE \"IAX%\";";
    if ($DB) { print "|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
...
if ( ( $iax_count > 0 ) || ( $agent_lookup < 1 ) ) {
    ...
    @PTlogins     = @MT;
    $stmtA =
"SELECT extension,phone_ip,login from phones where server_ip='$server_ip' and protocol='IAX2'  $phone_listSQL;";
    if ($DB) { print "|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows  = $sthA->rows;
...
"UPDATE phones set phone_ip='$NEW_IPaddr[$i]' where server_ip='$server_ip' and extension='$PTextensions[$i]';";
        ...
        if ( $agent_lookup > 0 ) {
            $stmtA =
"UPDATE vicidial_user_log set phone_ip='$NEW_IPaddr[$i]' where server_phone='$PTlogins[$i]' and server_ip='$server_ip' and phone_ip='LOOKUP' and event_date > \"$BDtsSQLdate\" and extension LIKE \"IAX%\";";
            $affected_rows =
              $dbhA->do($stmtA); #  or die  "Couldn't execute query:|$stmtA|\n";
            if ($DB) { print STDERR "\n|$affected_rows|$stmtA|\n"; }
        }
