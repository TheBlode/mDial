...
"  [--force-download] = force download of everything from audio store\n";
        print
"  [--force-upload] = force upload of all local audio files to the audio store\n";
        print
"  [--settings-override] = ignore database settings and run sync anyway\n";
        print
"  [--force-moh-rebuild] = ignore database settings and rebuild Music On Hold\n";
        print
"  [--gather-details] = gathers audio file details and populates audio_store_details table\n";
        print "\n";
        exit;
    }
    else {
...
$stmtA =
"SELECT active_asterisk_server,audio_store_purge FROM servers where server_ip = '$VARserver_ip';";
...
$web_prefix = '';
$stmtA =
"SELECT sounds_central_control_active,sounds_web_server,sounds_web_directory,admin_web_directory FROM system_settings;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArows = $sthA->rows;
if ( $sthArows > 0 ) {
...
if (
    (
           ( $sounds_central_control_active < 1 )
...
$stmtA =
  "UPDATE servers SET sounds_update='N' where server_ip='$VARserver_ip';";
...
if ( length($audio_store_purge) > 0 ) {
    ...
    $stmtA =
"UPDATE servers SET audio_store_purge='' where server_ip='$VARserver_ip';";
    $affected_rows = $dbhA->do($stmtA);
    @purge_data    = split( /\n/, $audio_store_purge );
    $i             = 0;
    foreach (@purge_data) {
        if ( length( $purge_data[$i] ) > 0 ) {
            if ( -e ("/var/lib/asterisk/sounds/$purge_data[$i]$wav") ) {
                `rm -f /var/lib/asterisk/sounds/$purge_data[$i]$wav`;
                $audio_file_deleted++;
            }
            if ( -e ("/var/lib/asterisk/sounds/$purge_data[$i]$gsm") ) {
                `rm -f /var/lib/asterisk/sounds/$purge_data[$i]$gsm`;
                $audio_file_deleted++;
            }
            if ( -e ("/var/lib/asterisk/sounds/$purge_data[$i]$ulaw") ) {
                `rm -f /var/lib/asterisk/sounds/$purge_data[$i]$ulaw`;
                $audio_file_deleted++;
            }
            if ( $audio_file_deleted < 1 ) {
                if ($DB) {
                    print "no audio file deleted: $purge_data[$i]|$i\n";
                }
            }
            if ( $DBX > 0 ) {
                print
"audio file delete process: $purge_data[$i]|$i|$audio_file_deleted\n";
            }
        }
        $i++;
    }
...
$URL =
"$sounds_web_server_URL_prefix/$admin_web_directory/audio_store.php?action=LIST&audio_server_ip=$VARserver_ip";
$URL =~ s/&/\\&/gi;
if ($DB) { print "\n$URL\n"; }
...
while ( $i <= $#list ) {
    ...
    if ( $DB > 0 ) {
        print "$i   $filename     $filedate     $filesize     $fileepoch\n";
    }
    $k          = 0;
    $found_file = 0;
    while ( $k <= $#sounds ) {
...
if ( $upload > 0 ) {
    ...
    while ( $k <= $#sounds ) {
        ...
        if ( $sounds[$k] =~ /\.wav$|\.gsm$/ ) {
            ...
            if ( $DB > 0 ) {
                print
"$k   $soundname     $sounddate     $soundsize     $soundepoch\n";
            }
            $i          = 0;
            $found_file = 0;
            while ( $i <= $#list ) {
                ...
                $filesize  = $file_data[3];
                $fileepoch = $file_data[4];
                if (   ( $filename eq "$soundname" )
                    && ( $filesize eq "$soundsize" ) )
                {
                    $found_file++;
                }
                $i++;
            ...
            if ( ( $found_file < 1 ) || ( $force_upload > 0 ) ) {
                $curloptions =
"-s '$sounds_web_server_URL_prefix/$admin_web_directory/audio_store.php?action=AUTOUPLOAD&audio_server_ip=$VARserver_ip' -F \"audiofile=\@$PATHsounds/$soundname\"";
                `$curlbin $curloptions`;
...
if ( $uploaded > 0 ) {
    $stmtA =
"UPDATE servers SET sounds_update='Y' where server_ip NOT IN('$VARserver_ip');";
    $affected_rows = $dbhA->do($stmtA);
}
if ( $gather_details > 0 ) {
    $gsm_count    = 0;
    ...
    while ( $i <= $#list ) {
        ...
        if ( $filesize > 0 ) {
            ...
            if ( $allowed_format > 0 ) {
                $stmtA =
"SELECT audio_format,audio_filesize,audio_length FROM audio_store_details where audio_filename = '$filename';";
                ...
                if ( $sthArows > 0 ) {
                    ...
                    $asd_filesize = $aryA[1];
                    $asd_length   = $aryA[2];
                    if (   ( $asd_format eq "$audio_format" )
                        && ( $asd_filesize eq "$filesize" )
                        && ( $asd_length eq "$audio_length" ) )
                    {
                        $old_count++;
                        if ( $DB > 0 ) {
                            print
"$i   audio file details unchanged: $filename - |$audio_format|$filesize|$audio_length|\n";
                        }
                    }
                    else {
                        $update_count++;
                        $stmtA =
"UPDATE audio_store_details SET audio_format='$audio_format',audio_filesize='$filesize',audio_length='$audio_length',audio_epoch='$fileepoch' where audio_filename='$filename';";
                        $affected_rows = $dbhA->do($stmtA);
                        if ( $DBX > 0 ) {
                    ...
                    $new_count++;
                    $stmtA =
"INSERT INTO audio_store_details SET audio_filename='$filename',audio_format='$audio_format',audio_filesize='$filesize',audio_length='$audio_length',audio_epoch='$fileepoch';";
                    $affected_rows = $dbhA->do($stmtA);
                ...
                if ( $DB > 0 ) {
                    print "$i   format is not allowed: $filename\n";
                }
            }
        }
        else {
            if ( $DB > 0 ) {
                print "$i   filesize is zero: $filename ($filesize)\n";
            }
        }
        $i++;
    }
...
$gsm = '.gsm';
$stmtA =
"SELECT rebuild_music_on_hold FROM servers where server_ip = '$VARserver_ip';";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArows = $sthA->rows;
if ( $sthArows > 0 ) {
...
$sthA->finish();
if (   ( $force_moh_rebuild > 0 )
    || ( $new_file_moh_rebuild > 0 )
    || ( $rebuild_music_on_hold =~ /Y/i ) )
{
    $stmtA =
"UPDATE servers SET rebuild_music_on_hold='N' where server_ip = '$VARserver_ip';";
    $affected_rows = $dbhA->do($stmtA);
    $stmtA =
"SELECT moh_id FROM vicidial_music_on_hold where remove='Y' and moh_id NOT IN('astdb','sounds','agi-bin','keys');";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArowsM     = $sthA->rows;
    @remove_moh_id = @MT;
    ...
    while ( $sthArowsM > $j ) {
        $stmtA =
"DELETE from vicidial_music_on_hold where moh_id='$remove_moh_id[$j]' and remove='Y' and moh_id NOT IN('astdb','sounds','agi-bin','keys');";
        $affected_rowsX = $dbhA->do($stmtA);
        $stmtB =
"DELETE from vicidial_music_on_hold_files where moh_id='$remove_moh_id[$j]' and moh_id NOT IN('astdb','sounds','agi-bin','keys');";
        ...
        if ($DBX) {
            print
"Deleting Existing MoH: $affected_rowsX|$affected_rowsY|$stmtA|$stmtB|rm -fr /var/lib/asterisk/$remove_moh_id[$j]\n";
        }
        $j++;
    }
    $stmtA =
"SELECT moh_id,moh_name,random FROM vicidial_music_on_hold where active='Y' and moh_id NOT IN('astdb','sounds','agi-bin','keys');";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArowsC     = $sthA->rows;
    @remove_moh_id = @MT;
...
`cp $PATHsounds/sip-silence.gsm $MoH_directory/0000_sip-silence.gsm`;
        ...
        $stmtA =
"SELECT filename,rank FROM vicidial_music_on_hold_files where moh_id='$moh_id[$j]' order by rank;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArowsF     = $sthA->rows;
        @remove_moh_id = @MT;
        ...
        while ( $sthArowsF > $m ) {
            ...
            if (   ( -e "$MoH_directory/$filerank[$m]$filename[$m]$wav" )
                || ( -e "$MoH_directory/$filerank[$m]$filename[$m]$gsm" ) )
            {
                if ( -e "$MoH_directory/$filerank[$m]$filename[$m]$wav" ) {
                    $temp_filesize_moh =
                      ( -s "$MoH_directory/$filerank[$m]$filename[$m]$wav" );
                    $temp_filesize_original =
                      ( -s "$PATHsounds/$filename[$m]$wav" );
                    $filesize_moh[$m] =
                      ( $filesize_moh[$m] + $temp_filesize_moh );
                    $filesize_original[$m] =
                      ( $filesize_original[$m] + $temp_filesize_original );
                    if ($DBX) {
                        print "WAV check existing:\n";
                        print
"$temp_filesize_moh|$filesize_moh[$m]|$MoH_directory/$filerank[$m]$filename[$m]$wav\n";
                        print
"$temp_filesize_original|$filesize_original[$m]|$PATHsounds/$filename[$m]$wav\n";
                    }
                }
                if ( -e "$MoH_directory/$filerank[$m]$filename[$m]$gsm" ) {
                    $temp_filesize_moh =
                      ( -s "$MoH_directory/$filerank[$m]$filename[$m]$gsm" );
                    $temp_filesize_original =
                      ( -s "$PATHsounds/$filename[$m]$gsm" );
                    $filesize_moh[$m] =
                      ( $filesize_moh[$m] + $temp_filesize_moh );
                    $filesize_original[$m] =
                      ( $filesize_original[$m] + $temp_filesize_original );
                    if ($DBX) {
                        ...
                        print
"$temp_filesize_moh|$filesize_moh[$m]|$MoH_directory/$filerank[$m]$filename[$m]$gsm\n";
                        print
"$temp_filesize_original|$filesize_original[$m]|$PATHsounds/$filename[$m]$gsm\n";
                    }
                }
                if (   ( $filesize_moh[$m] == $filesize_original[$m] )
                    && ( $filesize_moh[$m] > 0 ) )
                {
                    $fileexists[$m]++;
                }
...
`cp $PATHsounds/$sounds[$d] $MoH_directory/$filerank[$m]$sounds[$d]`;
                        ...
                        if ($DBX) {
                            print
"Copy new file: $PATHsounds/$sounds[$d] $MoH_directory/$filerank[$m]$sounds[$d]\n";
                        }
                    }
                    $d++;
                }
        ...
        foreach (@MoH_files) {
            ...
            if ($DBX) {
                print
"Checking file: $MoH_files[$d] $MoH_files_check[$d] $filelist_names\n";
            }
            if (   ( length( $MoH_files[$d] ) > 4 )
                && ( -f "$MoH_directory/$MoH_files[$d]" )
                && ( $filelist_names !~ /\|$MoH_files_check[$d]\|/ )
                && ( $MoH_files_check[$d] !~ /^0000_/ ) )
            {
                `rm -f $MoH_directory/$MoH_files[$d]`;
                if ($DBX) {
                    print
"Deleting file: $MoH_directory/$MoH_files[$d] $filelist_names\n";
                }
            }
            $d++;
        }
    ...
    if ($DBX) { print "reloading moh in asterisk\n"; }
