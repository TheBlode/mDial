...
(
    $TMsec,  $TMmin,  $TMhour, $TMmday, $TMmon,
...
print
"This program is designed to print sales-per-hour(SPH) to a file for agents' activity for the current week. \n\n";
...
print Dout
"DATE|AGENT|USER|MON HOURS|MON SPH|TUE HOURS|TUE SPH|WED HOURS|WED SPH|THU HOURS|THU SPH|FRI HOURS|FRI SPH|SAT HOURS|SAT SPH|SUN HOURS|SUN SPH|WEEK HOURS|WEEK SPH|MONTH HOURS|MONTH SPH$YEARheadPIPE|GROUP|\n";
$outline = "REPORT: SPH $Tyear-$Tmon-$Tmday\n\n";
$outline .=
"                               MONDAY      TUESDAY     WEDNESDAY   THURSDAY    FRIDAY      SATURDAY    SUNDAY       WEEK         MONTH$YEARhHEAD\n";
$outline .=
"AGENT                USER       HRS  SPH    HRS  SPH    HRS  SPH    HRS  SPH    HRS  SPH    HRS  SPH    HRS  SPH     HRS  SPH     HRS  SPH $YEARheadFIXED GROUP   \n";
print "$outline";
print out "$outline";
$stmtA =
"select full_name,user,user_group from vicidial_users order by user_group,full_name limit 1000;";
...
while ( $sthArows > $rec_count ) {
    ...
    $name[$rec_count]   = sprintf( "%-20s", $aryA[0] );
    while ( length( $name[$rec_count] ) > 20 ) { chop( $name[$rec_count] ); }
...
foreach (@Duser) {
    ...
    while ( $prevday >= 0 ) {
        ...
        $Dhours[$prevday] = 0;
        $stmtA =
"select sum(talk_sec+pause_sec+wait_sec+dispo_sec) from vicidial_agent_log where event_time <= '$enddate' and event_time >= '$begindate' and pause_sec<48800 and wait_sec<48800 and talk_sec<48800 and dispo_sec<48800 and user='$Duser[$i]' $SQLrestrict limit 1;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        ...
        $stmtA =
"select count(*) from vicidial_xfer_log vx,vicidial_closer_log vc where vx.call_date <= '$enddate' and vx.call_date >= '$begindate' and vc.status IN('A1','A2','A3','A4','SALE','UPSELL') and vx.user='$Duser[$i]' and vx.xfercallid=vc.xfercallid;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        if ( $sthArows > 0 ) {
        ...
        if ($DB) {
            print STDERR
"$hours[$prevday] $sales[$prevday] $sph[$prevday] $prevday $begindate\n";
        }
        $prevday = ( $prevday - 1 );
        $j++;
    }
    $stmtA =
"select sum(talk_sec+pause_sec+wait_sec+dispo_sec) from vicidial_agent_log where event_time <= '$endmonth' and event_time >= '$beginmonth' and pause_sec<48800 and wait_sec<48800 and talk_sec<48800 and dispo_sec<48800 and user='$Duser[$i]' $SQLrestrict limit 1;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
    if ( $sthArows > 0 ) {
    ...
    $stmtA =
"select count(*) from vicidial_xfer_log vx,vicidial_closer_log vc where vx.call_date <= '$endmonth' and vx.call_date >= '$beginmonth' and vc.status IN('A1','A2','A3','A4','SALE','UPSELL') and vx.user='$Duser[$i]' and vx.xfercallid=vc.xfercallid;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
    if ( $sthArows > 0 ) {
    ...
    if ( $YTDrun > 0 ) {
        ...
        $stmtA =
"select sum(talk_sec+pause_sec+wait_sec+dispo_sec) from vicidial_agent_log where event_time <= '$endmonth' and event_time >= '$TMyear$Ybegin' and pause_sec<48800 and wait_sec<48800 and talk_sec<48800 and dispo_sec<48800 and user='$Duser[$i]' $SQLrestrict limit 1;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        if ( $sthArows > 0 ) {
        ...
        $stmtA =
"select count(*) from vicidial_xfer_log vx,vicidial_closer_log vc where vx.call_date <= '$endmonth' and vx.call_date >= '$TMyear$Ybegin' and vc.status IN('A1','A2','A3','A4','SALE','UPSELL') and vx.user='$Duser[$i]' and vx.xfercallid=vc.xfercallid;";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        if ( $sthArows > 0 ) {
    ...
    print Dout
"$shipdate|$Dname[$i]|$Duser[$i]|$Dhours[0]|$Dsph[0]|$Dhours[1]|$Dsph[1]|$Dhours[2]|$Dsph[2]|$Dhours[3]|$Dsph[3]|$Dhours[4]|$Dsph[4]|$Dhours[5]|$Dsph[5]|$Dhours[6]|$Dsph[6]|$WEEKhours|$WEEKsph|$MONTHhours|$MONTHsph|$YEARpipe$Dgroup[$i]\n";
    if ( ( $WEEKhours > 0 ) || ( $MONTHhours > 0 ) ) {
        ...
        $outline =
"$name[$i] $user[$i] $hours[0] $sph[0]  $hours[1] $sph[1]  $hours[2] $sph[2]  $hours[3] $sph[3]  $hours[4] $sph[4]  $hours[5] $sph[5]  $hours[6] $sph[6]   $WEEKhours $WEEKsph   $MONTHhours $MONTHsph  $YEARfixed$group[$i]\n";
        print "$outline";
        print out "$outline";
    }
    else {
    ...
    if ( $Dgroup[$i] !~ /^$Dprevgroup$/ ) {
        ...
        while ( $prevday >= 0 ) {
            ...
            $Ghours[$prevday]  = sprintf( "%5s",  $GDhours[$prevday] );
            while ( length( $Ghours[$prevday] ) > 5 ) {
                chop( $Ghours[$prevday] );
            }
        ...
        if ( $YTDrun > 0 ) {
            ...
            $GYEARhours = sprintf( "%5s",  $GYEARhours );
            while ( length($GYEARhours) > 5 ) { chop($GYEARhours); }
        ...
        print Dout
"$shipdate|TOTAL $Dprevgroup||$GDhours[0]|$GDsph[0]|$GDhours[1]|$GDsph[1]|$GDhours[2]|$GDsph[2]|$GDhours[3]|$GDsph[3]|$GDhours[4]|$GDsph[4]|$GDhours[5]|$GDsph[5]|$GDhours[6]|$GDsph[6]|$GWEEKhours|$GWEEKsph|$GMONTHhours|$GMONTHsph|$YEARgpipe\n";
        $GWEEKhours = sprintf( "%.0f", $GWEEKhours );
        $GWEEKhours = sprintf( "%5s",  $GWEEKhours );
        while ( length($GWEEKhours) > 5 ) { chop($GWEEKhours); }
        $GWEEKsph    = sprintf( "%.1f", $GWEEKsph );
        $GWEEKsph    = sprintf( "%4s",  $GWEEKsph );
        $GMONTHhours = sprintf( "%.0f", $GMONTHhours );
        $GMONTHhours = sprintf( "%5s",  $GMONTHhours );
        while ( length($GMONTHhours) > 5 ) { chop($GMONTHhours); }
...
while ( $prevday >= 0 ) {
    ...
    $TOThours[$prevday]  = sprintf( "%5s",  $TOTDhours[$prevday] );
    while ( length( $TOThours[$prevday] ) > 5 ) { chop( $TOThours[$prevday] ); }
...
if ( $YTDrun > 0 ) {
    ...
    $TOTYEARhours = sprintf( "%5s",  $TOTYEARhours );
    while ( length($TOTYEARhours) > 5 ) { chop($TOTYEARhours); }
...
print Dout
"$shipdate|TOTAL||$TOTDhours[0]|$TOTDsph[0]|$TOTDhours[1]|$TOTDsph[1]|$TOTDhours[2]|$TOTDsph[2]|$TOTDhours[3]|$TOTDsph[3]|$TOTDhours[4]|$TOTDsph[4]|$TOTDhours[5]|$TOTDsph[5]|$TOTDhours[6]|$TOTDsph[6]|$TOTWEEKhours|$TOTWEEKsph|$TOTMONTHhours|$TOTMONTHsph|$YEARtpipe\n";
$TOTWEEKhours = sprintf( "%.0f", $TOTWEEKhours );
$TOTWEEKhours = sprintf( "%5s",  $TOTWEEKhours );
while ( length($TOTWEEKhours) > 5 ) { chop($TOTWEEKhours); }
$TOTWEEKsph    = sprintf( "%.1f", $TOTWEEKsph );
$TOTWEEKsph    = sprintf( "%4s",  $TOTWEEKsph );
$TOTMONTHhours = sprintf( "%.0f", $TOTMONTHhours );
$TOTMONTHhours = sprintf( "%5s",  $TOTMONTHhours );
while ( length($TOTMONTHhours) > 5 ) { chop($TOTMONTHhours); }
$TOTMONTHsph = sprintf( "%.1f", $TOTMONTHsph );
$TOTMONTHsph = sprintf( "%4s",  $TOTMONTHsph );
$outline =
"     TOTAL                    $TOThours[0] $TOTsph[0]  $TOThours[1] $TOTsph[1]  $TOThours[2] $TOTsph[2]  $TOThours[3] $TOTsph[3]  $TOThours[4] $TOTsph[4]  $TOThours[5] $TOTsph[5]  $TOThours[6] $TOTsph[6]   $TOTWEEKhours $TOTWEEKsph   $TOTMONTHhours $TOTMONTHsph$YEARtfixed\n";
print "$outline";
print out "$outline";
exit;
