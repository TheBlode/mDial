...
"  [--email-sender=vicidial@localhost] = sender for the email results\n";
        ...
        print "   tab-basic:\n";
        print
"   call_date  phone_number  vendor_id  status  user  first_name  last_name  lead_id  list_id  campaign_id  length_in_sec  etc...\n";
        print "   pipe-basic:\n";
        print
"   call_date|phone_number|vendor_id|status|user|first_name|last_name|lead_id|list_id|campaign_id|length_in_sec|source_id|etc...\n";
        print "   csv-basic:\n";
        print
"   call_date,phone_number,vendor_id,status,user,first_name,last_name,lead_id,list_id,campaign_id,length_in_sec,etc...\n";
        print "\n";
        ...
        if ( $args =~ /--campaign=/i ) {
            ...
            if ( $campaignSQL =~ /---ALL---/ ) { $campaignSQL = ''; }
            else {
                if ( $campaignSQL =~ /-/ ) {
                    $campaignSQL =~ s/-/','/gi;
                }
...
if ( $output_format =~ /^tab-basic$/ ) {
    $DLT = "\t";
    $txt = '.txt';
    if ( !$Q ) { print "---- tab-basic ----\n"; }
}
if ( $output_format =~ /^pipe-basic$/ ) {
    $DLT = "|";
    $txt = '.txt';
    if ( !$Q ) { print "---- pipe-basic ----\n"; }
}
if ( $output_format =~ /^csv-basic$/ ) {
    $DLT = ",";
    $txt = '.csv';
    if ( !$Q ) { print "---- ncad14csv ----\n"; }
}
if ( length($campaignSQL) < 2 ) {
    if ( length($NOTcampaignSQL) < 2 ) {
        ...
        $VALcampaignSQL =
          $vicidial_agent_log . ".campaign_id NOT IN($NOTcampaignSQL)";
    }
}
else {
...
if ( $dateset > 0 ) {
    $VALdateSQL =
"event_time >= '$shipdate 00:00:00' and event_time <= '$shipdate 23:59:59'";
    $RECdateSQL =
"start_time >= '$shipdate 00:00:00' and start_time <= '$shipdate 23:59:59'";
    $VLdateSQL =
"( (modify_date >= '$shipdate 00:00:00' and modify_date <= '$shipdate 23:59:59') or (last_local_call_time >= '$shipdate 00:00:00' and last_local_call_time <= '$shipdate 23:59:59') )";
}
$filename_prefix = '';
...
if ( !$Q ) {
    ...
    print
"This program is designed to gather agent activity and  logs post them to a file over FTP. \n";
    print "\n";
...
$stmtA =
"SELECT user,CONVERT_TZ(event_time,$convert_tz),lead_id,campaign_id,pause_sec,wait_sec,talk_sec,dispo_sec,dead_sec,status,agent_log_id from $vicidial_agent_log where $VALcampaignSQL and $VALdateSQL order by agent_log_id;";
if ($DBX) { print "|$stmtA|\n"; }
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArows  = $sthA->rows;
...
while ( $sthArows > $rec_count ) {
    ...
    if ( $output_format =~ /^pipe-basic$/ ) {
        $str =
"$user|$event_time|$lead_id|$campaign_id|$pause_sec|$wait_sec|$talk_sec|$dispo_sec|$dead_sec|$status|$agent_log_id\n";
    }
    if ( $output_format =~ /^tab-basic$/ ) {
        $str =
"$user\t$event_time\t$lead_id\t$campaign_id\t$pause_sec\t$wait_sec\t$talk_sec\t$dispo_sec\t$dead_sec\t$status\t$agent_log_id\n";
...
"$user,$event_time,$lead_id,$campaign_id,$pause_sec,$wait_sec,$talk_sec,$dispo_sec,$dead_sec,$status,$agent_log_id\n";
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print
              "$rec_count|$TOTAL_VAL|         |$user|$lead_id|$event_time|\n";
        }
    }
    $TOTAL_VAL++;
    $rec_count++;
...
$stmtA =
"SELECT user,location,lead_id,recording_id,CONVERT_TZ(start_time,$convert_tz) from $recording_log where $RECdateSQL order by recording_id;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArows  = $sthA->rows;
$rec_count = 0;
...
while ( $sthArows > $rec_count ) {
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print
"$rec_count|$TOTAL_REC|         |$user|$recording_id|$start_time|\n";
        }
    }
    $TOTAL_REC++;
    $rec_count++;
...
while ( $sthArows > $rec_count ) {
    @aryA    = $sthA->fetchrow_array;
    $lead_id = $aryA[0];
    if ( $quoted_vl > 0 ) { $lead_id = '"' . $lead_id . '"'; }
    $entry_date = $aryA[1];
    if ( $quoted_vl > 0 ) { $entry_date = '"' . $entry_date . '"'; }
    $modify_date = $aryA[2];
    if ( $quoted_vl > 0 ) { $modify_date = '"' . $modify_date . '"'; }
    $status = $aryA[3];
    if ( $quoted_vl > 0 ) { $status = '"' . $status . '"'; }
    $user = $aryA[4];
    if ( $quoted_vl > 0 ) { $user = '"' . $user . '"'; }
    $vendor_lead_code = $aryA[5];
    if ( $quoted_vl > 0 ) { $vendor_lead_code = '"' . $vendor_lead_code . '"'; }
    $source_id = $aryA[6];
    if ( $quoted_vl > 0 ) { $source_id = '"' . $source_id . '"'; }
    $list_id = $aryA[7];
    if ( $quoted_vl > 0 ) { $list_id = '"' . $list_id . '"'; }
    $gmt_offset_now = $aryA[8];
    if ( $quoted_vl > 0 ) { $gmt_offset_now = '"' . $gmt_offset_now . '"'; }
    ...
    $phone_code = $aryA[10];
    if ( $quoted_vl > 0 ) { $phone_code = '"' . $phone_code . '"'; }
    $phone_number = $aryA[11];
    if ( $quoted_vl > 0 ) { $phone_number = '"' . $phone_number . '"'; }
    $title = $aryA[12];
    if ( $quoted_vl > 0 ) { $title = '"' . $title . '"'; }
    $first_name = $aryA[13];
    if ( $quoted_vl > 0 ) { $first_name = '"' . $first_name . '"'; }
    $middle_initial = $aryA[14];
    if ( $quoted_vl > 0 ) { $middle_initial = '"' . $middle_initial . '"'; }
    $last_name = $aryA[15];
    if ( $quoted_vl > 0 ) { $last_name = '"' . $last_name . '"'; }
    $address1 = $aryA[16];
    if ( $quoted_vl > 0 ) { $address1 = '"' . $address1 . '"'; }
    $address2 = $aryA[17];
    if ( $quoted_vl > 0 ) { $address2 = '"' . $address2 . '"'; }
    $address3 = $aryA[18];
    if ( $quoted_vl > 0 ) { $address3 = '"' . $address3 . '"'; }
    $city = $aryA[19];
    if ( $quoted_vl > 0 ) { $city = '"' . $city . '"'; }
    $state = $aryA[20];
    if ( $quoted_vl > 0 ) { $state = '"' . $state . '"'; }
    $province = $aryA[21];
    if ( $quoted_vl > 0 ) { $province = '"' . $province . '"'; }
    $postal_code = $aryA[22];
    if ( $quoted_vl > 0 ) { $postal_code = '"' . $postal_code . '"'; }
    $country_code = $aryA[23];
    if ( $quoted_vl > 0 ) { $country_code = '"' . $country_code . '"'; }
    $gender = $aryA[24];
    if ( $quoted_vl > 0 ) { $gender = '"' . $gender . '"'; }
    $date_of_birth = $aryA[25];
    if ( $quoted_vl > 0 ) { $date_of_birth = '"' . $date_of_birth . '"'; }
    $alt_phone = $aryA[26];
    if ( $quoted_vl > 0 ) { $alt_phone = '"' . $alt_phone . '"'; }
    $email = $aryA[27];
    if ( $quoted_vl > 0 ) { $email = '"' . $email . '"'; }
    $security_phrase = $aryA[28];
    if ( $quoted_vl > 0 ) { $security_phrase = '"' . $security_phrase . '"'; }
    $comments = $aryA[29];
    if ( $quoted_vl > 0 ) { $comments = '"' . $comments . '"'; }
    $called_count = $aryA[30];
    if ( $quoted_vl > 0 ) { $called_count = '"' . $called_count . '"'; }
    ...
    $rank = $aryA[32];
    if ( $quoted_vl > 0 ) { $rank = '"' . $rank . '"'; }
    $owner = $aryA[33];
    if ( $quoted_vl > 0 ) { $owner = '"' . $owner . '"'; }
    $entry_list_id = $aryA[34];
    if ( $quoted_vl > 0 ) { $entry_list_id = '"' . $entry_list_id . '"'; }
    if ( $output_format =~ /^pipe-basic$/ ) {
        $str =
...
"$lead_id,$entry_date,$modify_date,$status,$user,$vendor_lead_code,$source_id,$list_id,$gmt_offset_now,$called_since_last_reset,$phone_code,$phone_number,$title,$first_name,$middle_initial,$last_name,$address1,$address2,$address3,$city,$state,$province,$postal_code,$country_code,$gender,$date_of_birth,$alt_phone,$email,$security_phrase,\"$comments\",$called_count,$last_local_call_time,$rank,$owner,$entry_list_id\n";
    ...
    if ( $DB > 0 ) {
        ...
        if ( $rec_count =~ /000$/i ) {
            print
              "$rec_count|$TOTAL_VL|         |$user|$lead_id|$modify_date|\n";
        }
    }
    $TOTAL_VL++;
    $rec_count++;
...
if ( $ftp_transfer > 0 ) {
    ...
    $ftp =
      Net::FTP->new( "$VARREPORT_host", Port => $VARREPORT_port, Debug => $DB );
