...
$SCstatuses = ' CBHOLD';
$stmtA =
"SELECT status from vicidial_statuses where scheduled_callback='Y' limit 10000000;";
...
$sthA->finish();
$stmtA =
"SELECT status from vicidial_campaign_statuses where scheduled_callback='Y' limit 10000000;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
$sthArowsSCS = $sthA->rows;
$rec_count   = 0;
...
$stmtA =
  "SELECT lead_id,status,callback_id from vicidial_callbacks limit 10000000;";
$sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
$sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
...
while ( $sthArowsCT > $rec_count ) {
    ...
    if ( $lead_statuses[$rec_count] =~ /INACTIVE/ ) {
        ...
        $stmtA =
"SELECT status from vicidial_list where lead_id='$lead_ids[$rec_count]';";
        $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
        $sthA->execute or die "executing: $stmtA ",          $dbhA->errstr;
        $sthArows = $sthA->rows;
        if ( $sthArows > 0 ) {
            ...
            if (   ( $purge_non_cb > 0 )
                && ( $SCstatuses !~ /$callback_statuses[$rec_count]/ ) )
            {
                $delete_lead++;
            }
        }
    ...
    if ($DBX) {
        print STDERR
"$rec_count|  |$lead_ids[$rec_count]|$lead_statuses[$rec_count]|$callback_statuses[$rec_count]|$callback_ids[$rec_count]|\n";
    }
    if ( $delete_lead > 0 ) {
        $stmtA =
"INSERT INTO vicidial_callbacks_archive SELECT * from vicidial_callbacks where callback_id='$callback_ids[$rec_count]';";
        if ( !$T ) { $affected_rowsA = $dbhA->do($stmtA); }
        $stmtA =
"DELETE from vicidial_callbacks where callback_id='$callback_ids[$rec_count]';";
...
if ( $remove_dup_cb > 0 ) {
    ...
    $stmtA =
"SELECT lead_id,callback_id from vicidial_callbacks order by lead_id, callback_id desc limit 10000000;";
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArowsCT = $sthA->rows;
    $rec_count  = 0;
    ...
    if ($DBX) {
        print STDERR
"        FIRST |$lead_ids[0]|$last_lead_id|   |$callback_ids[0]|$last_callback_id|\n";
    }
    while ( $sthArowsCT > $rec_count ) {
        $delete_lead = 0;
        if ( $rec_count > 0 ) {
            if ( $lead_ids[$rec_count] =~ /^$last_lead_id$/ ) {
                ...
                if ($DBX) {
                    print STDERR
"          DUP |$lead_ids[$rec_count]|$last_lead_id|   |$callback_ids[$rec_count]|$last_callback_id|\n";
                }
            }
            else {
                if ($DBX) {
                    print STDERR
"          NON |$lead_ids[$rec_count]|$last_lead_id|   |$callback_ids[$rec_count]|$last_callback_id|\n";
                }
            ...
            if ($DB) {
                print STDERR
"$rec_count|  |$lead_ids[$rec_count]|$callback_ids[$rec_count]|$delete_lead|\n";
            }
            if ( $delete_lead > 0 ) {
                $stmtA =
"INSERT INTO vicidial_callbacks_archive SELECT * from vicidial_callbacks where callback_id='$callback_ids[$rec_count]';";
