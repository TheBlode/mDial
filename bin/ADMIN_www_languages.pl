...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        print
"ADMIN_www_languages.pl - analyze php scripts for QXZ functions and contents\n";
        print "\n";
        print "options:\n";
        ...
        print
          "  [--one-file-only=/path/from/root] = process one php file only\n";
...
"  [--conffile=/path/from/root] = define astguiclient.conf config file to use\n";
        print
"  [--wipe-www-phrases-table] = deletes all records in www_phrases database table\n";
        print "\n";
        exit;
    }
    else {
        ...
        if ( $args =~
            /--one-file-only=/i )    # CLI defined file path for one file only
        {
...
if ( length($PATHone) > 2 ) {
    ...
    if ( ( $vicidial_only < 1 ) && ( $chat_only < 1 ) ) {
        ...
        foreach (@agcFILES) {
            ...
            $FILEparseNAME[$f] = '';
            $FILEparse[$f]     = '';
            if (   ( length( $agcFILES[$i] ) > 2 )
                && ( !-d "$PATHweb/agc/$agcFILES[$i]" ) )
            {
                $FILEparse[$f]     = "$PATHweb/agc/$agcFILES[$i]";
                $FILEparseNAME[$f] = $agcFILES[$i];
                $event_string =
                  "$PATHweb/agc/$agcFILES[$i]   $FILEparseNAME[$f]";
                if ( $DBX > 0 ) { print "$event_string\n"; }
                &event_logger;
                $f++;
            }
    ...
    if ( ( $agc_only < 1 ) && ( $chat_only < 1 ) ) {
        ...
        foreach (@vicidialFILES) {
            ...
            $FILEparseNAME[$f] = '';
            $FILEparse[$f]     = '';
            if (   ( length( $vicidialFILES[$i] ) > 2 )
                && ( !-d "$PATHweb/vicidial/$vicidialFILES[$i]" ) )
            {
                $FILEparse[$f]     = "$PATHweb/vicidial/$vicidialFILES[$i]";
                $FILEparseNAME[$f] = $vicidialFILES[$i];
                $event_string =
                  "$PATHweb/vicidial/$vicidialFILES[$i]   $FILEparseNAME[$f]";
                if ( $DBX > 0 ) { print "$event_string\n"; }
                &event_logger;
                $f++;
            }
    ...
    if ( ( $vicidial_only < 1 ) && ( $agc_only < 1 ) ) {
        ...
        foreach (@chat_customerFILES) {
            ...
            $FILEparseNAME[$f] = '';
            $FILEparse[$f]     = '';
            if (   ( length( $chat_customerFILES[$i] ) > 2 )
                && ( !-d "$PATHweb/chat_customer/$chat_customerFILES[$i]" ) )
            {
                $FILEparse[$f] =
                  "$PATHweb/chat_customer/$chat_customerFILES[$i]";
                $FILEparseNAME[$f] = $chat_customerFILES[$i];
                $event_string =
"$PATHweb/chat_customer/$chat_customerFILES[$i]   $FILEparseNAME[$f]";
                if ( $DBX > 0 ) { print "$event_string\n"; }
                &event_logger;
                $f++;
            }
            ...
            if ( ( $line =~ /\_QXZ\(/ ) && ( $line !~ /function \_QXZ/ ) ) {
                ...
                while ( $each_ct <= $#eachQXZ ) {
                    if ( $each_ct > 0 ) {
                        ...
                        if (   ( $eachQXZvalue[0] =~ /\(/ )
                            && ( length( $eachQXZvalue[1] ) > 0 ) )
                        {
                            $temp_QXZval_str =
                              $eachQXZvalue[0] . ")" . $eachQXZvalue[1];
                            if (   ( $eachQXZvalue[1] =~ /\(/ )
                                && ( length( $eachQXZvalue[2] ) > 0 ) )
                            {
                                $temp_QXZval_str .= ")" . $eachQXZvalue[2];
                                if (   ( $eachQXZvalue[2] =~ /\(/ )
                                    && ( length( $eachQXZvalue[3] ) > 0 ) )
                                {
                        ...
                        if ( $temp_QXZval_str =~ /\$/ ) {
                            ...
                            $QXZcount_var_ct =
                              ( $QXZcount_var_ct + $var_ct_in_line );
                            if ( $QXZvaronly > 0 ) {
                                $event_string = "$temp_QXZval_str";
                                print "$event_string\n";
                                &event_logger;
                        ...
                        if ( $temp_QXZval_str =~ /%\ds/ ) {
                            ...
                            $QXZcount_place_ct =
                              ( $QXZcount_place_ct + $place_ct_in_line );
                            if ( $QXZplaceonly > 0 ) {
                                $event_string = "$temp_QXZval_str";
                                print "$event_string\n";
                                &event_logger;
                        ...
                        $phrase_match = 0;
                        $stmtA =
"SELECT count(*) FROM www_phrases where phrase_text='$temp_QXZval_strSQL';";
                        ...
                        if ( $phrase_match < 1 ) {
                            if ( length($temp_QXZval_strSQL) < 1 ) {
                                if ( $DB > 0 ) {
                                    print
"EMPTY INSERT SKIPPED: |$temp_QXZval_strSQL|$FILEparseNAME[$f]|$FILEparseDIR[$f]|\n";
                                }
                            }
                            else {
                                $stmtA =
"INSERT INTO www_phrases SET phrase_text='$temp_QXZval_strSQL',php_filename='$FILEparseNAME[$f]',php_directory='$FILEparseDIR[$f]',source='www_languages_script',insert_date=NOW();";
                                $affected_rows = $dbhA->do($stmtA);
                                $QXZinserts++;
                                $event_string = "     $affected_rows|$stmtA|";
                                if ( $DBX > 0 ) { print "$event_string\n"; }
        ...
        $event_string =
          "File does not exist: $FILEparse[$f]   $FILEparseNAME[$f]";
        if ( $DB > 0 ) { print "$event_string\n"; }
        &event_logger;
    }
    $f++;
