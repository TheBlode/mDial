...
if ( -e "/usr/local/bin/tail" ) { $bintail = "/usr/local/bin/tail"; }
else {
    if ( -e "/bin/tail" ) { $bintail = "/bin/tail"; }
}
...
$stmtA =
"INSERT INTO vicidial_asterisk_output (server_ip, sip_peers, update_date) VALUES ('$server_ip',\"$sip_show_peers\",NOW()) ON DUPLICATE KEY UPDATE sip_peers=\"$sip_show_peers\",update_date=NOW();";
$affected_rows =
...
$stmtA =
"UPDATE vicidial_asterisk_output SET iax_peers=\"$iax_show_peers\",update_date=NOW() where server_ip='$server_ip';";
$affected_rows =
  $dbhA->do($stmtA);    #  or die  "Couldn't execute query:|$stmtA|\n";
if ($DB) { print STDERR "\n|$affected_rows|$stmtA|\n"; }
$i++;
...
$stmtA =
"UPDATE vicidial_asterisk_output SET asterisk=\"$tail_output\",update_date=NOW() where server_ip='$server_ip';";
$affected_rows =
  $dbhA->do($stmtA);    #  or die  "Couldn't execute query:|$stmtA|\n";
$dbhA->disconnect();
