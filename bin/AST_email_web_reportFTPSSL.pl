...
"  [--begin-date=YYYY-MM-DD] = begin date override, can also use '6days', '7days', '8days' and '30days', if not filled in will only use the --date\n";
        ...
        print
          "  [--ftp-port=XXX] = FTP SSL port number, optional, default is 21\n";
        print
"  [--ftp-dir=XXXXXXXX] = remote FTP SSL server directory to post files to\n";
        print
"  [--ftp-encrypt=X] = FTPSSL encryption type, E=Explicit, I=Implicit, default=E\n";
        print "\n";
        exit;
    }
    else {
...
if ( !$Q ) {
    ...
    print
"This program is designed to wget a web report and email or FTPSSL it. \n";
    print "\n";
}
if ( length($report_id) > 1 ) {
    $stmtA =
