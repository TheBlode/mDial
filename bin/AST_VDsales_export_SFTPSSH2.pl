...
if ( $ftp_transfer > 0 ) {
    if ( $sftp_mode > 0 ) {
        ...
        if ( $DB > 0 ) { $ssh2->debug("1"); }
        $ssh2->connect( "$VARREPORT_host", "$VARREPORT_port" )
          or die "Unable to connect Host $@ \n";
        $ssh2->auth_password( "$VARREPORT_user", "$VARREPORT_pass" )
          or die "Unable to login $@ \n";
        $ssh2->scp_put( "$PATHweb/vicidial/server_reports/$outfile",
            "$VARREPORT_dir/$outfile" );
        $ssh2->disconnect();
    }
    else {
        use Net::FTP;
