...
if ( length( $ARGV[0] ) > 1 ) {
    ...
    if ( $args =~ /--help/i ) {
        ...
        print
"  [--GSW] = look for already compressed GSM codec with RIFF headers and .wav extension files\n";
...
"  [--max-files=x] = maximum number of files to process, defaults to 100000\n";
        print
"  [--recipients=x\@y.com---z\@a.net] = encryption recipients addresses, REQUIRED\n";
        print
"    NOTE: all recipients must be loaded into keys on local server and accessible to gpg binary\n";
        print "\n";
        exit;
    }
    else {
        ...
        else { print "ERROR: recipient is required, exiting..."; exit; }
        if ( $args =~ /--max-files=/i ) {
                ...
                if ( $args =~ /--MP3/i ) {
                    ...
                    if ( $args =~ /--OGG/i ) {
                        ...
                        if ( $args =~ /--GSW/i ) {
                            ...
                            print "ERROR: no audio format defined, exiting...";
                            exit;
                        }
                    }
                }
    ...
    print
      "ERROR: cannot run script without any command line options, exiting...";
    exit;
}
...
foreach (@FILES) {
        ...
        && ( $maxfiles > $i ) )
    {
            ...
            && ( length( $FILES[$i] ) > 4 ) )
        {
            ...
            $stmtA =
"select recording_id from recording_log where filename='$SQLFILE' order by recording_id desc LIMIT 1;";
            if ($DBX) { print STDERR "\n|$stmtA|\n"; }
            ...
            if ( $GPG > 0 ) {
                ...
                if ($DB) {
                    print
                      "$i|$recording_id|$ALLfile|$GPGfile|     |$SQLfile|\n";
                }
`$gpgbin --trust-model always --output $PATHDONEmonitor/GPG/$GPGfile --encrypt --recipient $CLIrecipient $dir2/$ALLfile`;
                $stmtA =
"UPDATE recording_log set location='$HTTP://$server_ip/RECORDINGS/GPG/$GPGfile' where recording_id='$recording_id';";
                if ($DBX) { print STDERR "\n|$stmtA|\n"; }
