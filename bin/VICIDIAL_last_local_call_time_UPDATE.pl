...
print
"populates the new last_local_call_time field in the vicidial_list table from records in the vicidial_log and vicidial_closer_log table. \n\n";
$stmtA =
  "select lead_id,list_id,called_count from vicidial_list limit 10000000;";
if ($DBX) { print STDERR "\n|$stmtA|\n"; }
...
foreach (@lead_id) {
    ...
    $Cepoch     = 0;
    $stmtA =
"select call_date,start_epoch from vicidial_log where lead_id='$lead_id[$b]' order by call_date desc LIMIT 1;";
    if ($DBX) { print STDERR "\n|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
    ...
    $sthA->finish();
    $stmtA =
"select call_date,start_epoch from vicidial_closer_log where lead_id='$lead_id[$b]' order by closecallid desc LIMIT 1;";
    if ($DBX) { print STDERR "\n|$stmtA|\n"; }
    $sthA = $dbhA->prepare($stmtA) or die "preparing: ",        $dbhA->errstr;
    $sthA->execute                 or die "executing: $stmtA ", $dbhA->errstr;
    $sthArows = $sthA->rows;
    ...
    if ( $calculate_recycle_counts > 0 ) {
        if ( ( $Ncount[$b] == 0 ) or ( $Ncount[$b] == '' ) ) {
            $Ncount[$b] = 'N';
        }
        if ( $Ncount[$b] == 1 ) { $Ncount[$b] = 'Y'; }
        if ( ( $Ncount[$b] > 1 ) && ( $Ncount[$b] < 11 ) ) {
            $Ncount[$b] = ( $Ncount[$b] - 1 );
            $Ncount[$b] = "Y$Ncount[$b]";
        }
    ...
    else { $called_since_last_resetSQL = ""; }
    $stmtA =
"UPDATE vicidial_list set last_local_call_time='$NEWcall_date' $called_since_last_resetSQL where lead_id='$lead_id[$b]';";
    $affected_rows =
      $dbhA->do($stmtA);    #  or die  "Couldn't execute query:|$stmtA|\n";
    if ($DB) {
        print STDERR
"|$b|$lead_id[$b]|$Ncall_date|$Ccall_date|$list_id[$b]|$Ncount[$b]|$stmtA|\n";
    }
    $b++;
    ...
    if ( $b =~ /000$/i ) {
        print "|$b|$lead_id[$b]|$Ncall_date|$Ccall_date|$list_id[$b]|\n";
    }
}
$dbhA->disconnect();
$secY  = time();
